...
// parsed frontmatter declaration — the key's identity, structural equality
// of resolved values, the union match of two parsed frontmatters, and the
// shared rules for drawing a matched row — which is a property of
// `FrontmatterValueDto` itself, not of any one kind's comparison model.
 ...
 * where it offered a parsed frontmatter to match.
 *
 ...
 * both publish the rendered key `1` (api-types.ts § FrontmatterKeyKind), so
...
/**
 * Matches two parsed frontmatters by the parser's key identity — parsed
 ...
 * non-empty sequence or mapping opens `FrontmatterBlock` and everything else
 * stays a text line.
 */
export function valueOpensBlock(value: FrontmatterValueDto): boolean {
  return (
