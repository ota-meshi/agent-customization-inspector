...
// The prompt-and-command comparison's read-only authored-source diff (T505;
// research.md § 7, FR-011, FR-027). This kind's own copy of the surface
// (spec.md § Clarifications Session 2026-08-14) with no absent-side
// handling: a comparison of this kind is exactly two committed files that
// resolve one invocation name, so neither side can be a stated absence.
//
...
//
// A name the compared prompt or command mentions — an agent, a skill,
// another command, a `#file` target — is text on both sides like every other
...
const unregisterContentOwner = sessionViewState?.promptComparison.registerOpenContentOwner(() => {
...
<template>
  <div
    v-show="!mountError"
    ref="host"
    class="aci-prompt-source-diff"
    :class="{ 'aci-prompt-source-diff--fit': fitContent }"
  />
  ...
  <div v-if="mountError" class="aci-prompt-source-diff__fallback">
    <section
      v-for="side in [
      ...
      :key="side.caption"
      class="aci-prompt-source-diff__fallback-side"
    >
      ...
      <h4
        class="aci-prompt-source-diff__fallback-caption"
      ...
      <pre ref="fallbackElements" class="aci-prompt-source-diff__fallback-source" tabindex="0">{{
        purged ? '' : side.text
      }}</pre>
    </section>
  </div>
...
.aci-prompt-source-diff__fallback {
  display: grid;
  gap: 0.75rem;
...
.aci-prompt-source-diff__fallback-caption {
  font-size: 0.95rem;
  margin: 0 0 0.35rem;
}
...
.aci-prompt-source-diff__fallback-source {
  border: 1px solid var(--aci-border);
  border-radius: 4px;
  margin: 0;
  max-block-size: 28rem;
