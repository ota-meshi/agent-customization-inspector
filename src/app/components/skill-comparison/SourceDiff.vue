...
<template>
  <div v-show="!mountError" ref="host" class="aci-source-diff" />
  ...
  <div v-if="mountError" class="aci-source-diff__fallback">
    <section
      v-for="side in [
        {
          path: originalPath,
      ...
      :key="side.caption"
      class="aci-source-diff__fallback-side"
    >
      ...
      <pre
        ...
        class="aci-source-diff__fallback-source"
        tabindex="0"
...
.aci-source-diff__fallback-caption {
  font-size: 0.95rem;
  margin: 0 0 0.35rem;
}
...
.aci-source-diff__fallback-source {
  border: 1px solid var(--aci-border);
  border-radius: 4px;
  margin: 0;
