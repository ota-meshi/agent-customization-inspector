...
// The custom-agent comparison's read-only diff of one half of each compared
// file (T575; research.md § 7, FR-011). This kind's own copy of the surface
// (spec.md § Clarifications Session 2026-08-14) with no absent-side handling:
// a comparison of this kind is exactly two committed files that resolve one
// agent name, so neither side can be a stated absence.
//
// What it is given is a section rather than a file: the declared metadata
// serialized to a canonical document, or the instructions taken out of the
// format that held them. It is never a file's own bytes — two files written
...
// side is handed to the editor as the string the comparison built. There is no `v-html`, no link, and no image load anywhere here,
// and no control edits, merges, or reverts either side (FR-012).
//
// A name the compared agent mentions — a handoff, a preloaded skill, an
// `mcp_servers` command, a hook script — is text on both sides like every
// other line: highlighting is tokenizing rather than rendering, so nothing is
 ...
 * which is what the second sentence points the reader at.
 */
...
// The models this component mounts hold both files' authored source, so it
// is an owner the comparison state must clear synchronously — on the
// central purge (FR-027) and before a greater generation is adopted
// (data-model.md § BrowserState). Optional, because this component's
// contract is its props — a harness that renders it without the shell simply
// has no owner registry to join.
const sessionViewState = inject(SESSION_VIEW_STATE, undefined);
const unregisterContentOwner = sessionViewState?.customAgentComparison.registerOpenContentOwner(
  () => {
...
<template>
  ...
  <div v-if="mountError" class="aci-custom-agent-source-diff__fallback">
    <section
      v-for="side in [
      ...
      :key="side.caption"
      class="aci-custom-agent-source-diff__fallback-side"
    >
      ...
      <h4
        class="aci-custom-agent-source-diff__fallback-caption"
      ...
      <pre
        ...
        class="aci-custom-agent-source-diff__fallback-source"
        tabindex="0"
...
.aci-custom-agent-source-diff__fallback {
  display: grid;
  gap: 0.75rem;
...
.aci-custom-agent-source-diff__fallback-caption {
  font-size: 0.95rem;
  margin: 0 0 0.35rem;
}
