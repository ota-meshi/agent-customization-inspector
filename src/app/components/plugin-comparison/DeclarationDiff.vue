...
// The plugin declaration comparison surface (T831; research.md § 7, FR-011,
// FR-027): one plugin name's two declarations, each the JSON document that
// carrier declares it with, diffed read-only in Monaco. A catalog's side is
// its entry serialized to canonical JSON (declared-entries-json.ts) while a
// manifest's is the file it is, so both models are created as `json`
// whatever the carriers' own extensions would resolve to
...
const props = defineProps<{
   ...
   * so one plugin's declaration is never announced as the whole carrier
   * (FR-025).
   */
...
<template>
  ...
  <div v-if="mountError" class="aci-plugin-declaration-diff__fallback">
    <section
      v-for="side in [
      ...
      :key="side.caption"
      class="aci-plugin-declaration-diff__fallback-side"
    >
      ...
      <h4
        class="aci-plugin-declaration-diff__fallback-caption"
      ...
      <pre
        ...
        class="aci-plugin-declaration-diff__fallback-source"
        tabindex="0"
...
.aci-plugin-declaration-diff__fallback {
  display: grid;
  gap: 0.75rem;
...
.aci-plugin-declaration-diff__fallback-caption {
  font-size: 0.95rem;
  margin: 0 0 0.35rem;
}
