...
// The instruction comparison's read-only authored-source diff (T278;
// research.md § 7, FR-011, FR-027). The instruction kind's own copy of the
// surface (spec.md § Clarifications Session 2026-08-14) with no absent-side
// handling: an instruction comparison is exactly two committed files, so
// neither side can be a stated absence.
//
...
const unregisterContentOwner = sessionViewState?.instructionComparison.registerOpenContentOwner(
  () => {
...
<template>
  ...
  <div v-if="mountError" class="aci-instruction-source-diff__fallback">
    <section
      v-for="side in [
      ...
      :key="side.caption"
      class="aci-instruction-source-diff__fallback-side"
    >
      ...
      <pre
        ...
        class="aci-instruction-source-diff__fallback-source"
        tabindex="0"
...
.aci-instruction-source-diff__fallback {
  display: grid;
  gap: 0.75rem;
...
.aci-instruction-source-diff__fallback-caption {
  font-size: 0.95rem;
  margin: 0 0 0.35rem;
}
...
.aci-instruction-source-diff__fallback-source {
  border: 1px solid var(--aci-border);
  border-radius: 4px;
  margin: 0;
  max-block-size: 28rem;
