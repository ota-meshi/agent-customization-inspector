...
// path>`. An instruction row's unit is the file itself, so the path alone is
// the route's identity — no tool segment exists, because no per-tool fact
// (an invocation name, a per-tool parse) distinguishes what the page would
// show, and the recognizing tools are the inventory row's own statement. The
