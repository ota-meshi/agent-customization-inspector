...
watch([rowName, leftPath, rightPath], () => {
...
function switchTo(left: string, right: string): void {
  ...
  void router.replace(customAgentComparisonRouteFor(rowName.value ?? '', left, right));
}
...
const owningRow = computed<AgentInventoryEntryDto | null>(() => {
  ...
  for (const entry of snapshot.value?.agents ?? []) {
    ...
    if (paths.has(currentLeftPath.value) && paths.has(currentRightPath.value)) {
      return entry;
    }
  }
  return null;
...
function fileFacts(detail: FileDetailDto): string {
  const facts = [
    ...
    // so a `.claude/agents/CLAUDE.md`, a subagent by its directory and an
 ...
 */
function definitionsOf(sourceRelativePath: string): readonly CustomAgentSideDefinition[] {
  return (snapshot.value?.agents ?? []).flatMap((entry) =>
    entry.definitions
      .filter(
        (definition: AgentDefinitionDto) => definition.sourceRelativePath === sourceRelativePath,
      )
