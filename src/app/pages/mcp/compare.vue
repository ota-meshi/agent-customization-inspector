...
// route already use (FR-030). A selection the model cannot express — a name
...
function switchTo(left: string, right: string): void {
  ...
  void router.replace(mcpComparisonRouteFor(subjectName.value ?? '', left, right));
}
 ...
 * this release is the repository's.
 */
    ...
    sides: [
      {
        ...
        path: leftPath,
        detail: left,
        attribution: attributionText(leftPath),
      },
      {
        caption: 'Second file',
        path: rightPath,
        detail: right,
        attribution: attributionText(rightPath),
      },
    ] as const,
    leftPath,
    rightPath,
