// The tab strip a subject view shows one half of its subject through (QR-004,
...
/** What one subject view hands its tab strip; see the module header. */
export interface SubjectTabsOptions<Tab extends string> {
...
/** One subject view's tab strip; see the module header. */
export class SubjectTabs<Tab extends string> {
  /** The kind's own tabs, in the order the strip draws them. */
  ...
  /** The word both `id`s are built from; see {@link SubjectTabsOptions}. */
  readonly #idPrefix: string;

  /** Holds the kind's tabs and opens on the one it fixes as its first. */
  public constructor(options: SubjectTabsOptions<Tab>) {
  ...
  public select(tab: Tab): void {
    ...
    const hidden = document.getElementById(this.panelId(this.#activeTab.value));
...
/** Holds one subject view's tab strip; see {@link SubjectTabs}. */
export function useSubjectTabs<Tab extends string>(
  options: SubjectTabsOptions<Tab>,
): SubjectTabs<Tab> {
  return new SubjectTabs(options);
}
