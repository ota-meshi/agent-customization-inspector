// The custom-agent comparison view state for the browser SPA (T575;
// FR-011, FR-030, data-model.md § BrowserState · ComparisonSelection).
//
// This surface is the `agent` kind's, not a shared one: comparison is
...
// the agent name's row standing where the skill name's row stands. A custom
// agent is one file complete in itself, with no copy directory and no
...
// surface: a Codex TOML agent, a Claude Code subagent, and a Copilot agent
// profile all reach this page, and a TOML agent standing opposite a Markdown
...
//
// The name is carried rather than derived, unlike the skill and prompt routes,
// because this kind's rows genuinely overlap: a product names an agent by its
// declared `name` and another by the configuration file's own name, so one
// file sits on one row per product that names it and two files can share more
// than one row. Deriving the row from the paths would pick whichever name
// sorts first, and the compare page's pickers — which offer that row's
// files — would silently drop a third file the reader opened from.
//
// There is no standing pre-selection state, and a pair the model does not
// express — one file twice, or a name whose row does not hold both files — is
 ...
 */
export type CustomAgentComparisonViewStatus =
  ...
  | 'not-readable'
  /** A request failed ordinarily; see {@link CustomAgentComparisonState.errorMessage}. */
  | 'failed';
 ...
 * two files can share more than one name and a derived row would be whichever
 * of them sorts first — dropping a third file of the row the reader opened
 * from out of the compare page's pickers. A module function beside the state
...
/** Construction inputs for {@link CustomAgentComparisonState}. */
export interface CustomAgentComparisonStateOptions {
...
/**
 * The one open custom-agent comparison view (FR-011). Every public member has
 * a render site or caller in the compare route; nothing is exposed only so a
 * test can read it.
 */
export class CustomAgentComparisonState {
  ...
  /** See {@link CustomAgentComparisonStateOptions.refreshFreshly}. */
  readonly #refreshFreshly: () => Promise<void>;

  /** See {@link CustomAgentComparisonStateOptions.reportFatalFailure}. */
  readonly #reportFatalFailure: (error: Error) => void;

  /** Where the one open comparison stands; see {@link CustomAgentComparisonViewStatus}. */
  public readonly status = shallowRef<CustomAgentComparisonViewStatus>('idle');
  /**
  ...
  public constructor(options: CustomAgentComparisonStateOptions) {
