...
import {
  ...
  COPILOT_EXCLUDED_USER_RUNTIME_RULE,
  COPILOT_EXCLUDED_VSCODE_SETTINGS_RULE,
  COPILOT_GLOBAL_AGENT_RULE,
  COPILOT_GLOBAL_HOOKS_RULE,
  COPILOT_GLOBAL_INSTRUCTIONS_PATH_RULE,
  COPILOT_GLOBAL_INSTRUCTIONS_ROOT_RULE,
  COPILOT_GLOBAL_MCP_RULE,
  COPILOT_GLOBAL_SETTINGS_HOOKS_RULE,
  COPILOT_GLOBAL_SETTINGS_RULE,
  COPILOT_GLOBAL_SKILL_RULE,
...
export const COPILOT_RULE_RELATIONS: Readonly<Record<CopilotRuleId, RuleRelations>> = {
  ...
  /**
   * The Global instruction pair is based on the User lookups it admits — the
   * root file on the CLI's own, the path subtree also on the `~/.copilot`
   * subset of the VS Code user lookup — and explained by the layering
  ...
  /**
   * Both skill rules are based on both user skill lookups: each lookup names
   * both directories, and which rule admits a file is decided by the boundary
   * the file sits below rather than by which surface documented it (FR-045).
   */
  [COPILOT_GLOBAL_SKILL_RULE.ruleId]: {
    basedOnBehaviors: [COPILOT_CLI_USER_SKILLS_BEHAVIOR, COPILOT_VSCODE_USER_SKILLS_BEHAVIOR],
    explainedByStrategies: [
      COPILOT_CLI_SKILLS_SELECTION_STRATEGY,
      COPILOT_VSCODE_SKILLS_SELECTION_STRATEGY,
    ],
  },
  [COPILOT_AGENTS_HOME_SKILL_RULE.ruleId]: {
    basedOnBehaviors: [COPILOT_CLI_USER_SKILLS_BEHAVIOR, COPILOT_VSCODE_USER_SKILLS_BEHAVIOR],
    explainedByStrategies: [
      COPILOT_CLI_SKILLS_SELECTION_STRATEGY,
      COPILOT_VSCODE_SKILLS_SELECTION_STRATEGY,
  ...
  [COPILOT_GLOBAL_AGENT_RULE.ruleId]: {
    basedOnBehaviors: [COPILOT_CLI_USER_AGENTS_BEHAVIOR, COPILOT_VSCODE_USER_AGENTS_BEHAVIOR],
    explainedByStrategies: [
      COPILOT_CLI_AGENTS_SELECTION_STRATEGY,
      COPILOT_VSCODE_AGENTS_SELECTION_STRATEGY,
    ],
  },
  [COPILOT_GLOBAL_HOOKS_RULE.ruleId]: {
    basedOnBehaviors: [COPILOT_CLI_USER_HOOKS_BEHAVIOR, COPILOT_VSCODE_USER_HOOKS_BEHAVIOR],
    explainedByStrategies: [
      COPILOT_CLI_HOOKS_COMPOSITION_STRATEGY,
      COPILOT_VSCODE_HOOKS_COMPOSITION_STRATEGY,
    ],
  ...
  /**
   * The inline-hooks recognition rests on the hook lookup alone: a settings
   * behavior is a basis only for the settings rule over the same candidate,
   * because a hook recognition publishing nothing settings-derived must not
   * acquire a settings basis on the way (T624).
   */
  [COPILOT_GLOBAL_SETTINGS_HOOKS_RULE.ruleId]: {
    basedOnBehaviors: [COPILOT_CLI_USER_HOOKS_BEHAVIOR],
    explainedByStrategies: [COPILOT_CLI_HOOKS_COMPOSITION_STRATEGY],
  },
  [COPILOT_GLOBAL_SETTINGS_RULE.ruleId]: {
    basedOnBehaviors: [COPILOT_CLI_USER_SETTINGS_BEHAVIOR],
    explainedByStrategies: [COPILOT_CLI_SETTINGS_PRECEDENCE_STRATEGY],
  },
  [COPILOT_GLOBAL_MCP_RULE.ruleId]: {
    basedOnBehaviors: [COPILOT_CLI_USER_MCP_BEHAVIOR],
    explainedByStrategies: [COPILOT_CLI_MCP_SELECTION_STRATEGY],
  },
  /**
   * The User exclusion is based on every Copilot user surface no Global rule
   * admits. The three VS Code rows with admitted `COPILOT_HOME` subsets —
   * skills, agents, hooks — carry both backlinks: their profile and cross-home
   * locations stay excluded while their boundary subsets are admitted above
   * (contracts/vendors/github-copilot.md § Inspector Global rule).
   *
   * It is explained by no strategy: an exclusion says this product never
   * looks, so there is no composition for one to describe.
   */
  [COPILOT_EXCLUDED_USER_RUNTIME_RULE.ruleId]: {
    basedOnBehaviors: [
      COPILOT_CLI_USER_EXTENSIONS_BEHAVIOR,
      COPILOT_CLI_USER_LSP_BEHAVIOR,
      COPILOT_CLI_USER_PLUGINS_BEHAVIOR,
