...
//
// The read that derivation rests on is here too, beside the format, the key,
// and the plan it feeds (T1090): `.codex/config.toml` is the one pinned file
// whose declarations decide what this product's walk targets, and no other
// vendor this release recognizes configures a target at all.
//
...
/**
 * Reads the one configuration seed this product has (T1090): the repository's
 * own `.codex/config.toml`, probed at its pinned path and decoded through the
 * same single read path as every published file, beside the read it performed
 * — so the scan can seed the walk's classification cache with it and the
 * seed's own candidacy reuses this read instead of opening the file a second
 * time (T282).
 *
 ...
 * its own — the same path `codex.repo.config` admits — so the walk probes it
 * and publishes whatever it classifies there, and an unreadable one carries
 * `file-unreadable` in a partial generation (FR-028). A read that did happen
 * is seeded, so the walk classifies from this reader's bytes rather than
...
async function readConfigurationSeed(
  ...
  const seedSegments = ['.codex', 'config.toml'];
...
export async function readCodexConfiguredFallbackPlans(
  ...
  const seed = await readConfigurationSeed(root, continueScan);
