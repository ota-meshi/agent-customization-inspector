...
// the shipped `copilot.repo.skill` matcher, hands it to the one registry
 ...
 * only Copilot files are instructions or agents legitimately contributes
 * nothing to the inventory.
 *
 * Two of the three shipped selectors overlap other vendors' spellings — a
 * root `.agents` or `.claude` skill is one physical file the traversal reads
 * once and admits for each vendor's plan — so this list joining the scan
 * catalog is what turns those candidates into multi-tool recognitions rather
 * than duplicate rows. A nested `.claude` skill stays Claude's alone: Claude
 * documents lazy descendant discovery and Copilot documents no downward
 * lookup, so the two expansions legitimately differ on one shared spelling.
 *
 * Every shipped rule is compiled rather than filtered: a Copilot record that
 * authorizes no traversal is rejected by the {@link CopilotCompiledRule}
 ...
 */
export const COPILOT_REPOSITORY_RULES: readonly CopilotCompiledRule[] = Object.values(
  COPILOT_INSPECTION_RULES,
).map((rule) => new CopilotCompiledRule(rule));
