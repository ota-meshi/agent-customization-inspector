...
カレントディレクトリの中から、4つのツールが探しにいくカスタマイズファイルを並べたページが
ローカルで開きます。それが何なのか、どのツールが読むのか、そして中身が実際に何と書いてあるのか。

## 答えはこう出ます

![一覧画面。左は2つのSourceとそのスキャン状態が種類と件数の上に並び、各行はその名前・配下のファイル・各ファイルを読む製品とその根拠になっているsurface・複数のファイルが共有する名前の解決のされ方を示している](docs/images/inventory.png)
...
| `--inspect-personal-setup` | 上のディレクトリも調査します。これを付けること自体が同意で、ページ側では改めて尋ねません。 |
...
Spec Kit のスキルはリポジトリに入っています。Codex と Antigravity CLI 向けが
`.agents/skills/speckit-*`、Claude Code 向けが `.claude/skills/speckit-*`、Copilot 向けが
`.github/skills/speckit-*` なので、clone すればそれで準備は終わりで、`specify init` を走らせる
必要はありません。終わったタスクは `tasks.md` と `tasks.ja.md` の両方に、それを終わらせる
同じ変更でチェックを入れます。
