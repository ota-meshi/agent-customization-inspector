...
![一覧画面。左に種類と件数が並び、各行がファイル名・それを読むツール・同じ名前を持つ複数ファイルの解決のされ方を示している](docs/images/inventory.png)
...
![changelog という名前のスキルを2つのファイルで比較した画面。宣言されたメタデータ、指示本文、ソース全文が、それぞれ左右差分で並んでいる](docs/images/comparison.png)
...
`COPILOT_HOME` が設定されていればそちら）と、共有の `~/.agents` です。「個人設定を調査」を選ぶと、
読む前に対象の4か所が提示されます。`--inspect-personal-setup` はその確認をコマンドラインで与えるもの
