...
`CODEX_HOME`、`COPILOT_HOME`、`GEMINI_CLI_HOME` が設定されていればそちら）と、共有の `~/.agents`
です。左の「Sources」にある「Personal setup」を開くと、読む前に対象のディレクトリが提示されます。`--inspect-personal-setup` はその確認をコマンドラインで与えるもの
