...
/**
 * The declared task-ID space: T001 through T1227, less the ranges the withdrawn
...
const DECLARED_TASK_IDS: readonly string[] = Array.from({ length: 1227 }, (_, index) => index + 1)
...
describe('task set', () => {
  it('freezes the task and phase counts in both languages', () => {
    ...
    expect(englishTasks.size).toBe(1139);
    expect(japaneseTasks.size).toBe(1139);
