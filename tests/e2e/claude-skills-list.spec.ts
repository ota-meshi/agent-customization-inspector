...
test('lists every vendor\u2019s skills together, grouped by invocation name', async ({ page }) => {
  ...
  // is `claude-greet` to Copilot and to Claude Code alike, both invoking the
  // authored name — one grouping for every vendor, not one list per product
  // (FR-007).
...
test('badges each row with the exact products that recognized it', async ({ page }) => {
  await page.goto(host.origin);
  const items = page.locator('.aci-item');
  // The matrix as a user sees it: the root `.claude` row is Copilot+Claude,
  // which invoke one authored name; the root `.agents` row is Codex+Copilot,
  // likewise; and the nested `.claude` layer is Claude's alone — no Copilot
  // surface documents a downward lookup from a root context, so no Copilot
  // badge reaches it.
...
test('lists one file once under the name its recognizing products invoke it by', async ({
  page,
}) => {
  await page.goto(host.origin);
  // `.claude/skills/greet/SKILL.md` declares `name: claude-greet`. Copilot and
  // Claude Code both invoke what a root file declares, so the one file is one
  // row listing both — never a second row headed by its directory, a name no
  // menu of either product shows (FR-007).
  const shared = page.locator('.aci-item', { hasText: '.claude/skills/greet/SKILL.md' });
  await expect(shared.locator('.aci-row-head__name')).toHaveText(['claude-greet']);
  ...
  // is the vendor's own command spelling for a qualified nested skill
  // (FR-007, T1081).
...
test('groups a name declared from two locations into one row listing every recognizing product', async ({
  ...
  // rather than a row — Copilot and Claude Code on the `.claude` file, Codex
  // and Copilot on the `.agents` one — with the one applicable same-name
  // rule. Claude Code reads one of the two files, so it faces no collision
  // and states nothing (FR-007).
  ...
  // Neither Codex nor Claude Code has a collision here: each recognizes one
  // of the two files. Copilot recognizes both, so its statement — no single
...
test('quotes no Claude rule for two commands that only share a label', async ({ page }) => {
  // Two Claude skills in differently named directories declaring one name.
  // Both products list both files under it, but the clash Claude Code's
  // documented rule answers is between skill directories, and these two
  // differ — so that row quotes Copilot's rule alone, never a Claude rule for
  // a clash Claude does not have (FR-007).
...
test('names a nested skill with the root-relative prefix and states the Claude rule on both rows', async ({
  ...
  // depths. The root one is listed under its declared `name`, `claude-twice`;
  // the nested one is its own row under the root-relative qualified command —
  // `apps/web:wave`, the prefix from the directory holding its `.claude` and
  // the skill directory, the declared name unread (FR-007). Claude Code's
  // clash is between the two `wave` directories, so the official rule for a
  // clash within one root, that every definition stays available and Claude
  // picks the variant matching the files being worked on, is stated on both
  // rows — never a first-in-order winner, which is the rule between levels
  // this product lists as separate Sources.
  ...
  const root = page.locator('.aci-item', {
    has: page.locator('.aci-row-head__name', { hasText: /^claude-twice$/u }),
  });
...
test('filters the vendors apart with the tool filter', async ({ page }) => {
  ...
  // Claude Code's two rows: the root file under its authored name and the
  // nested layer under its directory-qualified command.
...
test('opens a listed file by its own identity into the detail route', async ({ page }) => {
  await page.goto(host.origin);
  // One row lists this file for both invoking products, and its link is the
  // file's own page: the products read the same document, so the route is
  // the file's own, and the row it was followed from rides in the query.
  const links = page
