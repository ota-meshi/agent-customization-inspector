...
fixture は、環境を build された home に向けることで使います — 4つの tool member のうち3つには
`COPILOT_HOME`、`CLAUDE_CONFIG_DIR`、`CODEX_HOME` を、そして `HOME` を。製品は共有 agent home
`~/.agents` と Antigravity CLI home `~/.gemini` の両方を `HOME` から自身で導出します。4つ目の
member は自身の設定を持ちません（specs/003-antigravity-cli-support/spec.md FR-008、FR-013、FR-045）。
いずれも `environment` が名前で keyed した形で返します。この製品の入力はそのプロパティだけであり、Global root を
引数として受け取ることはありません。
