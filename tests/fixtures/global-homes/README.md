...
`CLAUDE_CONFIG_DIR`, and `CODEX_HOME` for three of the four tool members, and `HOME`, from
which the product derives both the shared agent home `~/.agents` and the Antigravity CLI
home `~/.gemini` — the fourth member has no setting of its own
(specs/003-antigravity-cli-support/spec.md FR-008) — (FR-013, FR-045); all of them come
back from `environment` already keyed by name — because those properties are the product's only
