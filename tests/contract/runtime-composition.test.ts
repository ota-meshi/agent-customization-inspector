...
describe('normative relationship-only registry', () => {
  ...
  it('defines exactly fifteen relationship-only rule IDs', () => {
    // Eleven from the three vendor contracts that shipped first, three for
    // Antigravity CLI — its context import, its component references, and its
    // agent context — and the shared runtime edge, each a definition here and
    // never a shipped rule.
