// T077: what a declared value is and is not (data-model.md
// § DeclaredMetadataEntry, contracts/http-api.md § get-file-detail). One entry
// per field holding the value its parser resolved — which is what the file is
// named for, there being no occurrence index to name it after.
//
// The claim these tests defend is narrow and absolute: what the product reports
// for a field is the value a product loading that file would have — never a
...
function frontmatterOf(sourceText: string): Record<string, unknown> {
  return parseFrontmatter(sourceText) as Record<string, unknown>;
}
...
describe('declared values', () => {
  it('reports a literal credential whole, with no masking or reveal step', () => {
    ...
    expect(fields['description']).toBe(`token ${CONTENT_FIXTURE_SECRET}`);
  });
  ...
  it('leaves environment-reference syntax as the characters that were written', () => {
    ...
    expect(fields['description']).toBe('$HOME/${TOKEN}');
  });
  ...
  it('cuts no surrogate pair or combining sequence in half', () => {
    ...
    expect(fields['name']).toBe(ASTRAL_AND_COMBINING);
  });

  it('survives a JSON transport round trip unchanged', () => {
    const fields = frontmatterOf(`---\nname: "${ASTRAL_AND_COMBINING}"\n---\n`);
    const transported: { value?: string } = JSON.parse(JSON.stringify({ value: fields['name'] }));
