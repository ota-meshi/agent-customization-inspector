...
- 追加した原則:
  ...
  - IV. ドキュメントはプロダクトの一部
  - V. 誰もが参加しやすいプロジェクト
- 追加したセクション: 品質と安全性の基準、開発ワークフロー
- 追加した言語版: .specify/memory/constitution.ja.md
- 削除したセクション: なし。テンプレートのプレースホルダーを置換
- 更新したテンプレートとガイダンス:
...
- 入力と調査対象artifactは信頼できないものとして扱わなければならない（MUST）。実装は最小権限、
  resource上限、安全な失敗、secretを漏らさないlogと表示を使用しなければならない（MUST）。Trust
  boundary、権限、永続化、network、機密データを変更する場合は、securityとprivacyへの影響をレビュー
  しなければならない（MUST）。
