...
fileの一覧ではある唯一の項目`Files in no kind`である。Railの構成員はこの1つの試験から従う:
fileの一覧はrailの項目であり、Sourceの状態はrouteである。Source自身のdiagnosticがそこに無いのも
同じ理由による。それはfileを1つも並べず、唯一のSource単位のcodeはscanを失敗させてinventoryを
commitしないので、inventoryが出ている画面ではその項目は常に`0`だった。各Sourceは自分のものを
自分のsurfaceで述べる。Railの項目はiconを持たない — `Rule`や`Hook`の
...
- 単一coordinatorがcancellable `GlobalEnableOperation`のadmissionとその単一`GlobalBatchScan`、Repository scan、後続の
  ...
  1つのconsent recordは常にfixed closed-order tuple `[copilot, claude, codex, antigravity, agents]`をpreviewし、UI/APIのper-tool selectorを
  ...
  1つ所有する。Filesystem I/Oを伴わないrequest/`previewId` validation後も、initial enableはfrozen consentと5 controlすべてを
...
| ------------------------------------------------------------------- | ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tool-home設定を`undefined`としてcapture                             | `preview-default`                               | Session-startの1回の`node:os.homedir()` captureから、active-platform `node:path.join`とtool固定`.copilot`/`.claude`/`.codex` suffixを使ってfilesystem I/Oなしでexact stringを計算し、下記ordered rowでclassifyする。このtoolをfixed five-entry confirmationに保持してauthorityを作らない |
| Capture済みenvironment設定のlengthが0                               | `inputState: present-empty` / `preview-invalid` | Environment-origin valueだけに最初に適用する。Entryをfixed five-entry confirmationに保持し、fallbackもfilesystem/network I/Oも行わず、そのentry用root、Source、job、generationを作らない                                                                                               |
...
| それ以外（通常のhome外を含むabsolute string）                       | `inputState: eligible` / `preview-eligible`     | 保存するexact raw lexical valueをfilesystem/network I/Oなしでescapeしてserver保持preview recordに保持し、fixed five-entry confirmationに保持して1回のall-tools consent actionを待つ。このrowだけがconsent後admissionへ進める                                                           |
...
| Fixed-five Global admissionが全rootを決定的にreject                                                                                                                                                                                                     | `active-no-job`、Global coordinator                             | Active consent/controlを保持し、`scanRequestId`、batch、Source、generationを作らず、既存のcommit済み全IDを正確に維持する                                                                                                                                                                                                                               |
