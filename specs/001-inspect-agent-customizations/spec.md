...
- Q: How may a user select a Repository root other than the invocation working directory? → A: The CLI MUST accept `--cwd <path>`. When omitted, the selected Repository root is the exact string from the one `process.cwd()` capture. An absolute option is kept as given, and a relative option is resolved lexically against the captured invocation directory with the active platform's `node:path.resolve`. A repeated `--cwd` resolves to the argument parser's last value (deterministic last-wins) and is not separately rejected (superseded 2026-07-23: the "at most once, duplicate MUST error" rule was removed — repeated-argument policy belongs to the argument parser). A missing or empty value MUST produce a fixed actionable startup error and exit before creating a session or opening a browser. Root selection itself MUST perform zero filesystem/network I/O, and the rejected launch MUST perform no invocation-`cwd`-, `--cwd`-, or selected-root-derived I/O and no DNS, SMB, or outbound-network call. The selected root's readability is established only by the first scan, which fails with the source-scoped `root-unreadable` Diagnostic when the root does not exist or cannot be read as a directory (superseded 2026-07-22: the Windows pre-rejection list, the shared `LexicalAbsoluteRootParts` parser, and central source-boundary admission were removed).
...
3. **Given** `--cwd` lacks a non-empty value, **When** startup parses the command line, **Then** it emits a fixed actionable error and exits before session creation or browser opening.
