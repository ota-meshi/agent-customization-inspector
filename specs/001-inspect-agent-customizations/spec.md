...
|---|---|---|
| At least one NUL byte | `binary` | Diagnostic-only; expose no source text or comparison eligibility and make an otherwise publishable generation `partial` |
...
| Traversal is complete and one or more files have only file-confined outcomes (unreadable, binary, parse failure) while every unaffected file is complete | `partial` | Atomically commit one generation with affected-file diagnostics and complete unaffected results |
...
- **Source-relative Path**: A customization file's NFC display, filtering, lookup, and selection path relative to the one root of its owning Source. It is repository-relative to the selected Repository root only for the Repository Source; each Global Source uses its own root and namespace.
