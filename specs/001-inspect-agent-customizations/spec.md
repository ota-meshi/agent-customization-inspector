...
- Q: Does the child-process prohibition also forbid the product's browser-launch helper? → A: No. The only permitted product-initiated child process is the fixed operating-system browser-launch helper used at startup under FR-001, spawned through the maintained `open` package so cross-platform helper resolution stays that package's policy. It receives no inspection-derived content or path, authored value, or user-supplied command: the product passes exactly one argument, the closed loopback URL. The helper inherits the launch environment unchanged — the product writes no inspection-derived value into any environment variable, so nothing inspection-derived can reach the child through it, and an environment-filtering launcher would re-implement the maintained package's platform policy to protect nothing. A platform helper honoring the user's own environment configuration, such as `xdg-open` consulting `$BROWSER`, is applying user preference from the user's own shell, not an inspection-derived input. Inspection remains usable when automatic opening is disabled, unsupported, or fails. No customization-file discovery, read, parsing, display, comparison, or relationship processing may initiate a child process.
