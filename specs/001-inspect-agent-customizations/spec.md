...
- Q: What establishes the Repository source root? → A: The selected Repository root: the exact process working directory returned by the one `process.cwd()` capture when `--cwd` is omitted, or the one `--cwd` value — kept as given when absolute and otherwise resolved lexically against the capture with the active platform's `node:path.resolve`. Root selection itself performs zero filesystem/network I/O, and `package.json.bin` maps directly to the packaged CLI with no bootstrap read before it (superseded 2026-07-22: the Windows pre-rejection list, the shared `LexicalAbsoluteRootParts` parser, and the package-integrity bootstrap were removed with the admission-layer and defensive-check cleanups); a rejected selection performs no I/O derived from or directed at the invocation `cwd`, `--cwd`, or any Repository root and no DNS, SMB, or outbound-network call. The initial release has no separate repository picker or ancestor-root discovery.
...
- Q: Does generation 0 require filesystem admission before the Repository Source exists? → A: No. Session bootstrap MUST capture the invocation working directory from `process.cwd()`, select the Repository root from that string or a valid `--cwd` value using lexical path operations only, and create exactly one Repository Source synchronously with zero filesystem I/O. Its escaped root label and stable `sourceId` are non-authorizing session identity only; the first scan simply reads the retained raw selected root — there is no separate admission layer (superseded 2026-07-22: the central source-boundary admission module was removed with the trusted-workspace decision). A deterministic scan failure — including a root that does not exist or cannot be read as a directory — leaves that Source present with failed scan status, retains its source-scoped `root-unreadable` Diagnostic (FR-002), and publishes no customization files or generation. A thrown or rejected automatic-startup operation instead propagates to the process top level and may end the process or session.
- Q: How may a user select a Repository root other than the invocation working directory? → A: The CLI MUST accept `--cwd <path>`. When omitted, the selected Repository root is the exact string from the one `process.cwd()` capture. An absolute option is kept as given, and a relative option is resolved lexically against the captured invocation directory with the active platform's `node:path.resolve`. A repeated `--cwd` resolves to the argument parser's last value (deterministic last-wins) and is not separately rejected (superseded 2026-07-23: the "at most once, duplicate MUST error" rule was removed — repeated-argument policy belongs to the argument parser). An explicit empty value MUST produce a fixed actionable, source-value-free startup error and exit before creating a session or opening a browser. A missing value MUST be rejected at the same boundary by the argument parser's typed validation; the product MUST NOT duplicate that parser-owned check. Root selection itself MUST perform zero filesystem/network I/O, and the rejected launch MUST perform no invocation-`cwd`-, `--cwd`-, or selected-root-derived I/O and no DNS, SMB, or outbound-network call. The selected root's readability is established only by the first scan, which fails with the source-scoped `root-unreadable` Diagnostic when the root does not exist or cannot be read as a directory (superseded 2026-07-22: the Windows pre-rejection list, the shared `LexicalAbsoluteRootParts` parser, and central source-boundary admission were removed).
...
2. **Given** the user launches from another directory and supplies a relative or absolute `--cwd`, **When** the user starts an inspection, **Then** the selected root is shown as the one Repository Source and the process working directory is unchanged.
3. **Given** `--cwd` has an explicit empty value or is missing its value, **When** startup parses the command line, **Then** an explicit empty value emits the fixed actionable, source-value-free error while a missing value is rejected by the argument parser's typed validation, and either exits before session creation or browser opening.
...
- **FR-001**: Users MUST be able to launch the product through `npx` and open the resulting local inspection session in a browser. The CLI MUST capture `process.cwd()` exactly once and accept `--cwd <path>`; an absolute option is kept as given and a relative option is resolved against the captured invocation directory, and the result is the selected Repository root. When `--cwd` is supplied more than once, the argument parser's last value is the selected root (deterministic last-wins), which the product does not separately reject (superseded 2026-07-23: the earlier "accept at most once, duplicate MUST error" requirement was removed — repeated-argument policy belongs to the argument parser, and Gunshi resolves a repeated scalar option to its last value). An explicit empty value MUST produce a fixed actionable, source-value-free startup error before session creation or browser opening. A missing value MUST be rejected at the same boundary by the argument parser's typed validation; the product MUST NOT duplicate that parser-owned check. The CLI MUST NOT call `process.chdir()`. The initial release MUST NOT prompt for another repository path or search ancestor directories for a different root. The certification baseline is the finite, versioned set of exact browser builds against which the release candidate's automated browser and accessibility certification suites run, as recorded in the release evidence for that release candidate; it is a reproducible certification set rather than an exhaustive list of supported user browsers, and a pinned certified browser is one member of that set. The product MUST NOT select, probe, or verify the resolved handler's browser family or version, so automatic opening remains best-effort and a successful helper invocation is not compatibility evidence; whether a resolved browser is outside the certification baseline is determined from the recorded environment by the operator or study equipment, never by product-issued inspection. If automatic browser opening is disabled, unsupported, fails, is unavailable or unidentifiable, or resolves outside the certification baseline, the product MUST provide a usable printed local address for the pinned-certified-browser fallback.
