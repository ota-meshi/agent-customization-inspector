...
- Q: What names a skill inventory row? → A: The name each recognizing tool resolves, making the row's unit one name as one tool resolves it. Every tool resolves the authored frontmatter `name`; a Claude Code recognition of a nested skill prefixes it with the root-relative `/`-joined path of the directory holding its `.claude` and a `:`, so `apps/web/.claude/skills/deploy/SKILL.md` declaring `name: deploy` is `apps/web:deploy` on its Claude row. The last segment deliberately differs from the vendor's documented command name, which takes the skill directory name and treats the authored `name` as only a display label: comparing one skill across tools is what the row exists for, and the authored `name` is the one identity all three tools share, so the inventory keys every row by it and takes only the nested qualification shape from the vendor. One file whose recognizing tools resolve different names appears as a definition of each name's row. A file that declares no name — or declares it empty — is named by its skill directory, which is also where the vendors' own command names come from, so every row has a name and two such files in same-named directories share one.
...
- Q: What names the skill detail heading? → A: The row's own name — this product's provisional identity, the same one the inventory lists — with the addressed definition's documented invocation name beside it, computed by the projection and published as `definitions[].invocationName`. A definition is one tool's recognition, and each publishes its own tool's documented name: Claude Code's path-derived command name, or the authored name with the rows' directory fallback, which Codex and Copilot invoke. A detail URL is `/skills/<tool>/<source-relative path>` — the definition's own identity, tool first as the broader segment, each path segment percent-encoded — so which recognition of a file the page is about, its companions included, is the link's identity rather than a preference the page applies. That identity is stable across rescans and server launches — it is the URL's path half: the Source-relative Path is the file's identity on the wire — no per-generation file ID exists — and a detail request resolves it against the current committed snapshot (FR-030), so a bookmarked link's path keeps naming the same file across rescans and across launches that select the same root — the path is Source-relative, and a launch that selects another root (FR-001) resolves it against that root's scan. Whether the whole bookmark resolves is the origin's question: the origin is devframe's port selection, which reuses its fixed default port when free and moves to another only when that port is occupied (quickstart.md), so a moved port changes where a bookmark points, never which file its path names. A path the current scan does not hold for the URL's tool is reported as a dead link rather than resolved to a neighbour.

### Session 2026-08-14

- Q: Is the comparison surface shared across customization kinds? → A: No. A comparison surface is kind-specific. The shipped surface is the skill comparison — one skill name's copies compared file by corresponding file, entered from the name's inventory row or detail-page link and switched on the comparison itself; a file only one copy ships is part of that comparison, offered by the switcher and shown as its present side's complete content against its stated absence — and its route, page, composable, and components are skill-scoped: `/skills/compare`, `src/app/pages/skills/compare.vue`, `src/app/composables/skill-comparison.ts`, and `src/app/components/skill-comparison/`. Other kinds do not fit that model — an MCP comparison compares declarations inside carrier files rather than same-named file copies — so each family's comparison phase designs its own surface when it arrives, and no shared comparison route or module exists for it to extend; later-phase task entries that name the former shared comparison modules are superseded by this decision and are reshaped when their phase is planned. The URL names the comparison by the model's own coordinates — `/skills/compare?left=<entry path>&right=<entry path>&file=<copy-relative path>`, the two copies by their entry files' Source-relative Paths (FR-030) with `file` omitted for the entries themselves — so a pair the model does not express is reported rather than compared, and typed recognition rows are always the surface's own kind's.
...
- **FR-007**: For each readable recognized customization file, the inspector MUST show its source, source-relative path, file type, recognizing tools, complete source text, and known relationships. A file only the census lists is shown through its skill's detail with its own source, path, and read outcome — the complete source text when the read yielded text (FR-025); it has no file type, recognizing tool, or relationship to show, because nothing admitted it as a customization (FR-003) — a census-listed path a rule independently admits, a nested `SKILL.md`, keeps its own recognitions and rows. A carrier is the exception: a file whose inventory rows are the declarations inside it rather than the file — every MCP carrier, whether the declarations stand alone in a `.mcp.json` or sit beside unrelated settings in a `.codex/config.toml` — shows its complete source text nowhere. Codex's `.codex/config.toml` keeps that rule on every row it has, its own `settings/config` row included, because one file does not show its bytes on one screen and withhold them on another. What it declares is what it is inspected for, so a detail leads with one declaration and publishes it by the keys the file wrote, in the file's own order, with every value exactly as authored; showing the file itself would put every other declaration and setting it carries on a screen whose subject is one of them. The carrier still publishes its own file facts — source, path, encoding, size, and read outcome — because a file the inspector read is a file it lists (FR-003). A file that is itself the customization keeps the rule above: an instruction file, a skill, and a census-listed companion all show their complete source. A detail surface MUST lead with the customization rather than with the file that carries it: for a skill, its row name — this product's provisional identity, the same one the inventory lists — with the owning definition's documented invocation name beside it — each definition publishes its own tool's: Claude Code's command name derives from the path, the skill directory root-relative-prefixed when nested, whatever the frontmatter declares, while Codex and Copilot invoke the authored name with the same skill directory fallback the rows use; a definition whose extraction failed publishes none for a tool that invokes the authored name, because that name is unknown rather than absent (FR-028), while Claude Code's path-derived command still stands — and the page shows the definition its route addresses — a detail URL is `/skills/<tool>/<source-relative path>`, the definition's own identity, stable across rescans and server launches — so which tool's invocation name sits beside the heading is the link's identity rather than a preference the page applies, and a bookmarked link's path keeps naming the same file wherever a launch selects the same root, because the Source-relative Path is the file's one identity (FR-030) and a launch that selects another root (FR-001) resolves it against that root's scan — the origin being devframe's port selection, fixed-default unless occupied (quickstart.md), so only a moved port changes where the bookmark points. Then every key its frontmatter declares, presented as one YAML document through the read-only viewer — led by its `name` and `description`, every other key in the file's own order; the block's own language, so a reader compares it against their file and copies from it without converting — then the instructions that frontmatter block was removed from, with the files of its directory reachable beside them, each openable on its own. The complete source text MUST be served with them, and the inspector MUST NOT republish a value beside the source it was read from once that surface already shows it, because one screen would then carry one fact in two spellings. Every presented value MUST be shown as the statement it names rather than as the maintained identifier that keys it: a rule ID, a behavior or strategy ID, a matcher lookup base, and a closed status value are contract tokens that resolve a registry record or a wire vocabulary, and they MUST NOT appear as user-visible text. For presentation, a supported customization file type is the exact `(tool, kind)` recognition together with its admitted source form. Maintained supported-customization documentation MUST enumerate the closed `(tool, kind)` presentation allowlist and the supported source forms to which each row applies. A row MUST NOT enumerate declared fields: a skill's declarations are published by the keys the file wrote, and an authored key set is not closed. A relationship is eligible only when both its kind is named by that row and the admitted source form's exact extractor recognizes the authored occurrence; listing a relationship kind for one source form MUST NOT promote or infer it in another. The initial release MUST NOT infer any relationship the allowlist does not name. An inventory row MUST be named by the name each recognizing tool resolves for the skill, because a list that cannot name what it lists is not an inventory: every tool's name is the authored frontmatter `name` — or the skill directory name when the file declares none or declares it empty, because being a named directory is what a skill is, so every row has a name; a file whose extraction failed keeps the same directory-derived provisional identity, the directory being the path's own fact rather than something a failed parse could be read for (FR-028) — and a Claude Code recognition of a nested skill prefixes it with the root-relative `/`-joined path of the directory holding its `.claude` and a `:`, so `apps/web/.claude/skills/deploy/SKILL.md` declaring `name: deploy` is `apps/web:deploy` on its Claude Code row. The last segment deliberately differs from the vendor's documented command name, which takes the skill directory name and treats the authored `name` as only a display label (skills page § How a skill gets its command name): comparing one skill's definitions across tools is what the row exists for, and the authored `name` is the one identity all three tools share, so the inventory keys every row by it and takes only the nested qualification shape from the vendor. The inspector MUST prefix every nested Claude Code recognition, with the prefix relative to the selected root: the vendor qualifies on a name clash with a prefix relative to a session working directory, but the inspector observes no session, and whether an unqualified name is free depends on user- and enterprise-level layers it never reads, so the root-relative qualified spelling is the one stable name a static inventory can stand behind. An inventory row's unit MUST be the one the recognized kind uses rather than the physical file: a skill is one name as one tool resolves it, an MCP row is one declared server name listing every declaration resolving it — one per `(carrier, tool)`, a second carrier declaring the same name joining that name's row — an instructions row is one applicability range listing the files that govern it, plus the one row of files whose range is not known — a detail stays addressed by the file — and a configuration file is the file. Several files resolving to one name MUST publish one row listing each as a definition; a definition is one tool's recognition of one file — the ToolRecognition unit, one per `(file, tool)` — so a file two tools resolve to one name is two definitions of that row, a file whose tools resolve different names defines on each name's row, and one carrier declaring several servers MUST publish one row per declared name. Two files that declare no name and sit in same-named skill directories resolve to one name and MUST share its row, like any other files resolving to one name. A row that groups several definitions MUST state how each tool that recognizes two or more of them resolves that name, and MUST NOT order them, because the recorded statements differ by tool and none is completely documented. A definition whose extraction failed is not one of that count for a tool that invokes the authored name: the name it resolves is unknown, so its row membership is provisional grouping rather than collision evidence (FR-028). A tool that recognizes only one of them faces no collision there and MUST NOT be quoted, because its rule would answer a question the row is not asking it. The collision MUST also be one the quoted rule answers: Claude Code's documented rule answers the clash of unqualified commands, which come from skill directories, so its statement attaches to every row holding a Claude definition whose skill directory name is shared with another Claude-recognized skill of the same generation — two files sharing only the authored name under differently named directories are two commands with no clash, and no rule is quoted for them. The row's name — a resolved identity that is authored only when the file declared it — is the one name value a row carries; every other declared value and the complete authored source MUST be reachable only through the FR-027 detail and comparison surfaces, never through an inventory or session response. A path segment MUST NOT override an authored name: the skill directory names a row only when the file declares no name or an empty one, and the root-relative prefix qualifies whichever of the two the row has.
...
| Claude Code | `CLAUDE.md`, `.claude/CLAUDE.md`, `CLAUDE.local.md`, and nested instruction files; `.claude/rules`; skills; legacy commands; subagents; project and local settings; declared hooks; root MCP configuration; output styles; marketplace catalogs; plugin manifests | An authored `@path` import is source text and no target is opened; `AGENTS.md` is not recognized by filename alone; unreferenced scripts are not inferred to be hooks; a standalone `.claude/prompts` directory, managed settings, managed instructions, and unrelated user state are outside the Repository source |
| OpenAI Codex | `AGENTS.md` and `AGENTS.override.md`; `.agents/skills`; custom agent definitions; project configuration; hook declarations; MCP declarations; rules; plugin and marketplace metadata | An authored reference in an instruction file is source text and no target is opened; what depends on project trust or a working directory stays recorded in the vendor's maintained contract; deprecated user custom prompts and user-level skills are outside the Repository source |
