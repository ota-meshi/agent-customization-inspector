...
| A FileHandle `close` event confirms closure before that handle's retained close promise rejects | Only already-confirmed close lifecycle is retained | Apply the FR-041 event-confirmed-close observation: do not classify the cause, poison the registry, propagate the rejection, or publish any result from the observation; continue only from the already-confirmed successful closure |
| Any non-carveout proposed-root operation throws or rejects | Only work completed before propagation may have touched its proposed root | Apply FR-041 and abort the whole Global transaction; publish none of its provisional subset and retain the prior snapshot |
| One or more consented roots pass lexical, canonical, link, type, containment, and identity checks and no non-carveout operation throws or rejects | No access outside each proposed root | Admit those roots into one provisional Global transaction; publish every resulting Source together in one atomic generation, never as observable per-tool commits |
...
| A FileHandle `close` event confirms closure before that handle's retained close promise rejects | Continue from the already-confirmed successful closure | Apply only the FR-041 event-confirmed-close observation; do not classify the cause, poison the registry, propagate the rejection, or create an item, Diagnostic, result, response, or generation from the observation |
| A non-carveout operation owned by an accepted REST-triggered job throws or rejects before commit | `failed` for that `scanRequestId` | Let the error propagate to the REST job boundary; abort the attempt, revoke publication authority, commit no failed-attempt item, Source, recognition or derived result, scan-result body, success response, or generation, and expose only a generic path/content-free error representation. Retain any prior committed snapshot. If and only if this is an explicit rescan, create or replace that Source's stale-failure overlay and reference only the accepted job's Operation Error; an initial scan or initial/retry Global batch creates no stale overlay |
| A non-carveout REST request operation throws or rejects before asynchronous job acceptance | HTTP error; no `scanRequestId` | Let the error propagate to the REST request boundary; return only a generic path/content-free error representation and create no job, item, Source, result, or generation |
| A non-carveout automatic startup operation with no REST owner throws or rejects | No public terminal status is guaranteed | Let the error propagate to the process top level, commit nothing from the attempt, and make no product guarantee that the process or session survives |
...
- **FR-017**: The Codex Global source MUST inspect only the documented instruction fallback at the captured `CODEX_HOME`, or at `node:path.join(capturedHomedir, '.codex')` when that setting is absent: `AGENTS.override.md` first, falling back to `AGENTS.md` only under the closed FR-035 Global-branch conditions—exact `ENOENT` from the contract-declared initial target `lstat`, or a safely verified and read non-binary override whose decoded string is empty as defined by FR-035. As FR-035 specifies, a deterministic unsafe or binary override outcome MUST end the branch with its Diagnostic and no fallback, the FR-041 event-confirmed-close observation MUST leave the already confirmed close successful without selecting fallback, and any non-carveout throw or rejection MUST propagate under FR-041 without fallback.
...
- **FR-041**: Except for Node's exact `ENOENT` code from an `lstat` call at a contract-declared structural existence checkpoint and the event-confirmed-close observation declared below, a thrown exception or rejected operation from inspected-source reading MUST propagate without a catch in the filesystem, parser, recognition, or scan domain layers; those layers MUST NOT classify its cause, retry it, recover an item, or convert it into `complete` or `contracted-partial` data. The narrow `ENOENT` checkpoint MAY return only the specified `absent` or `entry-disappeared` structural outcome; it MUST NOT inspect the error message, apply to another error code or operation (including `open` or `read`), or authorize a read. One additional narrow closure observation—the event-confirmed-close observation—is likewise contract-declared: after the central resource registry has confirmed a FileHandle's closure through its `close` event, the registry MAY observe a later rejection of that same handle's one retained close promise and record the already-confirmed closure as successful lifecycle state; this observation MUST NOT inspect or classify the rejection's cause, MUST NOT recover, publish, or commit any item or result from it, and MUST NOT apply to an unconfirmed close, an `fs.Dir` close, or any other operation, whose throws and rejections remain governed by this requirement unchanged. The outer boundary that owns any other throw/rejection MAY catch only to express execution lifecycle. Before REST job acceptance, it MUST return a generic path/content-free HTTP error and create no `scanRequestId`; after acceptance, the job boundary MUST expose a generic path/content-free terminal error for that `scanRequestId`. Either REST path MUST keep the process and session available, commit no failed-attempt result body or generation, and retain any prior committed snapshot. An automatic startup operation with no REST owner MUST allow the error to reach the process top level; the product MUST NOT promise that the process or session survives. Product REST representations, operational logs, and telemetry MUST NOT expose the raw error or copy runtime-owned uncaught-error output. The product does not control whether the Node.js runtime or host writes its own uncaught-error diagnostic locally, and documentation MUST identify that residual local-disclosure and process-liveness limitation.
...
- Public Node.js filesystem promises may not be force-cancellable. Disable, shutdown, supersession, or any non-carveout thrown or rejected operation revokes publication authority and discards late results; a deterministic, non-throwing entry-local outcome eligible for FR-028 and the event-confirmed-close observation do not by themselves revoke the attempt's publication authority. The product does not promise physical cancellation of kernel I/O, process survival after an uncaught startup-owned error, or control over runtime-owned local uncaught-error output.
