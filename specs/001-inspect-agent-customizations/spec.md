...
- Q: What does it mean for Repository results to remain unchanged when Global enablement commits? → A: The Repository Source keeps its stable `sourceId`, and its customization-file membership, source-relative paths, readable or diagnostic states, recognitions, relationships, Repository-scoped diagnostics, and authored source text remain semantically unchanged because that operation does not rescan the Repository. Every successful initial or retry Global Source commit advances only the Global sequence's generation, rekeys only that sequence's generation-owned graph IDs, and invalidates only its previous-generation `FileDetail`, comparison selection or view, and editor-model state; the Repository sequence's IDs and views are neither rekeyed nor invalidated (FR-030). Those Global-sequence identity and user-interface lifecycle changes are not semantic changes to Repository results. An all-rejected attempt produces no Global Source commit and is outside this rule.
- Q: What location context does each Diagnostic carry? → A: Every retained Diagnostic has exactly one scope. A file-scoped Diagnostic requires a coherent `sourceId`, `fileId`, and `sourceRelativePath` tuple in which the file belongs to that Source and the path is that file's path within the Source; a source-scoped Diagnostic requires only `sourceId` and forbids `fileId` and `sourceRelativePath`. There is no pathless scope: every Diagnostic the product produces belongs to a Source. Scope describes location independently of whether the Diagnostic belongs to a committed generation or the session lifecycle.
...
- Q: Do Repository and Global inspection share one generation sequence? → A: No. Their lifecycles are independent — the Repository Source always exists, while Global sources exist only between enable and disable — so each keeps its own atomic generation sequence: the Repository sequence from bootstrap generation 0, and a Global sequence created by the enable commit and discarded by disable (which commits nothing). A commit rekeys and invalidates only its own sequence's IDs and views. The former carry-forward rules — a Global commit preserving the Repository inventory unchanged while rekeying every session ID — are removed. Cross-source comparison is unaffected: it always compares each source's last committed state.
...
- **FR-007**: For each readable recognized customization file, the inspector MUST show its source, source-relative path, file type, recognizing tools, complete source text, and known relationships. A census-listed companion is shown through its skill's detail with its own source, path, and complete source text; it has no file type, recognizing tool, or relationship to show, because nothing admitted it as a customization (FR-003). A detail surface MUST lead with the customization rather than with the file that carries it: for a skill, its declared name, then every key its frontmatter declares by the key the file wrote — led by its name and description — then the instructions that frontmatter block was removed from, with the files of its directory reachable beside them, each openable on its own. The complete source text MUST be served with them, and the inspector MUST NOT republish a value beside the source it was read from once that surface already shows it, because one screen would then carry one fact in two spellings. Every presented value MUST be shown as the statement it names rather than as the maintained identifier that keys it: a rule ID, a behavior or strategy ID, a matcher lookup base, and a closed status value are contract tokens that resolve a registry record or a wire vocabulary, and they MUST NOT appear as user-visible text. For presentation, a supported customization file type is the exact `(tool, kind)` recognition together with its admitted source form. Maintained supported-customization documentation MUST enumerate the closed `(tool, kind)` presentation allowlist and the supported source forms to which each row applies. A row MUST NOT enumerate declared fields: a skill's declarations are published by the keys the file wrote, and an authored key set is not closed. A relationship is eligible only when both its kind is named by that row and the admitted source form's exact extractor recognizes the authored occurrence; listing a relationship kind for one source form MUST NOT promote or infer it in another. The initial release MUST NOT infer any relationship the allowlist does not name. An inventory row MUST show the customization's own declared name when its recognizer extracted one, because that name is presentation identity rather than content: it is the name the products' own skill listings show — for a Claude Code repository skill it is only that display label, and the command comes from the directory — it is not recoverable from the Source-relative Path, since a skill's authored `name` need not match the directory holding it, and a list that cannot name what it lists is not an inventory. An inventory row's unit MUST be the one the recognized kind uses rather than the physical file: a skill is one declared name, a contained MCP server is one declaration inside its carrier, and an instructions or configuration file is the file. Several files declaring one skill name MUST publish one row listing each as a definition, and one carrier declaring several servers MUST publish one row per server. A row that groups several definitions MUST state how each tool that recognizes two or more of them resolves that name, and MUST NOT order them, because the recorded statements differ by tool and two of the three are incomplete. A tool that recognizes only one of them faces no collision there and MUST NOT be quoted, because its rule would answer a question the row is not asking it. The collision MUST also be one the quoted rule answers: for Claude Code the command name comes from the skill directory, so its statement appears only when two of its definitions share a directory name — two files sharing only the display label are two commands with no clash, and no rule is quoted for them. It is the one declared value a row carries; every other declared value and the complete authored source MUST be reachable only through the FR-027 detail and comparison surfaces, never through an inventory or session response. A row MUST NOT substitute a path segment for a name the file does not declare.
...
- **Diagnostic**: A session-only, actionable explanation of an empty result, deterministic malformed or binary content outcome, uncertainty, stale file, cycle, or boundary violation that does not duplicate customization source values. It has exactly one location scope. A file-scoped Diagnostic MUST require a coherent `sourceId`, `fileId`, and `sourceRelativePath` tuple: the file MUST belong to the identified Source, and the path MUST be that file's path within that Source. A source-scoped Diagnostic MUST require only `sourceId` and MUST forbid `fileId` and `sourceRelativePath`. There is no pathless scope: every Diagnostic MUST belong to a Source. This location scope is independent of whether the Diagnostic belongs to a committed generation or to the session lifecycle. An unexpected failure that is not confined to one file surfaces as an ordinary error and does not become a Diagnostic.
