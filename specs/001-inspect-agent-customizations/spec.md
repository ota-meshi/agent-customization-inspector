...
| OpenAI Codex | `AGENTS.md` and `AGENTS.override.md`; `.agents/skills`; custom agent definitions; project configuration; hook declarations; MCP declarations; rules; plugin and marketplace metadata | What depends on project trust or a working directory stays recorded in the vendor's maintained contract; deprecated user custom prompts and user-level skills are outside the Repository source |
