...
- **FR-003**: The inspector MUST discover repository customization files only at paths in a documented inspection path allowlist and MUST NOT indiscriminately interpret every file in the repository. The allowlist is anchored at the selected Repository root: the product reports that root's customizations, not the set a particular runtime working directory would produce. Where a vendor's own lookup depends on its working directory, that dependency is recorded as a condition of the recognition and MUST NOT widen the allowlist to other directories. A customization the vendors define as a directory rather than a file — a skill and its scripts, references, and assets — is read whole: the files accompanying an admitted candidate inside that candidate's own directory MUST be readable and displayable, because showing the entry point while withholding what ships with it would not show the customization. That is the only extension of the read set, it is bounded by the admitted candidate's own directory (contracts/inspection-path-allowlist.md § Bounded companion census), and an accompanying file MUST NOT thereby become a candidate: it acquires no rule, recognition, kind, or inventory row of its own.
...
- **Source**: An explicit source identity with a kind (`Repository` or `Global`), exactly one root location, enabled state, scan status, and zero or more Source Condition Facts. The Repository Source exists at session bootstrap with a stable `sourceId` and an escaped root label. A Global Source is created only by its atomic post-consent commit and is additionally identified by exactly one supported tool; customization files of different types within that root remain separate inventory items.
...
- The inspector may perform syntax-only parsing, reading the value a parser resolves for an allowlisted field, frozen-catalog classification, and projection of documented structural order, scope, condition, selection, and reference relationships. It does not interpret natural-language meaning, determine correctness, effectiveness, compliance, or quality, rank content, or advise remediation; a parse diagnostic is a descriptive failure, not a validation result.
