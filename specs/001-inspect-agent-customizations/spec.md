...
- **FR-009**: The inspector MUST NOT present a customization as one a product loaded, selected, enabled, or trusted. Discovery depends on runtime version, product surface, working directory, trust, flags, environment, organization policy, and undocumented conflict resolution, none of which the inspector observes; it therefore states what it found and where, and says nothing about that runtime — neither a verdict nor a per-file account of the inputs it did not read. The one statement it may carry about a product's own rules is what a vendor documents for a name several files contend for, on the inventory rows that show those files (FR-007).
