...
remediationごとにcomplete applicable automated gate matrix、影響するcandidate・profile・fixtureのevidence protocol、
...
Source-relative Pathはplanが保持したsegment — enumerated pathでは保存されたentry name、targeted fixed pathではimmutable registryの綴り — を`/`でjoinしたものであり、filesystem operationが使うのと同じ綴りである。
...
客観的pass ruleに対し、criterion固有のstable check IDと、このreleaseが実行するautomated・keyboardのevidenceを組み合わせて評価する。
`MANUAL-*`行はmanual checkが必要とするclosed matrix — packed candidate、3つのsupported
OS/browser/assistive-technology cell、responsive/visual profile、workflow state、input profile — を名指すが、
未実行のresidualとして記録し、どのpassing rowにも数えない。それらの環境を得たreleaseは、各行が名指すmatrixの下でそれらを実行する。
Axeのseverity結果だけを
...
5. Filesystem operationを行うのはplanが保持したsegment — enumerated pathでは保存されたentry name、
   targeted fixed pathではimmutable registryの綴り — だけとし、それを`/`でjoinしたものがpublicな
