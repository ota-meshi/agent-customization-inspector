...
- [x] US4 requires every successful initial or retry Global Source commit to preserve `Source.sourceId` and semantic Repository inventory/source content while advancing the generation and invalidating prior-generation detail, comparison, and editor state; all-rejected attempts create no commit *(amended 2026-08-08: the rekeying clause left with the decision that a file's identity is its Source-relative Path — a commit has no generation-owned file IDs to rekey.)*
- [x] The closed Diagnostic union defines exact location invariants: file scope requires a coherent `sourceId`/`sourceRelativePath` pair, source scope requires only `sourceId` and forbids the path, no pathless scope exists, and scope is orthogonal to generation-versus-lifecycle ownership *(amended 2026-08-04: the pathless session scope left with the decision that an unadmitted Global tool's failure is its control's `failureCode`.)* *(amended 2026-08-08: the file ID left the location shape with the decision that a file's identity is its Source-relative Path.)*
...
- [x] US3 is independently testable with two distinct readable Repository files before Global work and rejects the same file in both inputs, while US4 separately covers comparing two consented homes' files of one row inside the Global Source family — a comparison never pairs across families, so no Repository file is offered as a side of that pair, and Source-relative namespaces are never merged (§ Clarifications Session 2026-08-28)
...
- [x] One selector-free session-wide Global action binds the fixed five-member preview — the Copilot, Claude, Codex, and Antigravity homes and the shared agent home — evaluates all five entries, excludes a missing or unreadable root without blocking the others, and publishes every admitted Source in one batch and one atomic generation; an unexpected failure aborts the whole transaction *(amended 2026-08-30: restated for FR-045.)* *(amended 2026-09-11: five members, for the fourth supported tool.)*
...
- Validation iteration 11 passed all items on 2026-07-19 after defining US4's Repository
  ...
  the generation and invalidate prior-generation state *(amended 2026-08-30: restated for the
  decision that a file's identity is its Source and Source-relative Path.)*; defining exclusive
...
- Validation iteration 17 passed all items on 2026-07-20 after closing selected-root and
  ...
  selector-free fixed-member Global batching *(amended 2026-08-30: restated for FR-045.)*, exact
  structural-`lstat` `ENOENT` handling, REST versus
