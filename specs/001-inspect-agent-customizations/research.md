...
applicable automated gate matrix, every affected candidate, profile, and fixture
...
selection, or read-once semantics. A public Source-relative Path is the segments the plan
retained joined with `/` — an enumerated path's stored entry names, a targeted fixed path's
immutable registry spelling — the same spelling filesystem operations use.
...
criterion-specific stable check IDs with the automated and keyboard evidence this release
runs. Its `MANUAL-*` rows name the closed matrix a manual check would need — the packed
candidate, three supported OS/browser/assistive-technology cells, responsive/visual
profiles, workflow states, and input profiles — and are recorded as unexecuted residuals
that count toward no passing row; a release that gains those environments executes them
under the matrix each row names. An axe severity result alone is
...
4. An admitted candidate's NUL is binary/diagnostic-only/`partial`; a companion's is the
   ...
   by itself.
5. The segments the plan retained alone perform filesystem operations — an enumerated
   path's stored entry names, a targeted fixed path's immutable registry spelling — and
   joined with `/` they are the published classification and display path — a filesystem holds one entry per name,
