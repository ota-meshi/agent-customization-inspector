...
| `copilot.cloud.mcp.selection` | GitHub Copilot / Cloud agent | `merge-map`、`replace` | Out-of-box server、custom-agent server、Repository settingsの順で解決し、後のsourceが前のsame-name serverをoverrideする | `copilot.behavior.cloud.mcp` | `surface`、`engine-version`、`scope-availability`、`trust`、`approval`、`enablement`、`selection`、`settings-inputs`、`agent-context`、`external-runtime` | documented。Inputはlocal `.mcp.json`ではなくhosted | `github.copilot.custom-agents` |
