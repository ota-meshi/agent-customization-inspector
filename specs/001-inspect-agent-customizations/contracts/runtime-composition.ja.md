...
| `claude.settings.precedence` | Claude Code / shared CLI、VS Code、JetBrains core | `replace`、`merge-map`、`concatenate`、`deduplicate` | Managed、CLI、exact-launch-`cwd` local、exact-launch-`cwd` project、User settings順で解決し、documented scalar、array、object、exceptional-key merge modeを使う | `claude.behavior.repo.settings`、`claude.behavior.user.settings` | `surface`、`engine-version`、`runtime-cwd`、`scope-availability`、`trust`、`approval`、`selection`、`settings-inputs`、`managed-policy` | documented | `anthropic.claude-code.large-codebases.start-directory`、`anthropic.claude-code.sdk.setting-sources`、`anthropic.claude-code.settings.scopes-precedence` |
