...
| `codex.repo.settings` | Repository | `['.codex', 'config.toml']` | `codex.repo.config`がauthorするselectorの上でのRepository rootでの`exact`。3つのruleが1つのfileをadmitし、walkはそれらを1回のreadで1つのcandidateへmergeする。vendor contractがそのcarrierに`MCP`・内包`hook`・`settings/config`の別々のrecognitionを与えており、recognitionはruleが生むものだからである | `static-candidate` | `codex.behavior.repo.config` | Documented。Trust/runtime chainはconditional | `openai.codex.config-basic` |
...
| `codex.repo.hooks.inline` | Repository | `['.codex', 'config.toml']` | `codex.repo.config`が著すselectorと同じ、Repository rootでの`exact`。carrierのinline `[hooks]` tableはその1 fileの`hook` recognitionであり、この3つのruleは1 candidateを1度読むことになる | `static-candidate` | `codex.behavior.repo.config`、`codex.behavior.repo.hooks` | Documented。Trust/hook reviewはconditional | `openai.codex.hooks` |
...
それぞれはその1 fileのrecognitionであり — recognitionはruleが生み出すものだから、各自のruleのものである — 、
したがってinline `[hooks]` tableの`hook` rowと同じlayerのstandalone `hooks.json`の`hook` rowは、共有する
eventについて別個の宣言として残る。それがvendorが両方をloadすることの意味である（`codex.hooks.additive`）。
