...
| `claude.repo.output-style` | Repository | `[ANY_DIRECTORIES, '.claude', 'output-styles', /\.md$/u]` | `descendant-inventory` plus `direct-child`; style file is a direct child of each fixed output-styles directory | `static-candidate` | `claude.behavior.repo.output-style` | Eligibility requires an ancestor layer of the active session and selection by settings/session state | `anthropic.claude-code.output-styles.locations` |
