...
|---|---|---|---|---|---|
| `copilot.behavior.vscode.user.instructions` | User homeまたはVS Code profile | `~/.copilot/copilot-instructions.md`、`~/.copilot/instructions/**/*.instructions.md`、`~/.claude/rules/**/*.md`、profile instruction file | 他のinstruction sourceと加算的。`chat.instructionsFilesLocations`でlocationをenable/disable可能 | Consent済み`<COPILOT_HOME>` subsetだけをadmit — `copilot-instructions.md`は`copilot.global.instructions.root`、`instructions/**/*.instructions.md`は`copilot.global.instructions.path`。VS Codeへのapplicabilityには、そのboundaryがdocumented user locationを表すことも必要 | `vscode.copilot.instructions`, `vscode.copilot.settings` |
