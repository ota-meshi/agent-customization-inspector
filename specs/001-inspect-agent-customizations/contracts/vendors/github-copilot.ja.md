...
| `copilot.behavior.vscode.prompts` | Workspace root | `.github/prompts/*.prompt.md` | Explicit/manual invocation。Local agentでは、additional locationはそのagentと共にdeprecatedである`chat.promptFilesLocations`から取得 | —。Explicit prompt invocation | Documented。既定nested-directory behaviorは厳密には明記されない | `vscode.copilot.prompts`, `vscode.copilot.settings` |
| `copilot.behavior.vscode.hooks` | Workspace root | `.github/hooks/*.json`、`.claude/settings.json`、`.claude/settings.local.json`、agent-scoped hook declaration | Claude形式のfileはdefault offのsettingが必要。Agent-scoped hookはuser、workspace、plugin hookに加えて実行される。Parent discoveryはopt-in | `copilot.vscode.hooks.composition` | Documented。Preview featureはactivation-conditional | `vscode.copilot.hooks`, `vscode.copilot.custom-agents`, `vscode.copilot.customization` |
...
VS Codeはapplicableなinstruction sourceを加算的に扱い、それらの間のorderやprecedenceを文書化しない。競合は
いずれかに頼るのではなくsource側で解消するよう勧めている。applicableな全sourceはmodelへ渡される。
...
|---|---|---|---|---|---|
| `copilot.behavior.vscode.user.instructions` | User homeまたはVS Code profile | `~/.copilot/copilot-instructions.md`、`~/.copilot/instructions/**/*.instructions.md`、`~/.claude/rules/**/*.md`、profile instruction file | 他のinstruction sourceと加算的。Local agentでは、`chat.instructionsFilesLocations` — deprecatedで、そのagentだけが読む — がそのlocationをenable/disableし、既定値には`~/.copilot/instructions`と`~/.claude/rules`が含まれる。Agent Host sessionのlocationを切り替える設定はpageに無く、`~/.copilot/copilot-instructions.md`もそれに含まれる | Consent済み`<COPILOT_HOME>` subsetだけをadmit — `copilot-instructions.md`は`copilot.global.instructions.root`、`instructions/**/*.instructions.md`は`copilot.global.instructions.path`。VS Codeへのapplicabilityには、そのboundaryがdocumented user locationを表すことも必要 | `vscode.copilot.instructions`, `vscode.copilot.settings` |
...
| `copilot.behavior.vscode.user.hooks` | User home | `~/.copilot/hooks/*.json`、`~/.claude/settings.json` | Workspace、agent、plugin hookとともに参加し、それらの間のorderは文書化されていない。Claude形式のfileは`chat.useClaudeHooks`が必要 | Consent済み`<COPILOT_HOME>/hooks`のsubsetだけが`copilot.global.hooks`でadmitされる。`~/.claude/settings.json`のcompatibility readは`copilot.excluded.user-runtime`のまま | `vscode.copilot.hooks` |
...
7. **Documentationは急速に変化する。** Official Sources contractのcanonical page、列挙済みsection名、
   2026-07-20までのrecord別review日をmaintenance baselineとし、search-result snippetをevidenceにしない。
