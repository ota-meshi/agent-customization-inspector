...
| `copilot.behavior.vscode.user.hooks` | `documented` | `[preview]` | The same preview feature, seen from the User scope |
...
| `copilot.behavior.vscode.prompts` | Workspace root | `.github/prompts/*.prompt.md` | Explicit/manual invocation; for the Local agent, additional locations come from `chat.promptFilesLocations`, which is deprecated with that agent | —; explicit prompt invocation | Documented; default nested-directory behavior is not stated precisely | `vscode.copilot.prompts`, `vscode.copilot.settings` |
| `copilot.behavior.vscode.hooks` | Workspace root | `.github/hooks/*.json`; `.claude/settings.json`; `.claude/settings.local.json`; agent-scoped hook declarations | The Claude-format files require an off-by-default setting; agent-scoped hooks run in addition to user, workspace, and plugin hooks; parent discovery is opt-in | `copilot.vscode.hooks.composition` | Documented; preview features remain activation-conditional | `vscode.copilot.hooks`, `vscode.copilot.custom-agents`, `vscode.copilot.customization` |
...
VS Code treats applicable instruction sources as additive and documents no order or
precedence among them, advising that conflicts be resolved at their source rather than by
relying on one; all applicable sources are still provided to the model.

## VS Code User behavior

These are documented local User surfaces. Except for the Global rules defined
...
|---|---|---|---|---|---|
| `copilot.behavior.vscode.user.instructions` | User home or VS Code profile | `~/.copilot/copilot-instructions.md`; `~/.copilot/instructions/**/*.instructions.md`; `~/.claude/rules/**/*.md`; profile instruction files | Additive with the other instruction sources. For the Local agent, `chat.instructionsFilesLocations` — deprecated, and read by that agent alone — enables or disables its locations, `~/.copilot/instructions` and `~/.claude/rules` among its defaults; the page names no setting that switches an Agent Host session's locations, `~/.copilot/copilot-instructions.md` included | Only the consented `<COPILOT_HOME>` subset is admitted — `copilot-instructions.md` by `copilot.global.instructions.root` and `instructions/**/*.instructions.md` by `copilot.global.instructions.path`; VS Code applicability additionally requires that boundary to represent its documented user location | `vscode.copilot.instructions`, `vscode.copilot.settings` |
...
| `copilot.behavior.vscode.user.skills` | User home or VS Code profile | `~/.copilot/skills/<name>/SKILL.md`; `~/.agents/skills/<name>/SKILL.md`; `~/.claude/skills/<name>/SKILL.md`; configured locations | Available across workspaces; duplicate-name precedence is not documented | Only the consented `<COPILOT_HOME>/skills` and shared-agent-home subsets are admitted, by `copilot.global.skill` and `copilot.global.agents-home.skill`; the `~/.claude/skills` and configured locations remain `copilot.excluded.user-runtime` | `vscode.copilot.skills`, `vscode.copilot.settings` |
| `copilot.behavior.vscode.user.agents` | User home or VS Code profile | `~/.copilot/agents/*.md`; profile agent files | Available across workspaces; duplicate-name precedence against workspace, organization, and plugin agents is not documented | Only the consented `<COPILOT_HOME>/agents` subset is admitted, by `copilot.global.agent`; profile agent files remain `copilot.excluded.user-runtime` | `vscode.copilot.custom-agents`, `vscode.copilot.settings` |
...
| `copilot.behavior.vscode.user.hooks` | User home | `~/.copilot/hooks/*.json`; `~/.claude/settings.json` | Participates with workspace, agent, and plugin hooks, with no documented order between them; the Claude-format file requires `chat.useClaudeHooks` | Only the consented `<COPILOT_HOME>/hooks` subset is admitted, by `copilot.global.hooks`; the `~/.claude/settings.json` compatibility read remains `copilot.excluded.user-runtime` | `vscode.copilot.hooks` |
...
| `copilot.behavior.cli.user.skills` | User home | `~/.copilot/skills/<name>/SKILL.md`; `~/.agents/skills/<name>/SKILL.md` | Below project/inherited skills and above later sources in the documented first-found order | Accepted by `copilot.global.skill` below `COPILOT_HOME` and by `copilot.global.agents-home.skill` below the shared agent home (FR-045) | `github.copilot.cli.reference`, `github.copilot.skills` |
| `copilot.behavior.cli.user.agents` | User home | `~/.copilot/agents/*.agent.md` | Project-versus-user precedence is an unresolved official-documentation conflict; plugin agents are lowest | Accepted by `copilot.global.agent` below | `github.copilot.cli.reference`, `github.copilot.cli.custom-agents`, `github.copilot.cli.configuration`, `github.copilot.cli.plugins` |
| `copilot.behavior.cli.user.hooks` | `COPILOT_HOME` | `hooks/*.json`; inline hooks in `settings.json` | Policy, user, project, and plugin hooks are composed; all applicable same-event hooks execute | Accepted by `copilot.global.hooks` and `copilot.global.hooks.inline` below | `github.copilot.hooks`, `github.copilot.cli.configuration` |
| `copilot.behavior.cli.user.mcp` | `COPILOT_HOME` | `mcp-config.json` | Below session additional config, plugin, and workspace sources | Accepted by `copilot.global.mcp` below | `github.copilot.cli.reference` |
| `copilot.behavior.cli.user.settings` | `COPILOT_HOME` | `settings.json` | User layer in the documented settings cascade | Accepted by `copilot.global.settings` and `copilot.global.hooks.inline` below | `github.copilot.cli.configuration` |
...
| `copilot.repo.hooks` | Repository | `['.github', 'hooks', /\.json$/u]` | `direct-child` | `static-candidate` | `copilot.behavior.vscode.hooks`, `copilot.behavior.cli.hooks`, `copilot.behavior.cloud.hooks` | Documented root hook files, which every surface reads; an inline block in a settings document is the two settings hook rules' | `vscode.copilot.hooks`, `github.copilot.hooks` |
| `copilot.repo.hooks.settings` | Repository | `['.github', 'copilot', 'settings.json']`; `['.github', 'copilot', 'settings.local.json']` | `exact` for each selector | `static-candidate` | `copilot.behavior.cli.hooks` | The inline `hooks` block of the CLI's own settings pair. Its own rule rather than a second selector of the rule below, because a rule's surfaces are the behaviors it rests on and the editor's hook-locations table names the Claude-format pair alone | `github.copilot.hooks`, `github.copilot.cli.configuration` |
| `copilot.repo.hooks.settings.claude` | Repository | `['.claude', 'settings.json']`; `['.claude', 'settings.local.json']` | `exact` for each selector | `static-candidate` | `copilot.behavior.cli.hooks`, `copilot.behavior.vscode.hooks` | The inline `hooks` block of the cross-tool pair both surfaces read. The same physical files carry Claude Code's own hook recognition: one file, one read, one recognition per product | `vscode.copilot.hooks`, `github.copilot.hooks` |
...
| `copilot.global.skill` | The same exact consented `<COPILOT_HOME>` boundary | `['skills', ANY_NAME, 'SKILL.md']` | `direct-child` then `exact`; the skill name is exactly one direct child | `static-candidate` | `copilot.behavior.cli.user.skills`, `copilot.behavior.vscode.user.skills` | `copilot.cli.skills.selection`, `copilot.vscode.skills.selection` | FR-013, FR-014, FR-015, FR-018, QR-005 | `github.copilot.cli.reference`, `github.copilot.skills`, `github.copilot.cli.configuration` |
| `copilot.global.agent` | The same exact consented `<COPILOT_HOME>` boundary | `['agents', /\.agent\.md$/u]` | `direct-child` of the boundary's `agents/`; the pages document the `.agent.md` filename and no nested search | `static-candidate` | `copilot.behavior.cli.user.agents`, `copilot.behavior.vscode.user.agents` | `copilot.cli.agents.selection`, `copilot.vscode.agents.selection` | FR-013, FR-014, FR-015, FR-018, QR-005 | `github.copilot.cli.custom-agents`, `github.copilot.cli.configuration`, `github.copilot.cli.reference` |
| `copilot.global.hooks` | The same exact consented `<COPILOT_HOME>` boundary | `['hooks', /\.json$/u]` | `direct-child` of the boundary's `hooks/` | `static-candidate` | `copilot.behavior.cli.user.hooks`, `copilot.behavior.vscode.user.hooks` | `copilot.cli.hooks.composition`, `copilot.vscode.hooks.composition` | FR-013, FR-014, FR-015, FR-018, QR-005 | `github.copilot.hooks`, `github.copilot.cli.configuration` |
| `copilot.global.hooks.inline` | The same exact consented `<COPILOT_HOME>` boundary | `['settings.json']` | `exact`, over the selector `copilot.global.settings` authors; the inline `hooks` field is a `hook` recognition of that one file, so the two rules over the one path are one candidate read once | `static-candidate` | `copilot.behavior.cli.user.hooks` | `copilot.cli.hooks.composition` | FR-013, FR-014, FR-015, FR-018, QR-005 | `github.copilot.hooks`, `github.copilot.cli.configuration` |
| `copilot.global.settings` | The same exact consented `<COPILOT_HOME>` boundary | `['settings.json']` | `exact`; the user layer of the documented settings cascade, authored as JSONC | `static-candidate` | `copilot.behavior.cli.user.settings` | `copilot.cli.settings.precedence` | FR-013, FR-014, FR-015, FR-018, QR-005 | `github.copilot.cli.configuration` |
| `copilot.global.mcp` | The same exact consented `<COPILOT_HOME>` boundary | `['mcp-config.json']` | `exact`; the user-level MCP server definitions | `static-candidate` | `copilot.behavior.cli.user.mcp` | `copilot.cli.mcp.selection` | FR-013, FR-014, FR-015, FR-018, QR-005 | `github.copilot.cli.reference`, `github.copilot.cli.configuration` |
| `copilot.global.agents-home.skill` | The consented shared agent home: `node:path.join` of the session-start imported `node:os.homedir()` capture and `.agents`, which no documented setting relocates (FR-045) | `['skills', ANY_NAME, 'SKILL.md']` | `direct-child` then `exact` | `static-candidate` | `copilot.behavior.cli.user.skills`, `copilot.behavior.vscode.user.skills` | `copilot.cli.skills.selection`, `copilot.vscode.skills.selection` | FR-013, FR-014, FR-018, FR-045, QR-005 | `github.copilot.cli.reference`, `github.copilot.skills` |
...
fails the attempt as an ordinary error. The vendor's automatically managed files — application
state, saved permissions, session and command-history state, the session store, logs,
installed plugins, plugin data, IDE state, and MCP OAuth and secret storage — plus LSP
configuration, extensions, and credentials remain excluded even when stored below the same
boundary.
...
|---|---|---|
| `instructions` | — | An accepted `*.instructions.md`, repository `.github/copilot-instructions.md` or consented `<COPILOT_HOME>/copilot-instructions.md`, `AGENTS.md`, or Copilot-recognized `CLAUDE.md`; an authored CLI `@path` target is source text, never an extracted reference, and a supported frontmatter value such as `applyTo` declares the range the file governs rather than a target. Path-derived scope and enablement remain typed facts |
...
| `copilot.excluded.user-runtime` | `copilot.behavior.vscode.user.claude`, `copilot.behavior.vscode.user.skills`, `copilot.behavior.vscode.user.agents`, `copilot.behavior.vscode.user.prompts`, `copilot.behavior.vscode.user.hooks`, `copilot.behavior.vscode.user.mcp`, `copilot.behavior.vscode.user.settings`, `copilot.behavior.vscode.user.plugins`, `copilot.behavior.cli.user.plugins`, `copilot.behavior.cli.user.lsp`, `copilot.behavior.cli.user.extensions` | The user surfaces no Global rule admits: another tool's home and VS Code profile files (`~/.claude/*`, profile prompts, profile MCP and settings), configured extra locations, installed plugins and plugin data, user LSP configuration, user extensions, permissions state, logs, and caches | Documented User behavior remains visible in the tables above; a cross-home or profile read stays a recorded behavior with no Inspector recognition, and the vendor's automatically managed files are state rather than authored customization | FR-013, FR-014, FR-015, FR-018, QR-001, QR-005 | `vscode.copilot.instructions`, `vscode.copilot.skills`, `vscode.copilot.settings`, `vscode.copilot.custom-agents`, `vscode.copilot.prompts`, `vscode.copilot.hooks`, `vscode.copilot.mcp`, `vscode.settings`, `vscode.copilot.plugins`, `github.copilot.plugins`, `github.copilot.cli.reference`, `github.copilot.skills`, `github.copilot.cli.custom-agents`, `github.copilot.cli.configuration`, `github.copilot.cli.plugins`, `github.copilot.hooks`, `github.copilot.cli.lsp`, `github.copilot.cli.extensions` |
...
7. **Documentation is fast-moving.** Canonical pages, enumerated section names, and the
   per-record review dates through 2026-07-20 in the Official Sources contract are the maintenance
   baseline; search-result snippets are not evidence.
