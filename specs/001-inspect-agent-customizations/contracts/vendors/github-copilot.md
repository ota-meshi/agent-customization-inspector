...
| `copilot.behavior.vscode.user.hooks` | User home | `~/.copilot/hooks/*.json`; `~/.claude/settings.json` | User hooks are below workspace hooks for the same event; agent/plugin hooks may also execute | Only the consented `<COPILOT_HOME>/hooks` subset is admitted, by `copilot.global.hooks`; the `~/.claude/settings.json` compatibility read remains `copilot.excluded.user-runtime` | `vscode.copilot.hooks` |
