...
| **Descendant inventory** | An Inspector-only downward inventory below the retained Repository boundary. It inventories possible authored runtime contexts; it is not a Copilot runtime locator or proof of loading. |
...
because
CLI documents additional standard locations. Because `ANY_DIRECTORIES` can match zero
segments, the
...
| `copilot.repo.mcp` | Repository | `[ANY_DIRECTORIES, '.mcp.json']`; `[ANY_DIRECTORIES, '.github', 'mcp.json']` | `descendant-inventory` | `static-candidate` | `copilot.behavior.cli.mcp` | Documented CLI candidate inventory; runtime ancestor chain and trust conditional | `github.copilot.cli.reference` |
...
the value its parser resolved for that field; for server, environment, header, Hook-event,
metadata, and named-setting `*.name` IDs, that value is the authored map key. `marketplace.plugin.source` is the
