...
- Every checklist item has one primary outcome and at least one exact repository-relative owned file path; a basename-only shorthand is not an exact path.
...
matrix and its Japanese counterpart in the same change.
...
| FR-009 | T079–T080, T091, T913–T920, T1042, T1053 |
...
- [ ] T008 [P] Configure distinct unit, contract, integration, security, package, performance, and coverage projects in `vitest.config.ts`; make the dedicated security project include exactly `tests/security/**/*.test.ts`, exclude that root from every other project, and leave `tests/integration/security/` owned by the integration project
- [ ] T009 [P] Configure deterministic Chromium, Firefox, and WebKit primary-workflow and accessibility certification projects using the exact browser revisions installed by Playwright 1.61.1, while documenting that these pinned revisions are the reproducible automated baseline rather than an exhaustive user-browser list, in `playwright.config.ts`
...
- [ ] T011 [P] Create the BOM-free executable Node.js integrity-bootstrap skeleton with the exact shebang, built-in-only verification of the packed exact `engines.node` string `^24.11.0 || ^26.0.0` and the running version against `>=24.11.0 <25.0.0 || >=26.0.0 <27.0.0`, no static `dist/cli.mjs` import, and exactly one post-verification dynamic-import placeholder in `bin.mjs`
...
**Independent Test**: Verify closed DTOs and source-value-free diagnostics, exact package manifests, capability classification, the central Node.js filesystem authority, and generation-zero state without launching a product workflow.
...
- [ ] T015 [P] Add failing deterministic tests for parser-message, retained-graph, lifecycle-Diagnostic, control, and complete-envelope construction; prove that, except for Node's exact `ENOENT` from a contract-declared structural-`lstat` checkpoint, every thrown exception or rejected Node.js, decoder, parser, Worker, OS, filesystem, browser, assembly, serialization, or execution-environment operation propagates unchanged past filesystem/parser/recognition/scan domain layers without cause classification, retry, entry recovery, Diagnostic, result body, or generation; that a REST-owning outer boundary alone represents it as the closed generic `OperationError`, with null `scanRequestId` and no job before acceptance or the accepted job's non-null ID after `202`; that an ownerless automatic-startup rejection reaches the process top level; and that only the last committed snapshot remains in `tests/unit/shared/runtime-failures.test.ts`
- [ ] T016 [P] Add failing tests for the closed Diagnostic registry, deterministic aggregation, successful complete atomic publication, and the complete closed `OperationError { operationErrorId, code: "operation-failed", messageKey: "api.operationFailed", nextStepKey: "api.retryOrRestart", operationId, scanRequestId }` entity, rejecting any cause taxonomy, `safeArgs`, source/file/path/root/filename identity, inspected content/metadata, authored value, capability, body, parser/system/raw exception field, runtime argument, or descriptor; separately test the closed `OperationalEvent` schema and reject every extra/free-form field while allowing only fixed event codes and optional opaque session/source/file/scan/operation IDs in `tests/unit/shared/diagnostics.test.ts`, `tests/unit/shared/operation-errors.test.ts`, and `tests/unit/shared/operational-events.test.ts`
- [ ] T017 [P] Add failing tests for public entity shapes—including readable `utf-8 | utf-8-bom | utf-8-replaced` files with complete decoded text/ranges and preserved `U+FFFD`, diagnostic-only NUL-containing `binary`, one-root Source invariants, exact non-authorizing `SourceBoundary { displayRoot, origin }` with `process-cwd | cwd-option | default-home | environment`, and generation-0 origin selection. Add the closed `DocumentationStatus = documented | partially-documented | unknown | conflict`, fixed `LifecycleQualifier = preview | experimental | deprecated` ordering with duplicate rejection and no inferred `stable`, and exact record-specific `EvidenceAssessment { subjectKind, subjectId, documentationStatus, lifecycleQualifiers }`; require one sorted/deduplicated assessment for each owning/referenced behavior, rule, and strategy on provenance, Relationship, and Source Condition Fact records, with no scalar/worst-status/qualifier-union reduction and no use of `documentation-conflict` as a documentation status. Also cover closed descriptors, pathless session-scoped normalization-collision Diagnostics, ordinary Diagnostic scopes, the complete closed `OperationError`, opaque IDs, strict versioned envelopes, and rejection of internal authority, acknowledgement, validation, cause, or raw-error fields in `tests/unit/shared/entities.test.ts` and `tests/unit/shared/api.test.ts`
- [ ] T018 Create deterministic cross-platform fixtures for links, junctions, non-regular entries, deep trees, VCS internals, every row-1–28 mutation checkpoint, create/remove/rename during explicit directory enumeration, hard links with usable/unusable/changing bigint `dev`/`ino`/`nlink`, Codex ordered-fallback and late-derived aliases, effective and ineffective `O_NOFOLLOW`, and `platform-unobservable` outcomes. Use an external mutation harness while product instrumentation proves exactly one production content read per consumed group and zero mutation-capable product APIs/flags; record before/after content/length/identity/link/mode/mtime/ctime/xattr/ACL and OS-only atime separately in `tests/fixtures/adversarial/build-filesystem-fixtures.ts`
- [ ] T019 Add failing root, selector, enumeration, and resource-lifecycle tests for the exact ordered structural-`lstat` rows 1–28. Before row 1, directly test the one shared pure `LexicalAbsoluteRootParts` parser reused by CLI selection, Global preview classification, and central boundary admission: require zero filesystem/network calls and no checkpoint or authority on rejection; cover POSIX `/`, accepted components, repeated/trailing separators, dot/dot-dot, U+FFFD, NUL, and unpaired-surrogate vectors plus Windows accepted plain-drive roots/components and rejected UNC/network/device/current-drive, drive-relative, malformed-drive, repeated/trailing-separator, dot/dot-dot, NUL, and unpaired-surrogate vectors. Then prove row 20 and every row-2 shared-prefix component are revalidated per Global selector; each row-2 component immediately completes rows 4–7 before the next operand; rows 21–24 surround every directory open and rows 25–28 follow complete sibling collection before buffer use. Require registered `fs.Dir`, explicit `Dir.read()` until null, ephemeral dev/ino/mode/mtimeNs/ctimeNs directory guards, confirmed close before sibling-buffer use, raw `Dirent.name` operands, pathless session-scoped NFC-collision rejection before member I/O, exact structural-`lstat` `ENOENT` conversion only, and unchanged propagation of every other throw/rejection in `tests/unit/inspection/node-safe-fs.test.ts`
- [ ] T020 Add failing ticket/read and hard-link tests for private generation/`scanRequestId` binding, one-time use, rows 8–19 for every ticket around one primary-handle read, read-only/non-create/non-truncate open, same-handle identity, byte disposal, and zero mutation calls. Within exactly one Source scan attempt, complete static discovery/admission/group formation before static reads except Codex's content-dependent branch; accept a group only with exact bigint identity, `ino !== 0n`, `nlink > 0n`, stable equal `nlink`, and admitted count no greater than `nlink`; read each usable group once, while Sources, attempts, and generations remain independent. Require zero-read `safe-fs-ordered-fallback-alias-rejected` and `safe-fs-late-derived-alias-rejected` outcomes with no forbidden byte/provenance reuse in `tests/unit/inspection/node-safe-fs.test.ts`
- [ ] T021 Add failing boundary and process-wide `ClosableResourceRegistry` tests covering preallocated `opening` reservations, exact FileHandle/`fs.Dir` ownership, shared one-call close promises, FileHandle close-event confirmation, synchronous close throw, promise fulfillment/rejection before or after confirmation, `close-confirmed` versus `close-unknown`, late-event poison clearing, permanent directory-unknown restart, owner propagation, disable-lineage transfer, and no double close. Prove FileHandle event confirmation wins: a later raw close-promise rejection is observed but treated as successful with no poison or owner propagation, while rejection without confirmation remains unknown and propagates to the owner. Also prove every detectable file/directory replacement discards bytes/buffers, resource cleanup and late-result discard never claim hard cancellation, external mutation is not a product mutation, and `platform-unobservable` never counts as proof in `tests/integration/boundaries/node-safe-fs.test.ts` and `tests/unit/inspection/closable-resource-registry.test.ts`
...
- [ ] T023 [P] Add failing capability tests for 256-bit authentication, constant-time comparison, Host/Origin/fetch-metadata checks, no CORS, strict methods and media, no-store responses, environment-determined request-read failures with no partial processing, and errors that expose no capability or authored source value in `tests/contract/host-security.test.ts`
- [ ] T024 [P] Add failing build, packed-tarball, and runtime-bootstrap tests for root confinement, exact manifest schemas/order/declared byte lengths/hashes and closed file sets, complete hashing before import/bind, required-but-removed `200.html`/`404.html`, `index.html` as the only accepted HTML, rejection of `<base>`, nonces, executable attributes, external/relative executable URLs, unrecorded inline scripts, stale assets, and safe startup failure when the execution environment cannot completely read or verify an artifact in `tests/package/build-cleanup.test.ts`, `tests/package/static-manifest.test.ts`, and `tests/package/server-manifest.test.ts`
- [ ] T025 Add failing server-manifest and package-policy tests for exact `.mjs` records, required CLI/Worker entries, the recursively exact two-manifest set, production-graph digests and exact runtime leaves, exact `gunshi` 0.37.0 integrity and full bundled payload digest, root-API-only CLI imports with no `gunshi/agent`/lazy/custom-plugin path, and rejection of `open`, Rust/C/C++/Cargo, Node-API/native/binary/Wasm payloads, prebuilds, platform selectors, package shell helpers, non-Node shebangs, and lifecycle/runtime downloads in `tests/package/server-manifest.test.ts`, `tests/package/production-graph.test.ts`, and `tests/package/node-only-policy.test.ts`
- [ ] T026 [P] Add failing generation and session tests proving bootstrap generation 0 synchronously contains exactly one enabled idle Repository Source selected lexically from the captured invocation working directory and optional `--cwd`, with stable opaque `sourceId`, escaped non-authorizing `SourceBoundary`, empty files/Diagnostics, null `scanRequestId`, and zero filesystem I/O; automatic first scan starts from that Source. Cover one opaque request ID across every admitted automatic/explicit Source/progress/attempt/final status and successful generation; deterministic graph IDs; coordinator-locked serialization; atomic N+1 replacement; ID rekeying; single-buffer encoding; overflow rejection before mutation; last-commit retention; explicit-rescan stale state; late-result discard; generic closed pre-acceptance and accepted-job `OperationError` lifecycle; and an ownerless automatic-startup rejection that is not caught or converted and reaches the process top level in `tests/unit/session/scan-generation.test.ts` and `tests/unit/session/session.test.ts`

### Implementation

- [ ] T027 Implement the closed failure-observability contract without normalizing or inspecting thrown/rejected causes in filesystem, decoder, parser, Worker, recognition, or scan layers: except for the exact structural-`lstat` `ENOENT` carveout, let every throw/rejection propagate unchanged with no catch, retry, cause classification, recovered item, Diagnostic, scan result, response body, or generation. At the REST-owning outer boundary only, create the generic path/content-free closed `OperationError` with null `scanRequestId` before acceptance and no job/retention, or retain the matching non-null ID after `202`; let an ownerless automatic-startup rejection reach the process top level. Keep `OperationError`, closed Diagnostics, and fixed-code/opaque-ID `OperationalEvent` records disjoint; define no Inspector capacity ceiling; preserve the prior snapshot and revoke/discard failed or late work in `shared/runtime-failures.ts`, `shared/diagnostics.ts`, `shared/operation-errors.ts`, `shared/operational-events.ts`, and `src/host/runtime-failures.ts`
- [ ] T028 Implement public DTOs for readable `utf-8 | utf-8-bom | utf-8-replaced`, diagnostic-only `binary`, one-root Sources, generation 0, exact `SourceBoundary`, descriptors, Source Condition Facts, Diagnostics, and the complete closed `OperationError`. Implement `DocumentationStatus`, fixed-order duplicate-free `LifecycleQualifier[]`, and record-specific `EvidenceAssessment[]`; put scalar status/qualifiers only on each behavior, rule, or strategy, and preserve one sorted assessment per referenced subject on provenance, Relationship, recognition, and Fact DTOs without reduction. Enforce pathless session scope for normalization collisions and reject internal authority, acknowledgement, validation, cause, raw-error, scalar aggregate-status, or fabricated `stable` fields in `shared/entities.ts` and `shared/api.ts`
- [ ] T029 Implement private `InspectionRootContext`, `DirectoryEnumerationGuard`, and the single process-wide `ClosableResourceRegistry` in `src/inspection/safe-fs.ts`: preallocate `opening` before `open`/`opendir`, attach the exact resource synchronously, retain shared close promises/strong references, drive FileHandle close-event confirmation, distinguish `close-confirmed` from `close-unknown`, poison new filesystem work until confirmed, permit late FileHandle confirmation to clear poison, and require restart for an unconfirmed directory close. Treat a confirmed FileHandle close event as authoritative even if the raw close promise later rejects—observe that rejection but complete successfully without poison or propagation—while rejection without event confirmation remains unknown and propagates to the owner. Catch only exact Node `ENOENT` from a contract-declared structural `lstat`; propagate every other owner error unchanged, with a REST owner producing `OperationError` and an ownerless startup error reaching top level
- [ ] T030 Define and export the single side-effect-free shared pure `LexicalAbsoluteRootParts` parser from `src/inspection/safe-fs.ts`, with the exact closed POSIX/Windows grammar and parsed operands from the data-model contract, before implementing any row-1 operation. Make CLI root selection, Global preview classification, and central boundary admission call that same parser rather than reimplement it; parser rejection creates no checkpoint or authority and performs zero filesystem/network I/O. Define the closed immutable versioned `TraversalPlan` and structural checkpoint catalog, then implement rows 1–28 exactly in `src/inspection/safe-fs.ts`: per-selector row 20/shared-prefix row 2, immediate rows 4–7 for each selected component, rows 21–24 before every `opendir`, explicit registered `Dir.read()` until null, and rows 25–28 plus dev/ino/mode/mtimeNs/ctimeNs comparison and confirmed close before sibling-buffer use. Preserve raw operands, pathless session-scoped NFC-collision rejection, VCS exclusion, exact `ENOENT` outcomes, and generation-bound tickets. Complete all static admission/group formation before reads except Codex ordered fallback, and form hard-link groups only within one Source scan attempt from usable stable bigint dev/ino/nlink with admitted count no greater than nlink; Sources/attempts/generations do not share reads
- [ ] T031 Implement deterministic group consumption with every ticket's rows 8–19 around exactly one primary-handle read; same-handle identity, read-only flags, effective `O_NOFOLLOW`, byte disposal, exact structural-`lstat` disappearance, and unchanged propagation of every other throw/rejection. Implement zero-open/read Codex ordered-fallback alias rejection and late-derived alias rejection without byte/provenance reuse. Route all FileHandle/`fs.Dir` cleanup through `ClosableResourceRegistry`, transfer revoked pending resources to the disable lineage, invoke/join close once, discard late results, propagate unconfirmed close through its owner, and never claim hard kernel cancellation in `src/inspection/safe-fs.ts`
- [ ] T032 Implement only deterministic success-return `safe-fs-boundary-unverifiable` and related unusable-verification-data handling in `src/inspection/safe-fs.ts`: reject a root/shared ancestor at pathless session scope with `sourceId`, `fileId`, and `sourceRelativePath` all forbidden, and reject an admitted candidate at file scope only with its coherent primary `sourceId`/`fileId`/`sourceRelativePath`; never convert a throw/rejection into this Diagnostic, and let non-carveout exceptions propagate unchanged
- [ ] T033 Document that active source-root/ancestor replacement is outside the initial threat model everywhere, active final-component replacement is outside only without effective `O_NOFOLLOW`, every detectable change remains in scope and fails closed, and `platform-unobservable` outcomes are non-proving, plus the future public Node.js API or OS-enforced resolution path in `src/inspection/safe-fs.ts`
...
- [ ] T035 Replace only the cleanup/static placeholders: confine cleanup to generated roots in `scripts/clean-build-output.mjs`, then normalize/copy Nuxt `.output/public` into `dist/public` in `scripts/build-static-manifest.mjs` with normalized paths, the exact static-manifest schema and closed safe-file set, declared-versus-actual byte lengths, deterministic hashes and CSP hashes, accepted HTML/URL cases, and only the required fallback removals; fail the build if complete environment-supported reading or verification cannot finish
- [ ] T036 Replace only the server/package placeholders: generate the exact server `.mjs` manifest in `scripts/assemble-server-manifest.mjs`, digest the production dependency graph in `scripts/build-production-graph.mjs`, and recursively verify the exact two-manifest/package file set plus Node.js-only policy in `scripts/verify-package-files.mjs`, retaining the fixed Node ESM CLI and Worker entries scaffolded by T003
- [ ] T037 Implement deterministic generation construction whose generation 0 synchronously contains the exact enabled idle non-authorizing Repository Source selected from captured invocation `cwd`/`--cwd`, with stable source ID, empty files/Diagnostics, null request ID, and no I/O; reject any other generation-0 shape. Preserve one request ID across admitted status and successful generation, coordinator-locked serialization, atomic N+1 replacement, overflow rejection before mutation, explicit-rescan stale retention, accepted-job `OperationError` retention, startup rejection propagation to the process top level, and cleanup-only late-result discard after authority revocation in `src/session/scan-generation.ts`, `src/session/stale-failures.ts`, and `src/session/session.ts`
- [ ] T038 Implement the strict router skeleton with method/media/request-key/no-store guards, coordinator-locked complete payload capture, canonical production JSON before lock release, exact `Content-Length`, and unchanged-buffer delivery; catch a thrown/rejected REST-owned operation only at this outer boundary and return the generic closed `OperationError` with null `scanRequestId`, HTTP `500`, no created job/ID, result body, or generation before acceptance, or expose the retained accepted-job error through the session snapshot after `202`; never expose raw cause or convert a delivery failure into partial, and preserve an already committed snapshot in `src/host/api-router.ts`
...
**Independent Test**: Install the package, launch from a fixture invocation `cwd` with and without optional `--cwd`, open the printed loopback URL, authenticate from the one-time fragment, and verify generation 0 synchronously contains exactly one enabled idle Repository Source selected lexically from the captured invocation `cwd`/`--cwd`, with a stable opaque `sourceId`, escaped non-authorizing boundary, empty files/Diagnostics, null `scanRequestId`, and zero filesystem I/O.

**Visible Checkpoint**: The authorized browser screen starts and displays almost no product content.

### Tests first

- [ ] T040 [P] [US1] Add failing static-route and startup contracts for fixed manifest assets, exact declared-versus-actual lengths and hashes, closed SPA fallbacks, exact CSP, supported Node engines, complete package/two-manifest/every-asset verification before any CLI evaluation or bind, exactly one subsequent dynamic `import('./dist/cli.mjs')`, loopback-only binding, zero startup documentation/network access, and package checks that never classify customization content; prove any ownerless automatic-startup throw/rejection reaches the process top level without a product liveness guarantee or `OperationError`, Diagnostic, scan result, or raw-error operational record in `tests/contract/static-routes.test.ts` and `tests/contract/host-startup.test.ts`
- [ ] T041 [P] [US1] Add failing client tests for one-time fragment capture, memory-only Bearer use, authorization-lost reload behavior, and exact request-token/`clientDataEpoch`/generation/file-ID/`globalContentEpoch` guards. Require every inspection-data success to carry its captured epoch and be rejected unless the final response gate sees that epoch unchanged with a null `globalDisableInProgress`; separately require every liveness success to be a same-lock projection of the current epoch and current fence, accept a non-null fence as control data, and on a greater epoch or non-null fence perform the same full client-data purge used before a Global-disable request before rendering only control recovery. Also reject late responses and cover older/equal/newer generation behavior, zero persistence, and zero unauthorized calls in `tests/unit/app/api-capability.test.ts`
- [ ] T042 [P] [US1] Add failing browser-state tests for authorized polling, the exact liveness DTO `{ sessionId, globalContentEpoch, globalDisableInProgress }`, deadlines and full purges, client epoch increments, and late-response rejection. Cover generation 0 plus both recovery forms: ordinary fresh full-snapshot baseline adoption when the fence is null, and exact `GlobalFenceRecoverySnapshot` only while the fence is non-null; no full inspection DTO may render until terminal disable success or restart. Also cover authorization loss, matching-baseline Resume, and timer/request teardown in `tests/unit/app/session-shell.test.ts` and `tests/unit/app/liveness.test.ts`
- [ ] T043 [P] [US1] Add failing Gunshi CLI and packaged launch tests for the root `define`/`cli` API, positive default-true `open`/generated `--no-open`, and optional `--cwd <path>` accepted at most once: capture `process.cwd()` exactly once and retain that exact invocation string when omitted. On Windows, before `resolve`, reject every explicit two-leading-separator UNC/server-share/device spelling, single-separator current-drive/root-relative value, and drive-relative value including `C:` and `C:foo`; resolve only a plain relative option against the captured anchored drive-form value and retain an absolute drive option unchanged. On POSIX, retain an absolute option or resolve a relative option against the capture. Require every selected absolute result to pass the one shared pure `LexicalAbsoluteRootParts` parser reused by Global preview and central admission; perform zero filesystem/network I/O, never call `process.chdir()` or use per-drive working-directory semantics, and reject missing/empty/duplicate/pre-resolution-invalid/parser-rejected values with fixed actionable source-value-free output before session creation or browser opening. Cover POSIX accepted root/components, repeated/trailing separators, dot/dot-dot, U+FFFD, NUL, and unpaired surrogates plus Windows accepted plain-drive roots/components and rejected UNC/network/device/current-drive/root-relative, `C:`/`C:foo` drive-relative, malformed-drive, repeated/trailing-separator, dot/dot-dot, NUL, and unpaired-surrogate vectors. Also cover non-binding help/version, strict unknown/positional/rest rejection, awaited completion, exact package fields, isolated installation, the closed loopback URL, the fixed OS browser helper and environment allowlists, no inspection-derived helper input, manual fallback, clean shutdown, root-only imports, and zero inspected-source reads before admission in `tests/unit/cli.test.ts` and `tests/package/npx-launch.test.ts`
- [ ] T044 [US1] Add failing browser acceptance for the authorized generation-0 shell showing exactly one enabled idle Repository Source and its escaped non-authorizing selected-root label with empty files/Diagnostics and null `scanRequestId`, plus authorization loss, complete liveness purge, fresh-baseline control-only recovery, matching-baseline Resume, keyboard focus, and no Repository picker or ancestor discovery in `tests/e2e/boot.spec.ts`
...
- [ ] T046 [US1] Implement the verified loopback host-bootstrap boundary: use built-ins only for supported-engine and complete exact-set/schema/length/hash verification before CLI evaluation or bind, then exactly one dynamic `import('./dist/cli.mjs')`; let every ownerless startup throw/rejection, including automatic inspected-source work, reach the process top level without constructing `OperationError`, Diagnostic, or a liveness guarantee; implement loopback-only binding, capability-protected exact control-only liveness `{ sessionId, globalContentEpoch, globalDisableInProgress }`, a path/content-free fixed-code/opaque-ID `OperationalEvent` sink kept separate from CLI presentation, authenticated Diagnostics, and REST `OperationError`, and zero startup documentation/network access in `bin.mjs`, `src/host/api-router.ts`, `src/host/server.ts`, and `src/host/operational-events.ts`
- [ ] T047 [US1] Implement the Gunshi root `define`/`cli` entry with positive default-true `open`, generated `--no-open`, and optional `--cwd <path>` accepted once: capture `process.cwd()` once before validation and use its exact string when omitted. On Windows reject explicit two-leading-separator UNC/server-share/device, single-separator current-drive/root-relative, and `C:`/`C:foo` drive-relative input before `resolve`; resolve only plain relative input against the anchored capture and preserve an absolute drive input. On POSIX preserve absolute input or resolve relative input against the capture. Require the selected absolute result to pass the single shared pure `LexicalAbsoluteRootParts` parser from T030 that Global preview and central admission also call; perform zero filesystem/network I/O, never call `process.chdir()` or use per-drive working-directory semantics, and reject missing/empty/duplicate/pre-resolution-invalid/parser-rejected input before session/browser creation with fixed actionable output and nonzero exit. Retain strict unknown/positional/rest rejection, awaited completion, non-binding help/version, root-only imports, the one-time closed loopback URL, fixed OS browser helper, exact ambient allowlist and inspection-derived-input exclusions, usable printed-URL fallback, and graceful shutdown in `src/cli.ts` and `src/launch-browser.ts`
- [ ] T048 [US1] Implement one-time capability-fragment capture, memory-only authorization, exact request tokens, abortable requests, and `clientDataEpoch`/generation/file-ID/`globalContentEpoch` adoption guards. Before rendering inspection-data success, require its captured epoch to remain current and `globalDisableInProgress` to be null. Treat liveness separately as a same-lock current-epoch/current-fence projection that may carry a non-null fence; on its greater epoch or non-null fence, invoke the shared full client-data purge before adopting only the liveness/control recovery projection, reject every stale inspection-data response, and permit only fence recovery until terminal success in `app/composables/api.ts`
- [ ] T049 [US1] Implement one shared synchronous full client-data purge for session/liveness/document loss, pre-send Global disable, and observation of a greater `globalContentEpoch` or non-null fence. Clear every session/inventory/Source/file/Diagnostic/relationship/comparison/DOM/editor/filter/warning/acknowledgement value, abort requests, increment `clientDataEpoch`, and retain only the memory capability. When no fence exists, adopt a fresh authenticated full snapshot without comparing purged IDs and require matching-baseline Resume; while fenced, accept only `GlobalFenceRecoverySnapshot` and exact liveness control/error state. Add exact teardown and semantically equivalent messages in `app/composables/session.ts`, `app/composables/liveness.ts`, `app/app.vue`, `app/locales/en.ts`, `app/locales/ja.ts`, and `app/styles/main.css`
...
- [ ] T050 [US1] Create Codex SKILL fixtures for positive, nested, near-miss, hard-link, malformed-name, linked, empty, secret-bearing, performance, and injected thrown/rejected-operation checkpoints in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T055 [P] [US1] Add failing Repository scan tests beginning with the generation-0 Source already selected lexically from captured `cwd`/`--cwd`: its stable `sourceId`, escaped boundary, enabled idle state, empty files/Diagnostics, and null request ID exist synchronously with zero filesystem I/O before admission but never authorize a read, while central admission uses only the retained raw selected root. Verify raw path segments remain filesystem operands; NFC paths are collision-free public identities; each colliding raw-sibling group is rejected before member I/O and yields exactly one pathless session-scoped Diagnostic; hard links follow T020's one-Source-scan-attempt static-prepass, usable-identity, read-once, independent-attempt, primary/alias/provenance, and ordered-fallback/late-derived rules. Cover exact structural-`lstat` `ENOENT` returned outcomes; unchanged propagation of every other throw/rejection through domain layers; no attempt result/generation; REST `OperationError` or ownerless-startup top-level behavior; deterministic contracted-partial values only; request-bound tickets, atomic recognition, last-commit retention, revocation, no verdict, and no relationship-target reads in `tests/integration/repository-scan.test.ts`
- [ ] T056 [P] [US1] Add zero-activation/mutation tests under `--no-open` or instrumentation begun after the isolated startup helper. Use and record local fixture roots; instrument product socket/HTTP(S)/DNS/SMB/MCP/URI/image surfaces, separately classify and validate the two exact FR-022 authorized internal loopback classes at the issued `127.0.0.1` authority—closed unauthenticated static/SPA `GET`/`HEAD` and capability-authenticated declared API requests—and prove Codex SKILL discovery causes no child process, dynamic evaluation/import, MCP connection, prohibited direct product-issued outbound request, URI load, mutation-capable open, or filesystem mutation across every other surface. Prove explicit UNC/server-share/device inputs cause zero filesystem/DNS/SMB calls, and document lexically indistinguishable pre-mounted/mapped network storage as the excluded OS-mediated limitation. Compare content/length/identity/link/mode/mtime/ctime/xattr/ACL state and record OS-only atime separately in `tests/integration/security/zero-activation.test.ts`
- [ ] T057 [P] [US1] Add failing contracts for generation 0's exact Repository Source and boundary origins, `GET /api/v1/session`, exact liveness `{ sessionId, globalContentEpoch, globalDisableInProgress }`, and Repository rescan admission. Every normal inspection-data success carries and final-gate rechecks `globalContentEpoch` with a null fence; while a disable fence is non-null, the session route exposes only `GlobalFenceRecoverySnapshot` and every other full data route is `409 global-disable-pending`. When the process-wide resource registry is poisoned, require `POST /api/v1/repository/rescan` to return `409 resource-cleanup-restart-required` before scheduling, ID/job allocation, state mutation, or filesystem I/O. Retain the exact pre-/post-acceptance `OperationError`, startup ownership, request correlation, deterministic first-scan versus explicit stale behavior, stale IDs, and immutable whole-buffer delivery in `tests/contract/http-api-session.test.ts`
- [ ] T058 [P] [US1] Add failing inventory tests for Codex rows; an escaped, inert Repository root label rendered from `SourceBoundary.displayRoot` and `origin` distinctly from every Source-relative item path and never reused as a navigation/read locator; source/path/kind labels; progress; empty state; rescan; retry; diagnostics; and zero source text or declared-value exposure in session summaries in `tests/unit/app/inventory.test.ts`
...
- [ ] T061 [US1] Implement registry records and compile closed matcher/traversal/`DerivationProgram` grammar into the already-defined T030 `TraversalPlan` and segment-program types without widening them; enforce canonical round trips, reciprocal validation, one-edge derivation acyclicity, `./` Repository anchoring, production loading that excludes official-source evidence, and an allowlisted structural projection vocabulary that cannot represent natural-language interpretation/ranking, customization correctness/validity/compliance/effectiveness/quality verdicts, validation/lint, or remediation/fix behavior in `src/inspection/rules/types.ts` and `src/inspection/rules/registry.ts`
...
- [ ] T067 [US1] Implement plan-driven Repository enumeration from the separately admitted retained raw root, never the generation-0 boundary; preserve raw operands and collision-free NFC public paths, emit one pathless session-scoped Diagnostic for each normalization collision lifecycle, and implement T019–T021/T030–T031's rows 1–28, explicit registered directory enumeration, static hard-link prepass/read-once scope, usable identity/nlink checks, ordered-fallback/late-derived zero-read rejection, and process-wide close registry. Catch only exact structural-`lstat` `ENOENT`; let every other throw/rejection propagate unchanged with no domain result, retain only deterministic returned contracted-partial outcomes, and enforce authority revocation, late discard, and no verdict in `src/inspection/scan.ts`
- [ ] T068 [US1] Implement automatic first scan from the already-present generation-0 Repository Source and FIFO explicit rescan with one opaque request ID through Source/progress/attempt/successful generation; admit only the retained raw selected root, never the escaped boundary; serialize coordinator work, select generation at dequeue, reject duplicates, atomically publish complete or contracted-partial only for deterministic returned outcomes, retain the same Source on deterministic first-scan failure, and let an ownerless automatic-startup throw/rejection escape to the process top level with no Operation Error or liveness guarantee. For explicit REST rescan rejection, preserve the last commit and stale reference to its accepted-job Operation Error; clear only on successful replacement, invalidate generation IDs, and discard late work in `src/session/session.ts`, `src/session/stale-failures.ts`, and `src/session/scan-generation.ts`
- [ ] T069 [US1] Implement deterministic Repository summaries/admissions and the complete closed `OperationError` REST lifecycle in `src/host/api-router.ts`: the generation-0 Source has the exact escaped non-authorizing boundary and null request ID; successful admissions correlate Source/progress/status/generation; a pre-acceptance throw/rejection returns only HTTP `500` plus the generic error with null request ID and no job/retention; an accepted-job rejection retains the same closed error with its non-null request ID, no Diagnostic/result/generation/raw cause, and optional stale reference for an explicit rescan. Gate Repository rescan on the process-wide registry before scheduling: poisoned state returns `409 resource-cleanup-restart-required` with zero ID/job/state mutation or filesystem I/O and preserves the prior snapshot. Preserve conflicts, stale IDs and snapshots, Source-relative paths only in authenticated DTOs, and no root/path/raw-error projection into operational records
...
- [ ] T071 [US1] Implement the accessible Repository header with an escaped, inert `SourceBoundary.displayRoot`/`origin` root label visually and semantically distinct from Source-relative item paths and never used as a navigation/read locator; current/stale snapshot status; request-correlated progress/rescan controls that display only the active `scanRequestId` state and cannot let older status/inventory satisfy a newer command; Source-relative-path filters; Codex SKILL list; and item summaries in `app/pages/index.vue`, `app/components/inventory/ScanProgress.vue`, `app/components/inventory/InventoryFilters.vue`, `app/components/inventory/InventoryList.vue`, and `app/components/inventory/InventoryItem.vue`. Automatically updating status must expose keyboard-operable pause/resume plus on-demand refresh without stopping the underlying scan
...
**Purpose**: Make Codex `SKILL.md` files inspectable as complete inert authored source and closed allowlisted typed metadata without yet admitting the separate `agents/openai.yaml` physical candidate.
...
- [ ] T075 [P] [US2] Add failing post-read verified-byte tests proving any NUL byte yields diagnostic-only `binary`, no source/ranges/authored literals/comparison, and an otherwise publishable contracted-partial generation; every non-NUL file is decoded exactly once with UTF-8 replacement semantics as readable `utf-8`, `utf-8-bom`, or `utf-8-replaced`, one leading BOM is recorded/removed, inserted `U+FFFD` remains in complete `sourceText` and ordinary parsing/extraction/display/comparison without making the scan partial, and no alternate charset, retry, sampling, or truncation occurs. Cover inert Markdown/frontmatter, safe YAML, exact ranges, canonical typed literals, deterministic malformed extraction, mandatory Worker dispatch, and unchanged propagation of every decoder/parser/Worker/execution-environment throw/rejection with no domain catch, cause classification, recovered result, Diagnostic, or generation in `tests/unit/inspection/parsers.test.ts`
- [ ] T076 [P] [US2] Add failing Worker-pool tests for a fixed package URL and environment-owned worker/message/memory/time capacity with no Inspector numeric cap; prove every Worker crash, thrown handler, rejected message or parser operation propagates unchanged past the pool/parser/scan domain with no catch, cause classification, retry, Diagnostic, recovered parser/extraction/recognition/item/result/body/generation, or contracted-partial result, while the trigger-owning outer boundary alone applies the generic REST `OperationError` lifecycle or startup top-level propagation; allow Worker replacement only for a later admitted attempt, and cover authority revocation plus late-result discard in `tests/unit/inspection/seed-parsers.test.ts`
...
- [ ] T081 [P] [US2] Add failing file-detail contracts for complete inert authored source, exact ordered `authoredLiteral` values, strict/stale IDs, no-store behavior, source-value-free Diagnostics, and minimum metadata; prove a thrown/rejected REST-owned encoding or serialization operation is caught only at the outer router boundary and returns the generic closed pre-acceptance `OperationError` with null request ID, HTTP `500`, no job/result/generation or success bytes, while a post-commit delivery rejection leaves the committed snapshot unchanged, emits no successful payload, exposes no raw cause, and is never partial in `tests/contract/http-api-files.test.ts`
...
- [ ] T087 [P] [US2] Implement inert Markdown/frontmatter extraction with exact import/frontmatter spans, ECMAScript UTF-16 ranges, ordered duplicates, authored-literal round trips, internal typed semantics, and recognition-atomic failure in `src/inspection/parsers/markdown.ts`
- [ ] T088 [P] [US2] Implement atomic YAML core-schema extraction with aliases/custom tags disabled, CST/token-backed exact authored slices, ECMAScript UTF-16 ranges, ordered duplicates, internal typed semantics, and recognition-atomic failure in `src/inspection/parsers/yaml.ts`
- [ ] T089 [US2] Implement the parser Worker pool with a fixed package URL and environment-owned worker/message/memory/time capacity with no Inspector numeric cap; let every Worker crash, thrown handler, rejected message, decoder, or parser operation propagate unchanged without pool/parser/scan-domain catch, cause classification, retry, Diagnostic, recovered result, or partial publication; permit replacement only for a later admitted attempt and implement publication-authority revocation with late-result discard, leaving REST `OperationError` or startup top-level handling exclusively to the trigger-owning outer boundary in `src/inspection/parsers/pool.ts` and `src/inspection/parsers/worker.ts`
- [ ] T090 [US2] Implement exact `SourceTextRange` validation in ECMAScript UTF-16 code units, authored-literal/typed-semantic separation, duplicate-occurrence retention, overlap rules, and the JSON-safe typed semantic union whose integer, float, and date-time payloads are canonical strings, without credential detection or environment resolution, in `src/inspection/parsers/source-ranges.ts`
- [ ] T091 [US2] Implement the closed condition registry, evidence-linked `SourceConditionFact` and `ApplicabilityAssessment` records, and deterministic precedence projection in `src/inspection/applicability/conditions.ts`, `src/inspection/applicability/context.ts`, and `src/inspection/applicability/precedence.ts`
...
- [ ] T095 [US2] Integrate verified-byte classification and exactly-once UTF-8 replacement decoding in `src/inspection/scan.ts`: NUL-containing input becomes diagnostic-only `binary` and contracted-partial, while every non-NUL input becomes readable `utf-8`, `utf-8-bom`, or `utf-8-replaced`, records/removes one leading BOM, preserves inserted `U+FFFD` in complete authored source, and continues through literal extraction, atomic per-recognition parsing, display/comparison, and one-edge derivation without partial status by itself; perform no alternate decoding, sampling, or truncation, dispose raw bytes after snapshot construction, and let every read/decoder/parser/Worker/extractor throw/rejection propagate unchanged without domain catch or conversion
...
- [ ] T097 [US2] Implement `GET /api/v1/files/{fileId}` with strict opaque IDs, complete authored-source DTOs, exact ordered literal metadata, production encoding, no-store behavior, Diagnostics, and stale responses; catch a thrown/rejected encoding or serialization operation only at this REST-owning boundary and return the generic closed pre-acceptance `OperationError` with null request ID, HTTP `500`, no retained job/error/result/generation or success bytes, while a post-commit delivery rejection leaves the commit unchanged, emits no successful payload, exposes no raw cause, and is never represented as partial in `src/host/api-router.ts`
...
- [ ] T113 [US1] Integrate deterministic one-edge admission, one verified read per metadata file, alias aggregation, and source-value-free diagnostics in `src/inspection/scan.ts`
...
**Purpose**: Add complete literal source and closed allowlisted typed detail for each admitted `agents/openai.yaml` candidate.

**Independent Test**: Open valid, malformed, literal-credential-bearing, and changing metadata candidates, then inject thrown/rejected operations; verify atomic YAML extraction, exact authored literals with distinct typed semantics, unchanged exception propagation, seed provenance, stale handling, no masking/reveal controls or environment-reference resolution, zero activation, and cleanup on file or generation removal.
...
- [ ] T116 [P] [US2] Add failing Codex skill-metadata tests for allowlisted field IDs, ordered duplicate occurrences, exact YAML source slices/UTF-16 ranges, typed-semantic separation, seed provenance, unknown fields, malformed/overlapping ranges, deterministic returned recognition-atomic failures, environment-reference non-resolution, and exact evidence; prove every thrown/rejected decoder/parser/Worker/extractor operation propagates unchanged through metadata/recognition/scan layers without classification, retry, Diagnostic, recovered parser/extraction/metadata/recognition/derived/item/result/body/generation, or contracted-partial output, leaving only the prior commit and the owning outer-boundary lifecycle behavior in `tests/unit/inspection/codex-metadata.test.ts`
- [ ] T117 [P] [US2] Add failing file-detail and removed-reveal-route contracts for exact ordered skill-metadata literals, stale IDs, and zero client retention; prove a thrown/rejected pre-commit encoding operation is caught only by the REST router and returns the generic null-ID Operation Error with no job/result/generation/success bytes, while a post-commit delivery rejection leaves the commit unchanged, emits no success payload or raw cause, and is never partial in `tests/contract/http-api-files.test.ts`
...
- [ ] T120 [US2] Extend Codex recognition with allowlisted `agents/openai.yaml` fields, exact authored-literal extraction, seed applicability, relationships, diagnostics, and evidence in `src/inspection/recognizers/codex.ts`
...
**Independent Test**: Open Claude skills with metadata, contained declarations, references, supported-vendor symlinks, malformed frontmatter, and secrets; verify complete literal detail, the exact-launch skills-directory-plugin applicability fact, explicit `shared.excluded.symlink-target` diagnostics, no manifest read authority, no target reads, and unchanged Codex detail.
...
- [ ] T148 [US2] Extend Claude recognition with exact metadata, conditional applicability, exact-launch skills-directory-plugin facts, relationships, `shared.excluded.symlink-target` parity diagnostics, and evidence without creating a manifest candidate in `src/inspection/recognizers/claude.ts`
- [ ] T149 [US2] Integrate atomic Claude extraction and emit only direct one-hop provenance-scoped relationships in `src/inspection/scan.ts`; never recurse, expand, read a target, or confer authority, and retain complete deterministic relationships subject only to environment capacity. Let every extraction/relationship throw/rejection propagate unchanged without domain catch, cause classification, retry, item/recognition/relationship/derived result/body/generation, retain only the prior commit, and leave generic lifecycle representation to the trigger-owning outer boundary
...
**Independent Test**: Open Copilot skills from all three directories and shared physical files; verify closed allowlisted metadata, separate surface applicability, progressive-loading uncertainty, no winner claims, complete literal source, and unchanged Codex/Claude details.
...
- [ ] T173 [US2] Extend Copilot recognition with exact metadata, selection uncertainty, relationships, and exact evidence in `src/inspection/recognizers/copilot.ts`
...
**Independent Test**: Use an all-tool fixture with unique skills, duplicate names, shared physical files, hard-link aliases, deterministic item failures, secrets, and injected throws/rejections; verify deterministic rows, multi-recognition, filters, contracted-partial continuity only for returned entry-local outcomes, unchanged exception propagation with whole-attempt abort and no item, recognition, derivation, result body, or generation while only the prior committed snapshot remains, rescan replacement, and responsive interaction performance.

**Visible Checkpoint**: Users can filter and understand the complete skill-first inventory.

### Fixtures and tests first

- [ ] T178 [US1] Create the all-tool SKILL fixture with every supported selector, shared files, hard-link aliases, duplicate names, near misses, deterministic failures, secrets, and injected thrown/rejected-operation checkpoints in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T180 [P] [US1] Add failing integration tests for deterministic physical-file and recognition order, read-once merging, exact NFC primary/alias aggregation, atomic continuity, progress, and contracted-partial publication only after complete traversal and deterministic returned entry-local failure; prove every injected throw/rejection propagates unchanged without domain catch, cause classification, retry, extra read, recovered result, or new generation, retaining only the prior committed snapshot and leaving lifecycle handling to the trigger-owning boundary in `tests/integration/repository-scan.test.ts`
...
- [ ] T189 [US1] Preserve source-value-free diagnostics and keep the sensitive-content notice available before detail navigation without exposing source during inventory loading, empty, retry, or replacement states in `app/components/diagnostics/DiagnosticList.vue` and `app/components/inspection/SensitiveContentNotice.vue`
...
- [ ] T205 [US1] Create Codex instruction fixtures for overrides, regular files, configured fallbacks, empty files, numerous fallback names plus injected thrown/rejected-operation checkpoints, ancestry-comparable and incomparable paths, imports, secrets, malformed content, and near misses in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T208 [US1] Add failing scan tests for static Codex instructions and pure-function tests feeding validated in-memory fallback declarations into derivation, proving complete success retention subject only to vendor/runtime/environment capacity with no Inspector numeric declaration cap; prove any thrown/rejected derivation operation propagates unchanged without domain catch, cause classification, retry, partial declaration/plan/candidate/derived output, Diagnostic, item/result/body/generation, while the prior snapshot remains and only the trigger-owning boundary handles lifecycle. Also cover ancestry comparability, orphan/config escape rejection, and zero carrier/target access before rule registration in `tests/integration/repository-scan.test.ts`
...
**Independent Test**: Open static Codex instruction fixtures and verify override-first selection, broad-to-narrow conditional order, unknown runtime `cwd`, vendor/runtime-reported instruction-capacity facts without an Inspector cap, imports as relationships only, stale-ID behavior, diagnostics, and detail-state cleanup; separately verify fallback detail projection from an in-memory carrier without reading any config path.

**Visible Checkpoint**: Selecting a static Codex instruction opens complete inert detail with explicit order, vendor/runtime-reported instruction-capacity facts, conditions, and an honest pre-carrier fallback status.

### Tests first

- [ ] T216 [P] [US2] Add failing Codex tests for override-first selection, broad-to-narrow conditional order, unknown runtime `cwd`, vendor/runtime-reported instruction-capacity facts without an Inspector-defined cap, and all configured fallback basenames in `tests/unit/inspection/codex-composition.test.ts`
- [ ] T217 [P] [US2] Add failing import/reference tests for exact authored target slices, `targetOrigin`, documented defaults, internal normalization, lexical/cycle/boundary status, direct non-recursive relationships, deterministic retention subject to environment capacity, nested/transitive rejection before target access, environment-reference non-resolution, and zero target authority; prove every thrown/rejected relationship/provenance operation propagates unchanged with no domain catch, cause classification, retry, recovered relationship/provenance/recognition/derived output, Diagnostic, or generation, and only the trigger-owning outer boundary handles lifecycle in `tests/unit/inspection/relationships.test.ts` and `tests/integration/inspection-safety.test.ts`
...
- [ ] T222 [US2] Implement Codex instruction composition, fallback projection, byte-budget facts, and direct one-hop non-recursive provenance-relative relationship extraction in `src/inspection/applicability/precedence.ts` and `src/inspection/parsers/markdown.ts`; targets confer zero read authority and nested/transitive projection is omitted before access
- [ ] T223 [US2] Integrate Codex instruction exact authored-literal preservation, atomic parsing, complete deterministic direct relationship-only references subject only to environment capacity, parser scratch/transient-semantic disposal while retaining complete authored source for deterministic returned outcomes, and fallback provenance only after an admitted carrier exists; let every parser/relationship/assembly throw/rejection propagate unchanged without domain catch, cause classification, retry, item/recognition/relationship/derived result/body/generation, retaining only the prior commit in `src/inspection/scan.ts`; never recurse, expand, or read a relationship target
...
- [ ] T238 [P] [US2] Add failing Claude import tests for exact authored target slices/ranges, internal normalization, cycles, boundary status, direct non-recursive relationships, deterministic retention subject to environment capacity, nested/transitive rejection before target access, environment-reference non-resolution, and zero target authority; prove every thrown/rejected relationship/provenance operation propagates unchanged with no domain catch, classification, retry, recovered relationship/provenance/recognition/derived output, Diagnostic, or generation, leaving lifecycle representation only to the trigger-owning outer boundary in `tests/unit/inspection/relationships.test.ts`
...
- [ ] T242 [US2] Extend Claude instruction recognition with exact metadata, layer conditions, complete direct one-hop non-recursive relationships, source-value-free environment-failure diagnostics, and evidence in `src/inspection/recognizers/claude.ts`; relationship targets confer zero read authority and nested/transitive projection is omitted before access
- [ ] T243 [US2] Integrate Claude instruction parsing, exact authored-literal extraction, complete deterministic direct relationship-only imports subject only to environment capacity, and parser scratch/transient-semantic disposal while retaining complete authored source for deterministic returned outcomes; let every parser/relationship/assembly throw/rejection propagate unchanged without domain catch, cause classification, retry, item/recognition/relationship/derived result/body/generation, retaining only the prior commit in `src/inspection/scan.ts`; never recurse, expand, or read a relationship target
...
- [ ] T265 [US2] Extend Copilot recognition with closed allowlisted instruction metadata, surface conditions, pending settings applicability, relationships, diagnostics, and evidence in `src/inspection/recognizers/copilot.ts`
...
**Independent Test**: Use an all-vendor instruction fixture and verify the exact pre-carrier shared-file matrix, one physical item/read per admitted file, separate recognitions/provenances, no filename-based Codex promotion of nested `CLAUDE.md`, an explicit dormant fallback state, deterministic order, filters, contracted-partial continuity for deterministic entry-local non-capacity failures, and rescan cleanup.

**Visible Checkpoint**: Users can understand the complete static instruction inventory, every shared-file interpretation, and the one configured fallback integration that will activate when MCP admits its minimum carrier.

### Tests first

- [ ] T268 [US1] Finalize the pre-carrier all-vendor instruction fixture with `AGENTS.md` Codex+Copilot, root `CLAUDE.md` Claude+Copilot, nested `CLAUDE.md` Claude-only plus a dormant configured-fallback variant, Claude-only `CLAUDE.local.md`, every other selector, deterministic failures, secrets, exclusions, aliases, and injected thrown/rejected-operation checkpoints in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T270 [P] [US1] Add failing integration tests for one-read shared-file assembly, the exact pre-carrier recognition matrix, zero Codex recognition on a dormant nested fallback, deterministic provenance and NFC primary/alias order, atomic continuity, and contracted-partial publication only after complete traversal and deterministic returned entry-local failure; prove every injected throw/rejection propagates unchanged without domain catch, classification, retry, new item/derived result/body/generation, config or rejected-target access, retaining only the prior commit in `tests/integration/repository-scan.test.ts`
...
**Independent Test**: Compare exactly two readable current-generation instruction files and verify complete authored source plus exact occurrence-aligned metadata literals, layering, fallback, applicability, relationship, and provenance differences without correctness claims or environment-reference resolution.
...
**Independent Test**: Inspect config layers with validated fallback basenames, named servers, duplicates, absent fields, malformed tables, hostile commands, secrets, and standalone MCP near misses; verify atomic seed/derived-rule admission, all configured fallback rows subject to environment-owned capacity, owner-file identity, no synthetic MCP file, no standalone MCP candidate, no config-detail badge, one verified read, and zero connection.
...
- [ ] T282 [P] [US1] Add failing matcher tests for atomic `codex.repo.config` plus `codex.derived.fallback-basename` registration, exact config-carrier admission, and complete configured derivation on success subject to environment-owned capacity with no Inspector numeric declaration cap; prove any thrown/rejected registry/derivation operation propagates unchanged with no domain catch, cause classification, retry, partial registry/plan/candidate/instruction/derived output, Diagnostic, or generation. Also prove no standalone Codex MCP candidate and no promotion of plugin, agent-reference, User, managed, or arbitrary config paths in `tests/unit/inspection/rules.test.ts`
...
**Independent Test**: Open contained Codex declarations and verify active project-config precedence, trust conditions, duplicate names, parent/agent inheritance facts, exact authored-literal preservation, diagnostics, no prohibited or customization-selected DNS/socket/HTTP/MCP/auth/probing request, and no command, expansion, or referenced read; classify the two exact FR-022 authorized internal loopback HTTP classes separately.
...
- [ ] T321 [US2] Implement inert strict-JSON extraction with closed Claude MCP field IDs, exact tree-token source spans/UTF-16 ranges, source-order duplicates, authored-literal round trips, internal typed semantics, schema distinctions, and environment-owned parser capacity with no Inspector numeric cap; keep deterministic returned malformed/extraction outcomes recognition-atomic and source-value-free, but let every decoder/parser/extractor throw/rejection propagate unchanged with no catch, cause classification, retry, recovered parser/extraction/recognition/derived result, Diagnostic, or generation in `src/inspection/parsers/json.ts`
...
**Independent Test**: Inventory both exact workspace-root forms, keep nested `.mcp.json` CLI-only, merge root `.mcp.json` CLI/VS Code provenances into one file/read/recognition, expose the release-note/current-guide conflict, authorize no VS Code-owned root-schema fields or inferred winner, and reject general `.vscode/settings.json`, User/profile MCP, links, aliases, and near misses.

**Visible Checkpoint**: Users can distinguish the documented `.vscode/mcp.json` `servers` schema from the VS Code 1.118+ root-path provenance whose schema and total same-name order remain unknown.

### Fixtures and tests first

- [ ] T354 [US1] Create Copilot VS Code MCP fixtures for exact `.vscode/mcp.json`, exact 1.118+ root `.mcp.json`, root CLI/VS Code overlap, nested CLI-only near misses, malformed `servers`, hostile commands, secrets, links, aliases, general settings, User/profile state, and unsupported VS Code root-schema inference in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T357 [US1] Add browser acceptance for both Copilot VS Code MCP paths, the `.vscode` schema badge, root evidence-conflict/unknown-schema state, merged provenance, filters, exclusions, diagnostics, and no connection controls in `tests/e2e/copilot-vscode-mcp-inventory.spec.ts`

### Implementation

- [ ] T358 [US1] Add Copilot VS Code MCP/User/agent facts together with the 1.118/current-guide conflict, path-specific schema availability, total-order unknowns, complete base lookup/selection, and dormant-owner strategy records so the production registry remains closed without admitting Custom Agent files in `shared/registries/vendor-behaviors.ts` and `shared/registries/runtime-composition.ts`
- [ ] T359 [US1] Add the two exact VS Code MCP rules `copilot.repo.mcp.vscode` and `copilot.repo.mcp.vscode-root`; keep nested root-form files CLI-only and general settings, User, and profile locations path-negative without prematurely owning `copilot.excluded.vscode-settings` or defining new MCP exclusion IDs in `shared/registries/inspection-rules.ts`
- [ ] T360 [US1] Add current-guide and `vscode.copilot.mcp.workspace-root-release` records plus reciprocal backlinks for the conflicting VS Code MCP behavior/rules/strategy and both non-authorizing VS Code MCP/agent facts owned in this phase in `shared/registries/official-sources.ts`
- [ ] T361 [US1] Implement exact `.vscode/mcp.json` matching with its dedicated schema and exact root `.mcp.json` matching as VS Code path/surface-only provenance with no VS Code-owned extractor in `src/inspection/rules/copilot.ts` and `src/inspection/recognizers/copilot.ts`
- [ ] T362 [US1] Integrate Copilot VS Code MCP classification so root `.mcp.json` merges compatible CLI/VS Code provenances into one physical file/read and one Copilot/MCP recognition without changing nested CLI candidates in `src/inspection/scan.ts`
- [ ] T363 [US1] Extend MCP inventory rows and semantically equivalent English/Japanese messages for the `.vscode` schema, root evidence conflict/unknown schema and order, merged provenance, and exclusions in `app/components/inventory/InventoryItem.vue`, `app/locales/en.ts`, and `app/locales/ja.ts`

---

## Phase 31: Copilot VS Code MCP Detail

**Purpose**: Add complete literal `.vscode/mcp.json` detail plus exact path/evidence detail for 1.118+ root `.mcp.json`, preserving unknown root schema, total-order uncertainty, and trust conditions.

**Independent Test**: Open hostile and malformed `.vscode/mcp.json` and root `.mcp.json`; verify dedicated fields only for `.vscode`, CLI-only extraction plus VS Code path-only provenance for the shared root file, conflict/unknown same-name resolution, trust, exact authored literals, diagnostics, and zero connection.
...
- [ ] T366 [P] [US2] Add failing VS Code MCP-detail API tests for complete authored source, `.vscode` occurrence-ordered field literals/relationship targets, root path-only conflict provenance and zero VS Code-owned root fields, conditions, no environment-reference substitution, diagnostics, and stale IDs in `tests/contract/http-api-files.test.ts`
...
- [ ] T368 [US2] Add browser acceptance for sensitive-content acknowledgement, exact literal credential/environment-reference display, no process-environment sentinel substitution, no masking/reveal controls, complete literal VS Code MCP detail for both paths, `.vscode` schema versus root unknown-schema conflict, duplicate uncertainty, trust, diagnostics, and zero connection behavior in `tests/e2e/copilot-vscode-mcp-detail.spec.ts`

### Implementation

- [ ] T369 [US2] Extend the inventory-owned Copilot VS Code MCP strategy with the current-guide/1.118 conflict, path-specific schema availability, unknown root/`.vscode`/User/agent/plugin winner, trust, and provenance-specific authored relationship projections without adding strategy IDs in `shared/registries/runtime-composition.ts`
- [ ] T370 [US2] Implement `.vscode/mcp.json` VS Code schema metadata and root `.mcp.json` path-only conflict provenance with zero VS Code-owned extractor fields, plus duplicate uncertainty and trust metadata, in `src/inspection/recognizers/copilot.ts`
- [ ] T371 [US2] Add inert JSONC mode exclusively for the documented `.vscode/mcp.json` extractor, with tree-backed exact source spans/UTF-16 ranges, closed VS Code MCP field IDs, source-order duplicates, authored literals plus internal typed semantics, comments, schema distinctions, and environment-owned parser capacity with no Inspector numeric cap. Never invoke this VS Code extractor for root `.mcp.json` path-only provenance; its independent CLI extraction remains owned by the CLI parser. Keep deterministic returned malformed/extraction outcomes recognition-atomic and source-value-free, but let every decoder/parser/extractor throw/rejection propagate unchanged with no catch, cause classification, retry, recovered parser/extraction/recognition/derived result, Diagnostic, or generation in `src/inspection/parsers/json.ts`
- [ ] T372 [US2] Integrate `.vscode/mcp.json` exact authored-literal preservation, conditions, diagnostics, and non-following relationships plus root `.mcp.json` path-only conflict provenance and independent CLI-owned extraction, with zero VS Code-owned root fields or cross-provenance schema promotion, in `src/inspection/scan.ts`
- [ ] T373 [US2] Extend typed detail and semantically equivalent English/Japanese messages that distinguish the `.vscode/mcp.json` schema from root `.mcp.json` path-only conflict/unknown-schema provenance, merged CLI provenance, trust, safety, zero VS Code-owned root fields, and total-order uncertainty in `app/components/inspection/RecognitionDetails.vue`, `app/locales/en.ts`, and `app/locales/ja.ts`
...
- [ ] T392 [P] [US1] Add failing integration tests for read-once shared MCP, deterministic recognition/provenance order, admitted-owner attachment, dormant-owner nonpublication, aliases, and contracted-partial continuity only for deterministic returned entry-local outcomes; prove every injected throw/rejection propagates unchanged without domain catch, classification, retry, item/recognition/derived result/body/generation, retains only the prior commit, and initiates zero connection/target reads in `tests/integration/repository-scan.test.ts`
...
- [ ] T394 [US1] Complete priority read-once MCP file/owner assembly, deterministic recognition/provenance/schema order, owner-gated dormant adapters, no synthetic files, and source-value-free diagnostics in `src/inspection/scan.ts`
...
**Independent Test**: Compare exactly two readable current-generation rule files and verify complete literal source plus aligned paths, layers, trust, provenance, applicability, and documentation status.
...
- [ ] T472 [US2] Extend Copilot command recognition with exact metadata, conditions, relationships, diagnostics, and evidence in `src/inspection/recognizers/copilot.ts`
...
**Independent Test**: Verify one physical item/read and two recognitions for root direct-child `.claude/commands/*.md`, Claude-only recognition for nested commands, deterministic namespaces/provenance, filters, exclusions, aliases, injected throws/rejections with unchanged propagation and atomic abort, and rescan cleanup.

**Visible Checkpoint**: Users can distinguish shared root commands from nested Claude-only commands.

### Tests first

- [ ] T475 [US1] Finalize command fixtures for recursive Claude namespaces, root Copilot-compatible commands, nested Claude-only files, duplicate names, secrets, references, aliases, injected thrown/rejected-operation checkpoints, and near misses in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T478 [P] [US1] Add failing integration tests for read-once root commands, deterministic recognition/provenance and alias order, and contracted-partial continuity only for deterministic returned entry-local outcomes; prove every injected throw/rejection propagates unchanged without domain catch, classification, retry, item/recognition/derived result/body/generation, retains only the prior commit, and performs no referenced-target read in `tests/integration/repository-scan.test.ts`
...
**Independent Test**: Compare exactly two readable current-generation command files and verify complete literal source plus aligned namespaces, invocation, recognition, precedence, provenance, and references.
...
**Independent Test**: Compare exactly two readable current-generation prompt files and verify complete literal source plus aligned invocation, scope, provenance, applicability, and references.
...
- [ ] T514 [US1] Implement Codex agent matching and closed allowlisted recognition in `src/inspection/rules/codex.ts` and `src/inspection/recognizers/codex.ts`
...
**Independent Test**: Open hostile and malformed Codex agents and verify inert TOML parsing with environment-owned capacity, model/reasoning/sandbox/skills, parent inheritance, reapplied live sandbox/approval facts, MCP carrier inheritance/origin relationships, no agent-owned MCP recognition, config-path relationships, exact authored literals, diagnostics, detail-state cleanup, and zero connection.
...
- [ ] T517 [P] [US2] Add failing inert TOML parsing tests for Codex agent fields, deterministic returned malformed recognition-atomic extraction, and environment-owned parser capacity with no Inspector numeric cap; prove every decoder/parser/extractor throw/rejection propagates unchanged with no domain catch, cause classification, retry, parser/extraction/recognition/item/result/body/generation, Diagnostic, or contracted-partial result in `tests/unit/inspection/parsers.test.ts`
...
- [ ] T522 [US2] Extend the existing inert TOML carrier parser with Codex agent normalization and extraction in `src/inspection/parsers/toml.ts`
...
- [ ] T534 [US1] Implement Claude agent matching and closed allowlisted recognition in `src/inspection/rules/claude.ts` and `src/inspection/recognizers/claude.ts`
...
- [ ] T543 [US2] Extend Claude recognition with closed allowlisted agent metadata, owner-gated contained MCP, inert Hook origins, applicability, relationships, diagnostics, and evidence in `src/inspection/recognizers/claude.ts`
...
- [ ] T562 [US2] Extend Copilot recognition with closed allowlisted agent metadata, owner-gated MCP, inert Hook origins, applicability, relationships, diagnostics, and evidence in `src/inspection/recognizers/copilot.ts`
...
- [ ] T565 [US1] Finalize all-vendor custom-agent fixtures for every supported path, layer, duplicate name, shared Claude/Copilot file, owner-attached MCP declaration, Codex carrier-inheritance relationship, malformed metadata, secret field, reference, exclusion, alias, and injected thrown/rejected-operation checkpoint in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T568 [P] [US1] Add failing integration tests for read-once shared agents, deterministic Claude/Copilot agent/MCP recognition and provenance order, Codex relationship-only inheritance, aliases, and isolated deterministic returned entry-local outcomes; prove every injected throw/rejection propagates unchanged without domain catch, classification, retry, item/recognition/derived result/body/generation, retains only the prior commit, and creates zero synthetic files/connections or relationship-target reads in `tests/integration/repository-scan.test.ts`
...
**Independent Test**: Compare exactly two readable current-generation custom-agent files and verify complete literal source plus aligned context, tools, Claude/Copilot owner-attached MCP or Codex carrier-inheritance relationships as applicable, provenance, relationship, and condition differences.
...
- [ ] T593 [P] [US2] Add failing file-detail and removed-reveal-route contracts for complete literal TOML values, strict/stale IDs, no-store behavior, diagnostics, and exact metadata in `tests/contract/http-api-files.test.ts`
...
- [ ] T595 [US2] Extend the existing inert TOML carrier extraction with remaining Codex project-configuration fields and relative-base metadata while preserving closed fallback/MCP extraction in `src/inspection/parsers/toml.ts`
...
- [ ] T597 [US2] Extend Codex recognition with closed allowlisted config fields, fallback-name metadata, relationships, applicability, diagnostics, and exact evidence in `src/inspection/recognizers/codex.ts`
...
- [ ] T612 [US2] Add failing inert JSONC tests for comments, known fields, deterministic returned malformed recognition-atomic extraction, and environment-owned parser capacity with no Inspector numeric cap; require NUL-containing bytes to remain diagnostic-only `binary`, every non-NUL input to decode once as readable `utf-8`, `utf-8-bom`, or `utf-8-replaced` with retained `U+FFFD` continuing through JSONC parsing/display/comparison without partial status by itself, and every decoder/parser/extractor throw/rejection to propagate unchanged with no domain catch, classification, retry, result, Diagnostic, or generation in `tests/unit/inspection/parsers.test.ts`
...
- [ ] T617 [US2] Extend the existing inert JSONC mode with allowlisted Claude settings fields and closed declaration origins in `src/inspection/parsers/json.ts`
...
- [ ] T619 [US2] Extend Claude recognition with closed allowlisted settings metadata, owner-gated contained MCP, applicability, relationship-only targets, diagnostics, and evidence in `src/inspection/recognizers/claude.ts`
...
**Purpose**: Add complete inert Copilot settings detail with surface-specific precedence and closed allowlisted declaration metadata.
...
- [ ] T638 [US2] Extend inert JSONC extraction subject only to execution-environment capacity with allowlisted Copilot settings fields, recommendation identifiers, and closed declaration origins in `src/inspection/parsers/json.ts`
...
- [ ] T640 [US2] Extend Copilot recognition with closed allowlisted settings metadata, applicability, instruction re-projection facts, relationship-only targets, permanent MCP non-ownership, diagnostics, and exact evidence in `src/inspection/recognizers/copilot.ts`
...
- [ ] T652 [US1] Complete read-once shared-file assembly, deterministic settings/MCP recognition order, exact ownership/non-ownership, preserved Codex carrier facts, and atomic continuity for settings/configuration in `src/inspection/scan.ts`
...
**Independent Test**: Compare exactly two readable current-generation output-style files and verify complete literal source plus aligned layer, selection, surface availability, provenance, and metadata.
...
**Independent Test**: Open malformed and secret-bearing catalogs and inject thrown/rejected operations while verifying atomic JSON parsing, local source forms, remote/absolute/home/traversal rejection, complete deterministic targets per static seed, relationship-only components, exact authored literals, and Diagnostics. A deterministic returned malformed outcome may produce only contracted-partial after complete traversal; every throw/rejection propagates unchanged without domain classification or retry, aborts with no item/recognition/derived result/body/generation, retains only the prior commit, and performs zero plugin-target reads.
...
- [ ] T690 [P] [US2] Add failing local-source validation tests for leading-`./` catalog-relative containment, one target per declaration, all distinct targets per static seed in stable extractor/occurrence order, one-edge preparation, and rejection of forbidden derived authority while retaining authored relationships; prove every injected throw/rejection propagates unchanged without domain catch, cause classification, retry, item/recognition/source plan/derived result/body/generation, retaining only the prior commit in `tests/integration/repository-scan.test.ts`
...
- [ ] T694 [US2] Extend atomic JSON extraction with closed Codex catalog field IDs, exact source occurrences/ranges and authored literals, separate internal semantic paths, recognition-atomic failures, and source-value-free diagnostics in `src/inspection/parsers/json.ts`
...
- [ ] T696 [US2] Extend Codex recognition with closed allowlisted occurrence-ordered catalog metadata, validated semantic local-source declarations, exact authored relationships, applicability, diagnostics, and evidence in `src/inspection/recognizers/codex.ts`
- [ ] T697 [US2] Integrate atomic catalog parsing, complete authored-source retention, all distinct validated local targets per static seed in deterministic order, and relationship-only rejected/remote components; let every parser/extractor/derivation throw/rejection propagate unchanged without domain catch, cause classification, retry, item/recognition/source plan/derived result/body/generation, retain only the prior commit, and perform no derived reads yet in `src/inspection/scan.ts`
...
**Independent Test**: Open malformed and secret-bearing catalogs and inject thrown/rejected operations while verifying optional/local forms, catalog-relative containment, remote relationship retention, complete deterministic targets per static seed, owner-attached MCP, registration uncertainty, exact authored literals, and Diagnostics. A deterministic returned malformed outcome may produce only contracted-partial after complete traversal; every throw/rejection propagates unchanged without domain classification or retry, aborts with no item/recognition/derived result/body/generation, retains only the prior commit, and performs zero connection or plugin-target read.
...
- [ ] T714 [US2] Extend atomic JSON extraction with closed Claude catalog field IDs, exact source occurrences/ranges and authored literals, separate internal semantic paths, recognition-atomic failures, and source-value-free diagnostics in `src/inspection/parsers/json.ts`
...
- [ ] T716 [US2] Extend Claude recognition with closed allowlisted occurrence-ordered catalog metadata, validated semantic local-source declarations, exact authored relationships, owner-gated MCP, applicability, diagnostics, and evidence in `src/inspection/recognizers/claude.ts`
- [ ] T717 [US2] Integrate Claude catalog parsing with complete authored-source retention, all distinct local targets per static seed in deterministic order, owner-attached MCP with no synthetic file/connection, and relationship-only rejected/remote components; let every parser/extractor/derivation throw/rejection propagate unchanged without domain catch, classification, retry, item/recognition/source plan/derived result/body/generation, retain only the prior commit, and perform no derived reads in `src/inspection/scan.ts`
...
- [ ] T734 [US2] Extend atomic JSON extraction with closed Copilot catalog field IDs, exact source occurrences/ranges and authored literals, separate internal semantic paths, recognition-atomic failures, and source-value-free diagnostics in `src/inspection/parsers/json.ts`
...
- [ ] T736 [US2] Extend Copilot recognition with closed allowlisted occurrence-ordered catalog metadata, VS Code/CLI-only semantic local-source plans, exact authored relationships, Cloud runtime-unavailable conditions, applicability, diagnostics, and evidence in `src/inspection/recognizers/copilot.ts`
...
**Independent Test**: Verify one physical item/read with three marketplace recognitions plus Claude owner-attached MCP for the shared catalog, deterministic provenance/root-form ordering, no synthetic MCP file or connection, local-source plans, filters, exclusions, injected throws/rejections with unchanged propagation and atomic abort, Diagnostics, and rescan cleanup.
...
- [ ] T739 [US1] Finalize marketplace fixtures for every root form, local/remote source, shared triple-recognition file with Claude owner-attached MCP, malformed/secret catalog, alias, exclusion, and injected thrown/rejected-operation checkpoint in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T742 [P] [US1] Add failing integration tests for read-once shared catalogs, deterministic marketplace/MCP recognition and provenance order, aliases, and complete per-seed distinct-target handling subject only to environment capacity; prove every injected throw/rejection propagates unchanged without domain catch, classification, retry, item/recognition/source plan/derived result/body/generation, retains only the prior commit, and creates zero synthetic MCP file/connection or plugin-target reads before derivation in `tests/integration/repository-scan.test.ts`
...
- [ ] T744 [US1] Complete read-once marketplace physical-file assembly, deterministic multi-tool plus owner-attached MCP provenance, exact authored occurrences, and complete deterministic source-plan retention on success subject to environment-owned capacity with no Inspector numeric cap; let every assembly/derivation throw/rejection propagate unchanged without domain catch, cause classification, retry, source plan/provenance/recognition/item/body/generation/contracted-partial result, retaining only the prior commit; preserve no synthetic file and all exclusions in `src/inspection/scan.ts`
...
**Independent Test**: Compare exactly two readable current-generation catalog files and verify complete literal source plus aligned entries, source types, local-source plans, owner-attached MCP, provenance, registration, installation, enablement, conditions, and uncertainty without deriving or activating plugins.
...
**Independent Test**: Inventory the authored root manifest and one `.codex-plugin/plugin.json` below each validated `./` local Codex marketplace source; verify one-edge containment, complete deterministic retention subject only to environment capacity, missing target as no candidate, no orphan/remote/escaping/linked candidate, no recursive derivation, and one verified read per physical file. Every injected throw/rejection propagates unchanged without domain classification or retry, aborts with no item/recognition/derived result/body/generation, and retains only the prior commit.

**Visible Checkpoint**: Users can filter authored Codex plugin manifests with static or marketplace-derived provenance.

### Fixtures and tests first

- [ ] T751 [US1] Create Codex plugin-manifest fixtures for exact root, valid `./` local catalog sources, exact `.codex-plugin/plugin.json` targets, missing and numerous sources, remote/absolute/home/traversal sources, links, aliases, component declarations, near misses, and injected thrown/rejected-operation checkpoint in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T754 [US1] Add failing scan tests for complete deterministic static/derived Codex manifest retention subject only to environment capacity, missing-target/no-candidate handling, containment, links, aliases, read-once assembly, and no component reads; prove every injected throw/rejection propagates unchanged without domain catch, classification, retry, manifest item/recognition/provenance/derived result/body/generation, retaining only the prior commit in `tests/integration/repository-scan.test.ts`
...
- [ ] T759 [US1] Implement exact-root matching and direct one-edge Codex manifest derivation only from validated `./` local marketplace sources to the exact `.codex-plugin/plugin.json` target in `src/inspection/rules/codex.ts`
...
- [ ] T761 [US1] Integrate deterministic one-edge Codex manifest admission, one verified read, and exact NFC primary/alias aggregation; let every non-carveout read/derivation throw/rejection propagate unchanged without domain catch, cause classification, retry, manifest item/recognition/provenance/derived result/body/generation/contracted-partial result, retaining only the prior commit and leaving generic lifecycle representation to the trigger-owning outer boundary in `src/inspection/scan.ts`
...
- [ ] T769 [US2] Extend atomic JSON extraction with closed Codex plugin-manifest field IDs, exact component source occurrences/UTF-16 ranges and authored literals, separate internal typed semantics, recognition-atomic failure, and source-value-free diagnostics in `src/inspection/parsers/json.ts`
...
- [ ] T771 [US2] Implement closed allowlisted Codex plugin-manifest metadata and relationship-only components in `src/inspection/recognizers/codex.ts`
...
**Independent Test**: Inventory the authored root and validated local marketplace targets; verify optional absence, trust conditions, direct one-edge derivation with complete deterministic target retention subject only to execution-environment capacity, no recursive derivation, and no component reads.

**Visible Checkpoint**: Users can filter Claude plugin manifests with explicit root or marketplace-derived provenance.

### Fixtures and tests first

- [ ] T774 [US1] Create Claude plugin-manifest fixtures for exact root, valid local catalog sources, optional absence, numerous sources, ancestor near misses, links, aliases, components, forbidden sources, and injected thrown/rejected-operation checkpoint in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T777 [US1] Add failing scan tests for complete deterministic static/derived Claude manifest retention subject only to environment capacity, containment, links, aliases, read-once assembly, and no component reads; prove every injected throw/rejection propagates unchanged without domain catch, classification, retry, manifest item/recognition/provenance/derived result/body/generation, retaining only the prior commit in `tests/integration/repository-scan.test.ts`
...
- [ ] T782 [US1] Implement exact-root and direct one-edge local-marketplace Claude manifest derivation in `src/inspection/rules/claude.ts`
...
- [ ] T784 [US1] Integrate deterministic Claude manifest admission, one verified read, exact NFC aliases, and optional structural absence; let every non-carveout read/derivation throw/rejection propagate unchanged without domain catch, cause classification, retry, manifest item/recognition/provenance/derived result/body/generation/contracted-partial result, retaining only the prior commit and leaving generic lifecycle representation to the trigger-owning outer boundary in `src/inspection/scan.ts`
...
- [ ] T792 [US2] Extend atomic JSON extraction with closed Claude plugin-manifest field IDs, exact default/explicit component source occurrences/ranges and authored literals, separate internal typed semantics, recognition-atomic failure, and source-value-free diagnostics in `src/inspection/parsers/json.ts`
...
- [ ] T794 [US2] Implement closed allowlisted Claude plugin-manifest metadata, owner-gated MCP, and relationship-only components in `src/inspection/recognizers/claude.ts`
...
- [ ] T797 [US1] Create Copilot plugin-manifest fixtures for all four root/derived forms, order, numerous sources, shared Claude manifests, missing forms, links, aliases, components, CLI extensions, installed/hosted state, forbidden sources, and injected thrown/rejected-operation checkpoint in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T807 [US1] Integrate deterministic Copilot manifest admission, one verified read, exact NFC aliases, and complete environment-supported success handling; let every non-carveout read/derivation throw/rejection propagate unchanged without domain catch, cause classification, retry, manifest item/recognition/provenance/derived result/body/generation/contracted-partial result, retaining only the prior commit and leaving generic lifecycle representation to the trigger-owning outer boundary in `src/inspection/scan.ts`
...
- [ ] T813 [US2] Extend atomic JSON extraction with closed Copilot plugin-manifest field IDs, exact component source occurrences/ranges and authored literals, separate internal typed semantics, recognition-atomic failure, and source-value-free diagnostics in `src/inspection/parsers/json.ts`
...
- [ ] T815 [US2] Implement closed allowlisted Copilot plugin-manifest metadata and relationship-only components in `src/inspection/recognizers/copilot.ts`
...
- [ ] T818 [US1] Finalize plugin-manifest fixtures for every root/derived form, shared Claude/Copilot file with Claude owner-attached MCP, missing optional manifest, aliases, relationship-only components, exclusions, secrets, malformed content, and injected thrown/rejected-operation checkpoints in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T821 [P] [US1] Add local-manifest integration regressions for complete literal derived metadata, catalog-relative provenance, deterministic target retention subject only to environment capacity, read-once shared files, Claude owner-attached MCP, zero synthetic file/connection, and no component expansion; prove every injected throw/rejection propagates unchanged without domain catch, classification, retry, manifest item/recognition/provenance/derived result/body/generation, retaining only the prior commit in `tests/integration/repository-scan.test.ts`
- [ ] T822 [US1] Add browser acceptance for unified plugin-manifest inventory, filters, plan-driven derivation, shared recognition, Claude owner-attached MCP versus component paths, exclusions, deterministic Diagnostics versus generic Operation Error for a throw/rejection, and keyboard use in `tests/e2e/plugin-manifests-inventory.spec.ts`
...
- [ ] T826 [US1] Integrate direct one-edge local derivation, one verified read, deterministic cross-tool plus owner-attached MCP assembly, exclusions, zero synthetic files/connections, and no component expansion in `src/inspection/scan.ts`
...
**Independent Test**: Compare exactly two readable current-generation manifest files and verify complete literal source plus aligned authored metadata, form/seed provenance, registration, installation, enablement, trust, owner-attached MCP, component relationships, and uncertainty without activation or connection.
...
**Independent Test**: Verify one physical read and separate Claude/Copilot Hook recognitions on shared settings, standalone Codex/Copilot files, contained owner provenance, deterministic order, no synthetic files, exclusions, filters, aliases, injected throws/rejections with unchanged propagation and atomic abort, Diagnostics, and rescan cleanup.

**Visible Checkpoint**: Users can distinguish every supported standalone and contained Hook interpretation.

### Tests first

- [ ] T899 [US1] Finalize Hook fixtures for standalone Codex/Copilot files, Claude settings/skill/agent/plugin/marketplace owners, Copilot settings/agent owners, shared settings, relationship-only plugin paths, unreferenced scripts, secrets, aliases, exclusions, and injected thrown/rejected-operation checkpoints in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T903 [P] [US1] Add failing integration tests for read-once shared owners, deterministic Hook recognition/alias order, atomic continuity, and contracted-partial publication only after complete traversal and deterministic returned entry-local outcomes; prove every injected throw/rejection propagates unchanged without domain catch, classification, retry, referenced-hook/later read, new Hook/recognition/item/body/generation/contracted-partial result, retaining only the prior commit in `tests/integration/repository-scan.test.ts`
...
- [ ] T905 [US1] Complete read-once owner/file assembly, deterministic Hook recognition/provenance order, and no synthetic files; let every read/recognition/assembly throw/rejection propagate unchanged without domain catch, cause classification, retry, new Hook/recognition/provenance/item/body/generation/contracted-partial result, retaining only the prior commit and leaving generic lifecycle representation to the trigger-owning outer boundary in `src/inspection/scan.ts`
...
**Independent Test**: Install the package against the all-supported fixture and verify every allowlisted file, filter, recognition, injected throw/rejection with unchanged propagation, rescan path, package path, and performance target, including every late owner activation from the priority MCP adapters on one existing owner/read with zero synthetic files or connections; the currently owned Repository registry gate is exactly 49 IDs—36 static, five bounded-derived, seven vendor exclusions, and `shared.excluded.symlink-target`—and the four Global-era exclusions remain intentionally undefined until Phases 96–98.

**Visible Checkpoint**: US1 discovery is complete for every initial-release Repository customization family.

### Acceptance tests

- [ ] T913 [US1] Add the exact currently owned 49-ID Repository registry gate—36 static, five bounded-derived, seven vendor-excluded, and one shared-symlink exclusion—including distinct `copilot.repo.mcp` and `copilot.repo.mcp.vscode-root` provenances for one merged root file/read/recognition; prove all early and late-owner contained Hook/MCP recognitions add zero candidate rules, one existing owner ID/read is retained, no synthetic file is created, and the four deferred Global-era non-read exclusions are not yet defined in `tests/contract/inspection-rules.test.ts`
...
- [ ] T915 [US1] Add integration tests injecting throws/rejections from Node.js, decoders, parsers, Workers, filesystem, alias assembly, and publication; prove domain layers do not catch, classify cause, retry, or convert them, the attempt produces no item/recognition/derived result/Diagnostic/result body/generation, the trigger-owning REST boundary alone creates the generic pre- or post-acceptance `OperationError`, and an ownerless automatic startup rejection reaches the process top level, while the prior commit remains. Separately prove only deterministic returned entry-local outcomes can produce contracted-partial after complete traversal, exact structural-`lstat` `ENOENT` is the sole exception, no capacity ceiling or verdict exists, and authority revocation discards late work without hard-cancellation claims in `tests/integration/runtime-failures.test.ts`
- [ ] T916 [P] [US1] Add complete session/rescan API contracts for every Repository kind, including generation 0 with exactly one enabled idle Source selected from captured `cwd`/`--cwd`, stable source ID, escaped non-authorizing root, empty files/Diagnostics, null request ID, and zero source I/O; strict inventory envelopes; one request ID across admitted Source/progress/final state/successful generation; conflicts, stale IDs, atomic publication, capability-only authorization, path-free operational records, and no analysis/verdict fields. Require the complete generic `OperationError` entity and REST lifecycle: null request ID/HTTP `500`/no job or retention before acceptance; matching non-null ID, retained session error, no result/generation/raw cause after `202`; no entity for ownerless startup rejection; optional stale reference only for failed explicit rescan. Also require poisoned-registry Repository rescan rejection as pre-schedule `409 resource-cleanup-restart-required` with zero ID/job/state mutation/I/O and prior-state preservation in `tests/contract/http-api-session.test.ts`
- [ ] T917 [P] [US1] Add complete packaged Gunshi CLI tests for isolated install, fixed assets/Worker, same tarball, and optional `--cwd` at most once: capture invocation `process.cwd()` once and preserve its exact string when omitted; preserve POSIX absolute or Windows absolute-drive input; on Windows reject UNC/server-share/device, current-drive/root-relative, and `C:`/`C:foo` drive-relative forms before `resolve`; resolve only platform-valid plain relative input; require the selected result to pass the same shared pure `LexicalAbsoluteRootParts` parser; perform zero filesystem/network I/O and no `process.chdir()` or per-drive working-directory resolution; and fail missing/empty/duplicate/pre-resolution-invalid/parser-rejected input before session/browser creation with fixed actionable output. Include every complete POSIX/Windows accepted and rejected vector from T043. Also cover default-browser delegation, `--no-open`/printed-URL fallback, non-binding help/version, strict unknown/positional/rest rejection, awaited shutdown, root-only imports, and zero extra modes in `tests/package/npx-launch.test.ts`
- [ ] T918 [P] [US1] Extend T183 to the final registry and execute exactly ten fresh processes against one unchanged profile/fixture: wait for each automatic first scan outside timing, dispatch exactly one explicit rescan, capture its `scanRequestId`, start both timers at dispatch, and require the same ID on the qualifying visible/assistive status and committed-generation inventory; reject prior/automatic state, require the same at least nine runs to satisfy 1-second status, 10-second inventory, and both sub-100-ms interactions, record profile/digest/request ID/generation/environment, omit only personal identifiers/absolute user paths, and reject cache reset/snapshot reuse/cross-profile comparison in `tests/performance/repository-scan.test.ts` and `tests/performance/inventory-interactions.test.ts`
- [ ] T919 [US1] Add Repository-complete browser acceptance plus the documented discovery command target for inventory, filters, multi-recognition, diagnostics, empty state, request-correlated rescan/retry, keyboard use, atomic replacement, zero source/metadata/sensitive-value exposure before detail acknowledgement, and negative assertions that Inventory/Diagnostics/Source Condition Facts expose no natural-language interpretation/ranking, customization validity/correctness/compliance/effectiveness/quality verdict, validation/lint, or remediation/fix controls in `tests/e2e/repository-complete-inventory.spec.ts` and `tests/e2e/discovery.spec.ts`
...
**Independent Test**: Verify the complete currently owned 49-ID Repository rule registry—36 static, five bounded-derived, seven vendor-excluded, and one shared-symlink exclusion—plus explicit absence of the four deferred Global-era exclusions, the parser matrix, exact literal display and complete detail behavior under environment-owned capacity, safe filesystem boundary, every late owner-bound MCP activation, zero activation/connection/environment-reference resolution, file-detail and removed-reveal-route API behavior, relationships, diagnostics, stale cleanup, and zero candidate-rule additions or duplicate owner reads from contained Hook/MCP facts.

**Visible Checkpoint**: US2 inert-detail coverage is complete for every initial-release Repository customization family.

### Acceptance tests

- [ ] T920 [P] [US2] Add Repository-subgraph contracts for the exact currently owned 49-ID split of 36 static, five bounded-derived, seven vendor-excluded, and one shared-symlink exclusion, absence of the four deferred exclusions, distinct root CLI/VS Code rule provenances merged into one owner ID/read with path-only VS Code semantics, zero contained Hook/MCP candidate rules, the full early-contract-to-late-owner activation matrix, zero synthetic files/connections, every currently owned behavior/strategy/relationship/evidence backlink, exact closed presentation-allowlist membership for every emitted `(tool, kind, fieldId)` and relationship kind plus exact extractor applicability to the occurrence's actual admitted source form with no unlisted inference or cross-form promotion, reciprocal fingerprints, and offline separation in `tests/contract/inspection-rules.test.ts`, `tests/contract/runtime-composition.test.ts`, and `tests/contract/official-sources.test.ts`
- [ ] T921 [P] [US2] Add a four-parser matrix for JSONC, YAML, TOML, and Markdown/frontmatter proving NUL bytes are diagnostic-only `binary`, non-NUL bytes decode exactly once as readable `utf-8`, `utf-8-bom`, or `utf-8-replaced`, one leading BOM is removed/recorded, retained `U+FFFD` continues through atomic extraction/display/comparison without making the scan partial, and no charset fallback/sampling/truncation occurs; cover deterministic malformed returned outcomes, Worker replacement only for a later attempt, environment-owned capacity, and unchanged propagation of every decoder/parser/Worker/extractor throw/rejection with no domain catch, cause classification, retry, recovered result, Diagnostic, or generation in `tests/unit/inspection/parsers.test.ts` and `tests/unit/inspection/seed-parsers.test.ts`
- [ ] T922 [US2] Add thrown/rejected-operation tests across relationships, provenances, derivations, fallbacks, source occurrences, authored text, parser messages, retained graphs, and file details; verify unchanged propagation through domain layers with no catch, cause classification, retry, recovered value, Diagnostic, result body, or generation, atomic abort and prior-snapshot retention, and only the trigger-owning REST boundary's generic `OperationError` or ownerless-startup top-level behavior in `tests/integration/runtime-failures.test.ts`
- [ ] T923 [US2] Add thrown/rejected-operation tests across `SourceConditionFact`, `ApplicabilityAssessment`, Diagnostic construction/retention, and serialization; verify unchanged propagation through domain layers with no catch, cause classification, retry, recovered Fact/assessment/Diagnostic/recognition/derived result/body/generation, prior-snapshot continuity, no numeric caps, and only the trigger-owning REST boundary's generic `OperationError` or ownerless-startup top-level behavior in `tests/integration/runtime-failures.test.ts`
- [ ] T924 [P] [US2] Add full safety tests for malformed files, links, traversal, cycles, post-read verification, complete byte disposal, `O_NOFOLLOW`, cleanup-only late discard after disable/shutdown/supersession, read-only open flags, zero mutation-capable calls, unchanged content/length/identity/link/mode/mtime/ctime/xattr/ACL observations, and separately recorded OS-only atime residuals in `tests/integration/inspection-safety.test.ts`
- [ ] T925 [P] [US2] Extend zero-activation regression across every Repository family under `--no-open` or post-helper instrumentation. Use and record local fixture roots; instrument product socket/HTTP(S)/DNS/SMB/MCP/URI/image surfaces, separately classify and validate the two exact FR-022 authorized internal loopback classes, and prove discovery/read/parse/display/comparison/relationship processing causes zero child/evaluation/MCP/prohibited direct product-issued outbound request/URI/image/mutation/reference reads across every other surface. Prove explicit UNC/server-share/device inputs cause zero filesystem/DNS/SMB calls, while documenting lexically indistinguishable pre-mounted/mapped network storage as the excluded OS-mediated FR-022 limitation. Capture operational output and require only fixed codes/opaque IDs with zero paths, roots, filenames, inspected content/metadata, authored values, capabilities, bodies, raw errors, exception strings, or Diagnostic arguments in `tests/integration/security/zero-activation.test.ts`
- [ ] T926 [P] [US2] Add complete file-detail and removed-reveal-route API contracts for every readable kind: `utf-8`, `utf-8-bom`, and `utf-8-replaced` expose complete source/ranges/authored literals and comparison eligibility with retained `U+FFFD`, while `binary` is diagnostic-only and forbids them; enforce source-form allowlists, unknown-key retention in readable text, strict/no-store/capability-only envelopes, stale IDs, and no acknowledgement route. Prove a thrown/rejected pre-commit encoding/serialization operation returns only the generic pre-acceptance `OperationError` with null request ID, HTTP `500`, no job/result/generation/success payload or raw cause; a post-commit delivery rejection leaves the commit unchanged with no success payload and never partial; and no analysis/validation/verdict/remediation field appears in `tests/contract/http-api-files.test.ts`
- [ ] T927 [US2] Add Repository-complete browser acceptance for memory-only presentation acknowledgement, complete literal readable detail including `utf-8-replaced` text and comparison, diagnostic-only binary with no authored values, exact metadata/relationships, no masking/reveal/substitution, and zero executable rendering. Verify deterministic returned malformed/binary/boundary outcomes alone may commit contracted-partial after complete traversal; every throw/rejection produces no new result/generation and appears only as a generic REST Operation Error, with explicit-rescan stale marking, while ownerless automatic first-scan rejection reaches the process top level and deterministic first-scan failure retains the generation-0 Source without stale overlay. Also cover purge/scoped cleanup, stale routes, and no analysis/verdict/remediation controls in `tests/e2e/repository-complete-detail.spec.ts` and `tests/e2e/inspection-safety.spec.ts`
...
- [ ] T928 [US3] Add lifecycle regressions for rescan invalidation of selections, request tokens, FileDetail state, Monaco models/workers/subscriptions, late-owner MCP projections, client epochs, and stale IDs; retain acknowledgement across ordinary scoped route/file/Source/generation cleanup, but reset it on document reload and every central full purge, including liveness loss, pre-request Global disable, and observed greater Global epoch/non-null fence. Prove no complete source text, declared authored metadata, relationship target, or comparison request/DOM/editor state exists before acknowledgement across every Repository kind in `tests/integration/session-lifecycle.test.ts`
- [ ] T929 [US3] Add Repository-complete browser acceptance plus the documented comparison target for literal comparison and typed structural differences between two readable current-generation files in the same Repository Source at this checkpoint; assert no semantic ranking, merge, validation, lint, content verdict, policy/remediation, synchronization, conversion, formatting, or fix suggestion; also cover late-owner MCP selection through real owner IDs, runtime-only/dormant rejection, fallback behavior, accessibility, and lifecycle cleanup in `tests/e2e/repository-complete-comparison.spec.ts` and `tests/e2e/comparison.spec.ts`
...
**Independent Test**: Use isolated environment inputs and fake homes to verify zero proposed-path I/O, exact three-tool preview entries, complete environment-supported escaping, unchanged propagation and generic pre-acceptance Operation Error with no partial preview after a thrown/rejected construction, invalid overrides, versioned digest binding, stale/replayed rejection, accessible bilingual review, and one-time non-authorizing ownership of `codex.behavior.user.memories`, `codex.behavior.user.prompts`, and `claude.behavior.user.workflows`.

**Visible Checkpoint**: Users can review the exact Global roots, patterns, exclusions, lexical validity state, and contract version before enabling inspection.

### Fixtures and tests first

- [ ] T930 [US4] Create isolated Global-home fixtures for exact candidates, exclusions, fallback, invalid overrides, links, aliases, unreadable roots, injected Node.js/OS/filesystem throws/rejections, distinct literal credentials/environment references, sentinel process values, inert executable-looking payloads, and before/after content/length/identity/link/mode/mtime/ctime/xattr/ACL observations with OS-only atime recorded separately; document unchanged non-carveout propagation, generic REST Operation Error ownership, no file-size/count validation, and no validity/lint/verdict from availability in bilingual guidance at `tests/fixtures/global-homes/build-fixtures.ts`, `tests/fixtures/global-homes/README.md`, and `tests/fixtures/global-homes/README.ja.md`
...
- [ ] T932 [P] [US4] Add failing preview tests for zero filesystem/network I/O and the complete ordered Global `inputState` algorithm: environment-only empty is `present-empty`; U+0000 or unpaired UTF-16 is `invalid`; active-platform `path.isAbsolute()` false is `relative`; absolute spellings rejected by shared pure `LexicalAbsoluteRootParts` are `invalid`; only parser-accepted absolute spellings are `eligible` and retain exact parsed operands. Include POSIX `/tmp//repo`, dot/dot-dot, trailing-separator, and U+FFFD vectors plus Windows UNC/network/device/current-drive, repeated-separator, dot/dot-dot, trailing-separator, malformed-drive, and accepted plain-drive vectors. Also cover exact lexical roots, complete environment-supported escaping, and the exact minimal three-entry frozen preview. Prove capture-or-replace occurs only through the state-changing POST operation; current-preview retrieval is non-mutating. Any thrown/rejected capture, escaping, digest, or serialization operation propagates unchanged from the consent domain with no catch, cause classification, partial DTO/digest/state mutation, or path authority, leaving REST representation to the router's generic pre-acceptance `OperationError` in `tests/unit/host/global-consent.test.ts`
- [ ] T933 [US4] Extend preview tests for immutable typed traversal-plan programs, ordered session-keyed raw/display digest binding, fixed-format verification inputs, stale/replayed invalidation, and `confirmedTools` fixed exactly to `[copilot, claude, codex]` in closed order—including invalid frozen entries—with no eligibility narrowing or UI/API selector; initial enable evaluates all three, while retry derives every missing Source server-side from that same tuple in `tests/unit/host/global-consent.test.ts`
- [ ] T934 [P] [US4] Add failing contracts for non-mutating `GET /api/v1/global/consent-preview`, which returns only the current frozen preview or `404 consent-preview-missing`, and state-changing `POST /api/v1/global/consent-preview` with exact `{}` body and mandatory exact same-origin `Origin`, which captures and atomically creates/replaces the unconsented preview and returns `201`. Cover no-store, zero proposed-root I/O, active-consent/enable/disable conflicts, and generic null-ID HTTP `500` on a thrown/rejected POST capture/encoding with zero success bytes, job, retention, or state mutation; GET never recaptures environment state. A poisoned process-wide resource registry must make POST return `409 resource-cleanup-restart-required` before environment/default-home capture, digest/encoding, state mutation, job allocation, or I/O, while GET remains pure current-state retrieval in `tests/contract/http-api-global.test.ts`
- [ ] T935 [US4] Add failing browser acceptance for bilingual roots, patterns, lexical states, exclusions, generic Operation Errors with no raw cause, keyboard review, and zero pre-consent source results or enable requests in `tests/e2e/global-consent-preview.spec.ts`
...
- [ ] T938 [US4] Implement POST-owned complete environment/default-home preview capture and the ordered Global `inputState` algorithm through the shared pure `LexicalAbsoluteRootParts` parser, carrying exact parsed operands only for `eligible` roots and performing zero filesystem/network I/O. Atomically create-or-replace the preview and apply presentation escaping with no normalization or root creation; expose the frozen record to GET as a pure current-state retrieval. On any thrown/rejected capture, classification, escaping, digest, or serialization operation, propagate unchanged out of the consent domain with no catch, cause classification, partial DTO/digest, stored mutation, or path authority in `src/host/global-consent.ts`
...
- [ ] T940 [US4] Implement strict paired preview handlers in `src/host/api-router.ts`: non-mutating `GET /api/v1/global/consent-preview` returns only the current frozen preview or `404`, while `POST /api/v1/global/consent-preview` requires exact `{}` plus same-origin `Origin` and alone captures/atomically creates or replaces an unconsented preview with `201`. Before POST capture or scheduling, reject a poisoned process-wide registry with `409 resource-cleanup-restart-required` and zero environment/default-home capture, digest/encoding, job/state mutation, or I/O; GET remains pure current-state retrieval. Preserve exact no-store/conflict behavior; catch a thrown/rejected POST capture/encoding only at this REST boundary and return the closed null-ID HTTP `500` `OperationError` with no success bytes, job, retention, raw cause, state mutation, or path authority; GET performs no environment recapture
- [ ] T941 [US4] Implement accessible preview presentation for exact roots, patterns, lexical states, exclusions, version, and generic Operation Errors without raw cause or Inspector-defined capacity fields/values in `app/components/consent/GlobalConsentPreview.vue`
...
- [ ] T943 [US4] Add semantically equivalent English/Japanese Global preview, generic Operation Error, invalid override, digest, and consent messages with no raw cause or path/value leakage in `app/locales/en.ts` and `app/locales/ja.ts`

---

## Phase 96: Fixed-Three Global Enable Foundation and Codex Batch Member

**Purpose**: Validate the exact stored preview through the selector-free enable endpoint, establish fixed-three controls and one serialized `GlobalBatchScan`, and add the Codex root/instruction branch as one possible member without publishing any new/provisional Source for an admitted missing-Source member before the single batch commit; carried existing Sources remain visible.

**Independent Test**: Submit the exact preview-bound body with no tool selector, evaluate `[copilot, claude, codex]`, reject false/stale/mismatched or extra-key requests, and verify zero admitted roots produce `active-no-job`, while one to three admitted roots transfer together into exactly one `GlobalBatchScan` with one shared `scanRequestId`, one working set, and no new/provisional public Source for those members before one complete or contracted-partial atomic generation; exercise the Codex member, accepted/rejected partition, disable race, retry state with carried existing Sources still visible, Repository preservation, and exact `codex.excluded.user-runtime` ownership.

**Visible Checkpoint**: Global controls show the fixed confirmed tuple and shared pending/retryable batch state; no new/provisional Codex or sibling Global Source appears until the batch commits, while carried existing Sources remain visible.

### Tests first

- [ ] T944 [P] [US4] Add failing Codex post-consent tests for canonical component identity, links/aliases, invalid overrides, and exhaustive first-non-empty traces: a safely read non-binary decoded override with `trim().length > 0`—including retained `U+FFFD`—short-circuits; safely proven empty/BOM-only/whitespace-only advances; only exact Node `ENOENT` from the contract-declared pre-observation structural target `lstat` becomes `absent` and advances; post-observation structural-`lstat` `ENOENT` becomes `entry-disappeared` with no fallback; binary and every deterministic unsafe/type/boundary outcome stop with their Diagnostic; and every other throw/rejection, including `ENOENT` from `open`/`read`, propagates unchanged without fallback, aborts the whole fixed-three transaction, and produces no context/candidate/plan/authority/batch/result. If the safely consumed empty override and later admitted fallback share the same usable physical identity, require zero fallback opens/reads, no byte/provenance merge or reuse, an unpublished empty override, and only the diagnostic-only contracted-partial fallback `safe-fs-ordered-fallback-alias-rejected` in `tests/unit/host/global-consent.test.ts`
- [ ] T945 [P] [US4] Add failing `POST /api/v1/global/enable` contracts for `confirmed: true`, exact version/preview/digest binding, no tool selector, extra-key and false/stale/mismatch rejection, fixed `confirmedTools: [copilot, claude, codex]`, server-derived initial-all or retry-missing set, exact accepted/rejected partition, and zero-admitted `active-no-job` with null ID and no new job/Source/generation—initial enable has no Global Source, while retry preserves all existing Sources/controls and the prior snapshot. Cover one-to-three-admitted `queued` with exactly one shared `scanRequestId` and one unpublished `GlobalBatchScan`. Require a poisoned process-wide registry to return `409 resource-cleanup-restart-required` before admission/scheduling with zero ID/job/control/Source/generation/state mutation or I/O for both initial enable and retry. Require generic pre-acceptance `OperationError` with null ID on any non-carveout throw/rejection: initial enable activates no consent/control/job, while retry leaves existing consent/control, Sources, and prior snapshot unchanged; require one retained terminal error with the shared non-null ID/no subset commit after `202` in `tests/contract/http-api-global.test.ts`
- [ ] T946 [P] [US4] Add failing initial-enable/retry coordinator tests for atomic fixed-three control activation and admission partition, operation ID/epoch checks, FIFO generation selection, conflicts, zero public provisional Source, zero-admitted `active-no-job` with no new job/Source/generation and preserved retry state, and exactly one `GlobalBatchScan` with one request ID/publication authority/working set for every nonempty admitted subset. Prove a non-carveout throw/rejection before acceptance activates no initial consent/control/job, but on retry leaves existing consent/control, Sources, and prior snapshot unchanged; after `202` it terminates the shared batch with one generic `OperationError`. Either lifecycle commits no sibling subset/generation, preserves the prior snapshot without initial/retry stale overlays, and discards late work after disable/shutdown/supersession, with one `202` versus `409 global-disable-pending` linearization in `tests/unit/session/coordinator.test.ts`
- [ ] T947 [P] [US4] Add boundary tests for the Codex member of the fixed-three transaction: exact raw segments remain filesystem operands, public NFC paths/collisions/aliases follow T019/T030, only the exact Codex Global instruction branch is touched, fallback obeys T944, excluded surfaces receive zero operations, no new/provisional public Source for an admitted missing-Source member appears before the one batch commit while carried existing Sources remain visible, any non-carveout throw/rejection aborts every sibling, and Repository state is preserved in `tests/integration/global-boundaries.test.ts`
...
- [ ] T950 [US4] Add browser acceptance for exact-preview submission with no tool selector, fixed `[copilot, claude, codex]` confirmation, per-tool accepted/rejected controls tied to one shared batch/request ID, `active-no-job`, generic batch Operation Error, safe deterministic boundary Diagnostics, zero new/provisional pre-commit Global Source/file rows for admitted missing-Source members while carried existing Sources remain visible, and retained Repository results in `tests/e2e/global-codex-admission.spec.ts`

### Implementation

- [ ] T951 [US4] Implement Codex root admission as one member of the fixed-three operation: retain raw root provenance, reject deterministic link/component-alias/type/boundary outcomes, catch only exact structural-`lstat` `ENOENT` for its contracted value, propagate every other throw/rejection unchanged, and transfer an admitted unpublished context/IDs to its `GlobalToolControl` only in the atomic all-tools decision that feeds the single `GlobalBatchScan` in `src/host/global-consent.ts`
...
- [ ] T956 [US4] Implement the compiled `codex-global-first-non-empty` traversal plan with exact `utf-8`/`utf-8-bom`/`utf-8-replaced` emptiness semantics, the sole pre-observation structural-`lstat` exact-`ENOENT` absence fallback, no fallback for post-observation disappearance, binary, deterministic unsafe outcomes, or any other throw/rejection, at most one file, exact `codex.excluded.user-runtime`, and unchanged propagation of every non-carveout throw/rejection to abort the whole batch in `src/inspection/rules/codex.ts`
- [ ] T957 [US4] Implement Codex scanning only as one member of the single `GlobalBatchScan`, with one admitted root, exact fallback, raw/NFC/alias semantics, deterministic Diagnostics, and no new/provisional public Source or graph for admitted missing-Source members until every committable result is published together in the batch's one atomic generation; leave carried existing Sources visible and propagate a member throw/rejection unchanged to abort the whole batch in `src/inspection/scan.ts`
- [ ] T958 [US4] Implement selector-free initial enable and exact-consent retry from fixed `[copilot, claude, codex]`: initial evaluates all three, retry derives every missing Source server-side, and one atomic decision activates fixed controls plus accepted/rejected outcomes. If none is admitted, return `active-no-job` with null ID and no new job/Source/generation—initial enable has no Global Source, while retry preserves all existing Sources/controls and the prior snapshot; otherwise transfer every context/ID together into exactly one `GlobalBatchScan` with one shared request ID, authority, and working set. Before REST acceptance, any non-carveout throw/rejection leaves initial state unactivated or, on retry, leaves existing consent/control, Sources, and prior snapshot unchanged, and is represented only by a generic null-ID Operation Error; after `202`, it terminates the shared request with one retained error and no subset Source/generation/stale overlay; preserve the prior snapshot and discard revoked/late work in `src/session/session.ts` and `src/session/scan-generation.ts`
- [ ] T959 [US4] Implement `POST /api/v1/global/enable` with a poisoned-registry pre-schedule gate that returns `409 resource-cleanup-restart-required` with zero ID/job/control/Source/generation/state mutation or I/O for initial enable and retry; strict selector-free guards; stored-preview validation; constant-time digest comparison; fixed-three confirmation; server-derived retry set; accepted/rejected partition; one shared request ID for a queued batch or null for `active-no-job`; retry/disable conflicts; no Source summaries or client path authority; and the exact generic pre-acceptance/accepted-job `OperationError` lifecycle with no raw cause or partial subset in `src/host/api-router.ts`
- [ ] T960 [US4] Wire the single explicit fixed-three confirmation control directly to the selector-free enable endpoint, never offer a per-tool selector, and handle stale preview, accepted/rejected partition, one shared batch state, `active-no-job`, generic Operation Error, and accessible focus in `app/pages/global-consent.vue`
- [ ] T961 [US4] Implement fixed-three confirmed controls whose pending entries all reference the one validation/admission/batch operation and shared request ID, retryable entries are server-derived missing Sources only after pending empties, and `active-no-job` for an evaluated missing tool or any unpublished member never implies a new Source or hides a carried existing Source in `app/components/consent/GlobalSourceControls.vue`
- [ ] T962 [US4] Add semantically equivalent English/Japanese messages for fixed-three Global admission, single-batch/request, accepted/rejected outcomes, `active-no-job`, retryable boundary/fallback, and generic Operation Error, explicitly distinguishing no new/provisional pre-commit Source from carried existing Sources that remain visible, in `app/locales/en.ts` and `app/locales/ja.ts`

---

## Phase 97: Claude Global Batch Member
...
- [ ] T963 [P] [US4] Add failing Claude post-consent boundary tests for canonical roots, raw/NFC/alias identity, links, invalid overrides, and missing/unreadable files inside the fixed-three operation; only exact structural-root-`lstat` `ENOENT` or deterministic lexical/link/type/boundary returned values reject Claude while siblings remain eligible, whereas every other throw/rejection propagates unchanged and aborts the whole batch with no subset Source/generation in `tests/unit/host/global-consent.test.ts`
- [ ] T964 [P] [US4] Add boundary tests for only Claude Global `CLAUDE.md`, zero operations on neighboring User/runtime surfaces, a distinct Claude control/context but no independent job, zero new/provisional pre-commit Source for the admitted missing-Source member while carried existing Sources remain visible, exact raw/NFC/alias behavior, one shared batch request/working set, atomic all-member publication, whole-batch throw/rejection abort, and Repository/prior-Source preservation in `tests/integration/global-boundaries.test.ts`
...
- [ ] T967 [US4] Add browser acceptance for Claude's confirmed/pending/retryable outcome within the fixed-three controls, shared batch request/progress, deterministic per-tool Diagnostics, generic whole-batch Operation Error, zero new/provisional pre-commit Claude Source/file rows while carried existing Sources remain visible, atomic sibling publication, and retained Repository results in `tests/e2e/global-claude-admission.spec.ts`

### Implementation

- [ ] T968 [US4] Implement Claude root admission as one fixed-three member: retain raw provenance, reject deterministic link/component-alias/type/boundary outcomes, catch only exact structural-`lstat` `ENOENT` for its contracted value, propagate every other throw/rejection unchanged, and transfer an admitted unpublished context/IDs to the Claude control only in the atomic decision feeding the one `GlobalBatchScan` in `src/host/global-consent.ts`
...
- [ ] T974 [US4] Implement Claude scanning as one member of the single `GlobalBatchScan`, with exactly one root, raw/NFC/alias semantics, deterministic member Diagnostics, and zero new/provisional member-Source publication until all admitted members' committable results publish together in one generation while carried existing Sources remain visible; propagate any member throw/rejection unchanged to abort the whole batch in `src/inspection/scan.ts`
- [ ] T975 [US4] Implement Claude control/context outcome and retry state as projections of the one serialized fixed-three admission/batch operation, not an independent job; share its request/progress with admitted siblings, preserve prior state until the one atomic commit, and on any non-carveout throw/rejection publish one accepted-job Operation Error for the shared ID with no new item/Source/result/generation or initial/retry stale overlay, then discard late work in `src/session/session.ts`
- [ ] T976 [US4] Add semantically equivalent English/Japanese messages for Claude Global admission, exact exclusion, shared-batch progress, deterministic rejection/retry, and whole-batch Operation Error, explicitly distinguishing no new/provisional pre-commit Source from carried existing Sources that remain visible, in `app/locales/en.ts` and `app/locales/ja.ts`

---

## Phase 98: Copilot Global Batch Member
...
- [ ] T977 [P] [US4] Add failing Copilot post-consent boundary tests for absent/default versus invalid overrides, canonical roots, raw/NFC/alias identity, links, and missing/unreadable files inside the fixed-three operation; only exact structural-root-`lstat` `ENOENT` or deterministic lexical/link/type/boundary returned values reject Copilot while siblings remain eligible, whereas every other throw/rejection propagates unchanged and aborts the whole batch with no subset Source/generation in `tests/unit/host/global-consent.test.ts`
...
- [ ] T981 [US4] Add browser acceptance for Copilot's confirmed/pending/retryable outcome within the fixed-three controls, shared batch request/progress, deterministic invalid-override Diagnostics, generic whole-batch Operation Error, zero new/provisional pre-commit Copilot Source/file rows while carried existing Sources remain visible, atomic sibling publication, and retained Repository results in `tests/e2e/global-copilot-admission.spec.ts`

### Implementation

- [ ] T982 [US4] Implement Copilot root admission as one fixed-three member: distinguish absent/default from invalid preview, retain raw provenance, reject deterministic link/component-alias/type/boundary outcomes, catch only exact structural-`lstat` `ENOENT` for its contracted value, propagate every other throw/rejection unchanged, and transfer an admitted unpublished context/IDs to the Copilot control only in the atomic decision feeding the one `GlobalBatchScan` in `src/host/global-consent.ts`
...
- [ ] T990 [US4] Add semantically equivalent English/Japanese messages for Copilot Global override, admission, exact exclusions, shared-batch progress, deterministic rejection/retry, and whole-batch Operation Error, explicitly distinguishing no new/provisional pre-commit Source from carried existing Sources that remain visible, in `app/locales/en.ts` and `app/locales/ja.ts`

---

## Phase 99: Atomic Global Batch Result Integration

**Purpose**: Integrate zero to three separately identified tool-specific Global Sources, each with one tool/root, through exactly one initial/retry `GlobalBatchScan` commit without merging roots or exposing a per-tool commit.

**Independent Test**: Evaluate the fixed tuple, deterministically admit zero to three roots, and verify an empty subset creates no new job/generation while preserving carried Sources/controls and the prior snapshot, while a nonempty subset uses one request, working set, result, and generation to publish every admitted separately identified Source together; verify stable Source IDs, graph rekeying, exact 57-rule total, deterministic accepted/rejected partitioning, whole-batch abort on a non-carveout throw/rejection, detail/comparison reuse, excluded-surface isolation, and retryable missing-Source controls.
...
- [ ] T991 [P] [US4] Add integrated boundary tests for the exact three vendor instruction sets and one fixed-three transaction: zero admitted roots allocate no new `scanRequestId`/job/Source/generation and preserve all carried Sources/controls and the prior snapshot; one to three admitted roots yield separately identified one-tool/one-root Sources together in exactly one complete or contracted-partial generation with one shared request ID and no observable per-tool commit. Derive each escaped boundary one way from its retained raw context; preserve raw filesystem operands, collision-free NFC primary/alias semantics and distinct provenance within each Source; never reverse preview/display labels into authority; perform zero excluded-surface reads; abort the entire subset on every non-carveout throw/rejection; and preserve Repository/prior Sources in `tests/integration/global-boundaries.test.ts`
- [ ] T992 [US4] Add final Global registry contracts proving exactly 57 rule IDs—the 49-ID pre-Global gate plus three vendor `*.excluded.user-runtime` records, `shared.excluded.managed-remote-state`, and four Global static read-authorizing rules—exact exclusion ownership, reciprocity, zero contained Hook/MCP candidate additions, and existing-source evidence backlinks in `tests/contract/vendor-behaviors.test.ts`, `tests/contract/inspection-rules.test.ts`, `tests/contract/runtime-composition.test.ts`, and `tests/contract/official-sources.test.ts`
- [ ] T993 [P] [US4] Add coordinator tests for one atomic batch publication of every admitted tool-specific Source; one shared request ID, authority, working set, result, and exactly one generation; stable `Source.sourceId`; one-root/tool invariants; one-way boundaries from retained raw contexts; carried-Source semantic preservation with generation-owned ID rekeying; deterministic contracted-partial member outcomes; no per-tool commit; whole-batch throw/rejection failure with one generic Operation Error and no subset/stale overlay; missing-Source retry state; progress; and conflicts in `tests/unit/session/coordinator.test.ts`
- [ ] T994 [P] [US4] Add lifecycle tests proving one successful initial/retry batch advances the session generation exactly once, publishes all admitted Sources together, preserves process-lifetime Source IDs and carried semantic inventory/authored content, rekeys every generation-owned file/recognition/provenance/relationship/Diagnostic ID, invalidates stale FileDetail/comparison/Monaco state, and leaks no provisional context/pending admission; prove no poll can observe an intermediate per-tool commit in `tests/integration/session-lifecycle.test.ts`
- [ ] T995 [P] [US4] Add failing exact-display API/integration tests for Global literal credentials, environment references, process sentinels, inert executable-looking payloads, binary and `utf-8-replaced` files, injected throws/rejections, and mutation observations; prove readable text is exact with no substitution, binary is diagnostic-only, and every non-carveout throw/rejection propagates without domain classification/retry/result, aborts the whole shared batch, commits no subset/generation, and appears only as the generic pre- or accepted-job Operation Error with no raw cause or initial/retry stale overlay, while prior state and filesystem observations remain intact in `tests/contract/http-api-files.test.ts` and `tests/integration/global-literal-display.test.ts`
- [ ] T996 [P] [US4] Add failing zero-activation security tests using recorded local Global fixture roots and instrumented product socket/HTTP(S)/DNS/SMB/MCP/URI/image surfaces. Separately classify and validate the two exact FR-022 authorized internal loopback classes; prove zero dynamic evaluation, command/hook execution, browser-helper launch, prohibited direct product-issued outbound or MCP request across every other surface, environment substitution, or mutation-capable filesystem call; prove explicit UNC/server-share/device roots cause zero filesystem/DNS/SMB calls and retain the documented OS-mediated mounted/mapped-storage limitation. Require path/content-free fixed-code/opaque-ID operational output, and prove Source Condition Facts cause zero local/hosted I/O and no file/relationship/comparison identity in `tests/security/global-zero-activation.test.ts` and `tests/integration/source-condition-facts.test.ts`
- [ ] T997 [US4] Add browser acceptance for selector-free fixed-three enablement, `active-no-job`, and one shared batch request; verify separately identified admitted Sources appear simultaneously after one generation, with escaped inert boundaries distinct from preview and Source-relative paths; filters, Diagnostics, exact readable literals including replacement characters, diagnostic-only binary, no activation/substitution/analysis/verdict, Fact isolation, detail reuse and cross-Source comparison. A non-carveout batch throw/rejection must show only one generic Operation Error, publish no subset/generation or `StaleSourceFailure`, and preserve prior Repository/Global state and stable Source IDs in `tests/e2e/global-enable.spec.ts`

### Implementation

- [ ] T998 [US4] Finalize fixed-three post-consent admission as three one-root `GlobalToolControl` records and operation-local contexts, with initial evaluation of all tools, server-derived missing-Source retry, deterministic rejected partition, and one atomic transfer of every admitted context/ID into exactly one `GlobalBatchScan`; create no independent per-tool job or new/provisional pre-commit Source, keep carried existing Sources visible, and propagate any non-carveout throw/rejection before transfer in `src/host/global-consent.ts`
- [ ] T999 [US4] Finalize all Global behavior, exactly four Global static candidate rules, the existing exact exclusions, strategy references, 48 source backlinks, and the exact 57-rule total in `shared/registries/vendor-behaviors.ts`, `shared/registries/inspection-rules.ts`, `shared/registries/runtime-composition.ts`, and `shared/registries/official-sources.ts`
- [ ] T1000 [US4] Implement one integrated `GlobalBatchScan` that consumes every admitted tool/root member through one request, publication authority, and working set; keep each member's selectors/root/Source identity isolated, apply exact Codex fallback and raw/NFC/alias rules, assemble deterministic complete or contracted-partial member results, publish no new/provisional member result or Source until the coordinator accepts the whole batch, and leave carried existing Sources visible; let any non-carveout member throw/rejection propagate unchanged and abandon every sibling in `src/inspection/scan.ts`
- [ ] T1001 [US4] Atomically publish every admitted tool-specific Global Source together only when the single batch has a committable complete or contracted-partial result after complete traversal. Construct each boundary one way from its admitted raw context, retain internal authority outside DTOs/logs, carry Repository and prior Global semantic content unchanged, preserve all Source IDs, advance the session generation exactly once, rekey all generation-owned IDs, invalidate old detail/comparison/editor state, and clear only participating deterministic failures. A zero-admitted operation allocates no new `scanRequestId`/job/Source/generation and preserves all carried Sources/controls and the prior snapshot; any non-carveout throw/rejection or other noncommittable batch outcome commits no subset/generation, retains prior graphs and retry controls, and for initial/retry creates no `StaleSourceFailure` in `src/session/session.ts` and `src/session/scan-generation.ts`
- [ ] T1002 [US4] Complete `POST /api/v1/global/enable` responses for exact fixed-three accepted/rejected partitions, one shared request ID plus `queued` for a nonempty batch or null plus `active-no-job` for an empty subset, conflicts, retry state, and generic pre-/post-acceptance Operation Errors, while leaving every admitted-member Source publication to a later session poll after the one atomic batch commit and preserving carried existing Sources in `src/host/api-router.ts`
- [ ] T1003 [US4] Implement Repository plus separately identified Codex/Claude/Copilot Global Source and tool filters; one-root summaries that render each enabled Source's escaped, inert `SourceBoundary.displayRoot`/`origin` distinctly from consent-preview display and Source-relative item paths and never as a locator; shared detail navigation; and cross-Source-capable comparison navigation after a Global commit in `app/composables/filters.ts`, `app/composables/session.ts`, and `app/pages/index.vue`
- [ ] T1004 [US4] Complete fixed-three confirmation and per-tool outcome/retry controls tied to one shared batch request/progress, focus recovery, `active-no-job`, generic whole-batch Operation Error, and simultaneous separate-Source outcome presentation in `app/pages/global-consent.vue` and `app/components/consent/GlobalSourceControls.vue`; reuse the T071 pause/resume and on-demand-refresh contract for automatically updating status
- [ ] T1005 [US4] Add semantically equivalent English/Japanese fixed-three/single-batch, one-root separate Source, accepted/rejected, `active-no-job`, whole-batch failure, retry with carried existing Sources kept visible, source/tool-filter, detail/comparison, and shared-progress messages in `app/locales/en.ts` and `app/locales/ja.ts`
...
**Independent Test**: Queue Repository and Global work, trigger contracted-partial and fatal Global attempts, and verify dequeue-time generations, process-lifetime-stable Repository and Global `Source.sourceId` values, rekeying only of generation-owned graph IDs, atomic publication under environment-owned capacity, duplicate conflicts, retained consent/boundaries/prior graph, and successful explicit retry.

**Visible Checkpoint**: Users can rescan Global results and recover from a failed attempt without re-consenting.

### Tests first

- [ ] T1006 [US4] Add failing coordinator tests for serialized cross-source FIFO processing, dequeue-time generations, one `scanRequestId` across admission/progress/final status/commit, duplicate conflicts, fatal retention, and per-job counters; require every non-carveout throw/rejection to propagate without domain catch or cause classification, abort with no item/recognition/derived result/Diagnostic/result body/generation, retain the prior snapshot, and terminate an accepted explicit rescan only through the generic Operation Error for that same request ID in `tests/unit/session/coordinator.test.ts`
- [ ] T1007 [US4] Extend coordinator tests for carried-Source graph construction, lifecycle/control state, serialized state transitions, and one shared close attempt per process-wide registry resource record, including registry-confirmed close, `close-unknown`, poison, late FileHandle confirmation, and directory restart semantics. Also cover publication-authority revocation after disable/shutdown/supersession with pending filesystem work treated as cleanup-only, late discard, no later source I/O, responsive API, and no hard-cancellation assertion in `tests/unit/session/coordinator.test.ts`
- [ ] T1008 [P] [US4] Add failing `POST /api/v1/global/rescan` contracts for strict `sourceId`, `ScanAdmission { scanRequestId, source }`, same-ID waiting/active/final status and successful generation, one identified Global Source, unknown/removed Source, disable-pending/duplicate conflicts, and rejection of older state as completion. A poisoned process-wide registry must return `409 resource-cleanup-restart-required` before admission/scheduling with zero ID/job/state mutation or I/O and preserve all prior Source/control/stale state. A pre-acceptance throw/rejection must return the generic null-ID Operation Error with no job; an accepted rescan throw/rejection must expose only the retained generic error with that request ID, no attempt result/generation/raw cause, the stale prior snapshot, and a Source stale reference; cover retry and stale IDs in `tests/contract/http-api-global.test.ts`
- [ ] T1009 [P] [US4] Add concurrency tests for enable completion, queued Repository/Global scans, contracted-partial publication, fatal retention, explicit retry, and unchanged consent/boundaries in `tests/integration/global-concurrency.test.ts`
- [ ] T1010 [P] [US4] Add lifecycle tests distinguishing a successful initial/retry batch—which publishes every admitted Source together and advances exactly one generation—from a later successful explicit single-Source Global rescan—which replaces only that Source while carrying all others. Both preserve every Source ID and carried semantic inventory/authored content, rekey all generation-owned graph IDs, and invalidate old FileDetail/comparison/Monaco state; only the explicit rescan clears its Source stale reference, while an all-rejected enable/retry commits no generation or ID change in `tests/integration/session-lifecycle.test.ts`
- [ ] T1011 [US4] Add browser acceptance for Global rescan, waiting/active progress, duplicate prevention, contracted-partial diagnostics, fatal retry, and retained prior results in `tests/e2e/global-rescan.spec.ts`

### Implementation

- [ ] T1012 [US4] Implement FIFO rescan for one identified tool-specific Global Source, complete or contracted-partial session-wide commits that preserve all Source IDs, regenerate carried and replaced generation-owned graph IDs, clear only the rescanned Source's stale failure, preserve sibling failures, and invalidate old FileDetail/comparison state in `src/session/session.ts`, `src/session/stale-failures.ts`, and `src/session/scan-generation.ts`
- [ ] T1013 [US4] Implement serialized carried-source generation construction and per-job counters with environment-owned capacity; except for the exact structural-`lstat` `ENOENT` conversion, let every throw/rejection propagate unchanged through session/scan domain code without cause classification, retry, Diagnostic, item/recognition/derived result/body/generation, retain the prior snapshot, and leave lifecycle conversion to the trigger-owning boundary; revoke publication authority on disable/shutdown/supersession and discard/release late work once in `src/session/session.ts` and `src/session/scan-generation.ts`
- [ ] T1014 [US4] Implement strict `POST /api/v1/global/rescan` for one opaque `sourceId`; gate a poisoned process-wide registry before admission/scheduling with `409 resource-cleanup-restart-required`, zero ID/job/state mutation or I/O, and complete prior Source/control/stale preservation. Otherwise return admission and preserve its request ID through progress/final status/commit; enforce disable/duplicate conflicts and catch non-carveout throws/rejections only at this REST boundary, returning a generic null-ID pre-acceptance Operation Error with no job or retaining the same-ID accepted-job error with no attempt result/generation/raw cause and a stale prior snapshot; preserve retry/stale-Source responses and path-free operational output in `src/host/api-router.ts`
- [ ] T1015 [US4] Implement Global rescan loading, duplicate suppression, stale recovery, fatal retry, and progress refresh in `app/components/consent/GlobalSourceControls.vue` and `app/composables/session.ts`
- [ ] T1016 [US4] Add semantically equivalent English/Japanese Global rescan, queue, public `partial` (contracted-partial only), failure-retention, and retry messages in `app/locales/en.ts` and `app/locales/ja.ts`
...
**Independent Test**: Purge the browser before requesting disable, disable during Repository/enable/Global work, and repeat/join/retry after failures. Verify epoch/fence response gates, control-only recovery, generic errors and restart when closure is unconfirmed, registry-owned cleanup, N+1 Repository-only `remove-active-state`, generation-preserving `cleanup-only` only for unpublished initial enable, and immediate fresh-snapshot recovery after pre-acceptance failure or true no-op.
...
- [ ] T1017 [US4] Add failing coordinator tests for first non-no-op disable acceptance atomically choosing and retaining `commitKind`, incrementing `globalContentEpoch`, installing non-null `globalDisableInProgress`, revoking authority, and fencing data. Choose `remove-active-state` whenever public Global consent/control/Source exists and commit exactly Repository-only N+1 with rekeyed generation IDs; choose `cleanup-only` only for unpublished operation-local initial enable and preserve N/all IDs. Prove a true no-op requires the complete absence of tool-specific Global Source/graph, active consent, retained admitted root context, affected `FileHandle`/`fs.Dir` registry records in `opening`/`open`/`closing`/`close-unknown`, running/queued Global scan/enable work, and retained disable failure while the registry is not poisoned; unrelated Repository work does not prevent it. Cover joined requests, retained failed barrier/retry lineage, generic `OperationError`, exact resource references, Repository requeue-once only after success, and no rollback/rebase in `tests/unit/session/coordinator.test.ts`
- [ ] T1018 [P] [US4] Add failing HTTP contracts for strict empty-body `POST /api/v1/global/disable`; the full true-no-op predicate and mutationless `200` result; unrelated registry poison returning mutationless `409 resource-cleanup-restart-required`; exact liveness `{ sessionId, globalContentEpoch, globalDisableInProgress }` projected from one current lock snapshot and allowed to report a non-null current fence; final response gates that permit an inspection-data success only when its captured epoch is still current and the current fence is null, while liveness success instead returns that same-lock current epoch/current-fence projection; `GlobalFenceRecoverySnapshot` as the session route's sole fenced success; `409 global-disable-pending` on every other inspection-data route and generation mutation; joined/retry/failed responses; generic pathless Operation Error; and exact N versus N+1 terminal buffers in `tests/contract/http-api-global.test.ts` and `tests/contract/http-api-session.test.ts`
- [ ] T1019 [P] [US4] Add concurrency tests for interrupted Repository/enable/Global work, queued cancellation, acceptance-versus-buffer-bound races, joined disable, retained failure and retry with the same cleanup lineage/epoch/commit kind, requeue-once after terminal success, true no-op, and pre-acceptance failure. Prove the fence never reopens on post-acceptance failure and no stale captured-epoch response publishes in `tests/integration/global-concurrency.test.ts`
- [ ] T1020 [P] [US4] Add boundary instrumentation proving disable performs zero enumeration/read, emits zero expected-cancellation Diagnostics, adopts every affected FileHandle/`fs.Dir` record from the process-wide `ClosableResourceRegistry`, joins one close per resource, and cannot complete until every record is `close-confirmed`; cover FileHandle event/promise races, `close-unknown`, late-event recovery, directory restart requirement, and no guessed closure in `tests/integration/global-boundaries.test.ts`
- [ ] T1021 [P] [US4] Add lifecycle tests proving the browser performs a full purge before sending disable and again before rendering any greater epoch/non-null fence, removing every session/inventory/Source/file/Diagnostic/relationship/authored/detail/comparison/Monaco/filter/warning/acknowledgement value. While fenced expose only liveness plus `GlobalFenceRecoverySnapshot`; after `remove-active-state` fetch fresh Repository-only N+1, after `cleanup-only` fetch unchanged N, and after pre-acceptance failure/no-op immediately refetch a full snapshot. Prove late results never publish and resources release only through confirmed registry cleanup in `tests/integration/session-lifecycle.test.ts`
- [ ] T1022 [US4] Add browser acceptance plus the documented Global-consent target for preview/enable/rescan/disable, pre-request purge, epoch/fence observation purge, exact control-only failed/retry/join/restart recovery, no purged-content restoration, enable/disable races, focus restoration, and fresh terminal snapshots. Cover both public-state N+1 `remove-active-state` and unpublished-initial-enable N `cleanup-only`, plus pre-acceptance/no-op immediate recovery in `tests/e2e/global-disable.spec.ts` and `tests/e2e/global-consent.spec.ts`

### Implementation

- [ ] T1023 [US4] Implement the serialized priority zero-I/O barrier: on first non-no-op acceptance fix `remove-active-state` versus `cleanup-only`, increment command/content epochs, install the fence, revoke authority, cancel Global work, hold running Repository work for one post-success requeue, and retain the exact operation/error/cleanup lineage across failure, join, and retry. Evaluate the complete true-no-op predicate under coordinator ownership: no tool-specific Global Source/graph, active consent, retained admitted root context, affected `FileHandle`/`fs.Dir` registry record in `opening`/`open`/`closing`/`close-unknown`, running/queued Global scan/enable work, or retained disable failure, and a non-poisoned registry; unrelated Repository work is permitted. A true no-op or pre-acceptance failure performs no I/O, creates no job, and mutates no generation, epoch, or fence; a post-acceptance failure keeps all data fenced in `src/session/session.ts`
- [ ] T1024 [US4] Implement cleanup solely through the process-wide `ClosableResourceRegistry`, including late opening-resource adoption, shared close promises/events, confirmed-close requirement, poison/restart behavior, retry sweep, and no double-close or hard-cancellation claim. After a fully encoded success buffer is ready, atomically commit Repository-only N+1/rekeying/removal for `remove-active-state`, or remove only unpublished operation-local state and preserve N/all IDs for `cleanup-only`, then clear the fence in `src/session/session.ts`, `src/session/stale-failures.ts`, and `src/session/scan-generation.ts`
- [ ] T1025 [US4] Implement strict `POST /api/v1/global/disable` at the REST boundary with empty-body validation, a mutationless `200` only for the complete true-no-op predicate, mutationless `409 resource-cleanup-restart-required` for an otherwise no-op request when an unrelated registry poison exists, pre-acceptance responses, first-acceptance/join/retry progress, generic `OperationError` on failed drain/close/serialization while keeping the process and fence alive, restart guidance for unconfirmed cleanup, exact terminal generation/commit-kind response, and immutable buffer delivery in `src/host/api-router.ts`
- [ ] T1026 [US4] Implement pre-request full client-data purge, disable submission/loading, fenced `GlobalFenceRecoverySnapshot` rendering, failed retry/join/restart controls, no-op/pre-acceptance immediate full refetch, terminal fresh-snapshot adoption, and focus restoration in `app/pages/global-consent.vue`, `app/components/consent/GlobalSourceControls.vue`, and `app/composables/session.ts`
- [ ] T1027 [US4] Implement the shared full-purge and response gate: clear all session/global/Repository DTOs and rendered/derived state including sensitive-content acknowledgement before disable and upon greater epoch/non-null fence; reject every stale/late response; while fenced retain only capability plus exact control/error recovery; after the fence clears fetch the full authoritative snapshot rather than reconstructing purged content in `app/composables/filters.ts`, `app/composables/liveness.ts`, `app/composables/comparison.ts`, and `app/composables/monaco.ts`
- [ ] T1028 [US4] Add semantically equivalent English/Japanese messages for pre-request purge, epoch/fence control-only recovery, failed retry/join/restart, true no-op/pre-acceptance refetch, `remove-active-state` Repository-only N+1, and operation-local `cleanup-only` unchanged N in `app/locales/en.ts` and `app/locales/ja.ts`
...
- [ ] T1029 Draft semantically equivalent operational guidance in `README.md`/`README.ja.md` covering verified launch and exact `--cwd`; generation 0; Node/build/browser baseline; allowlists; raw/NFC/collision/hard-link scope and aliases; rows 1–28, directory mutation guards, process-wide confirmed-close registry and restart limits, including the four poisoned-registry pre-schedule `409 resource-cleanup-restart-required` route gates with zero state/job/I/O; boundaries and record-specific evidence assessments; FR-022's prohibited direct product-issued outbound-request definition, its two exact authorized internal loopback classes and their static-path/method versus API route/Host/Origin/capability constraints, zero-prohibited-request local-fixture assertion, explicit-UNC zero-filesystem/DNS/SMB guarantee, and OS-mediated pre-mounted/mapped-network limitation; readable replacement-decoded text and binary; acknowledgement and capability-only API; no semantic verdicts/capacity ceilings; exact structural-`lstat` `ENOENT` versus unchanged propagation; REST Operation Errors versus startup top-level rejection; fixed-three Global consent/rescan; and FR-042 disable pre-purge, epoch/fence, exact liveness/recovery DTOs, post-acceptance failure/retry/join/restart, public-state N+1 removal versus unpublished-operation N cleanup. Also cover evidence manifests, SC-002, mutation/atime, privacy/exclusions/maintenance, and the bilingual 55-row WCAG contract; require `tests/contract/documentation.test.ts` to reject divergence.
- [ ] T1030 Create semantically equivalent SC-001/SC-006 study kits with the maintainer-owned bilingual plan naming study, recruitment/compensation-funding, moderation/review, support, privacy/retention, equipment/session, and accessibility owners; the fixed twenty-participant order; recording of the actual OS default handler or its unavailability and, when resolvable, the actual browser family/revision; mandatory use and recording of the printed-URL manual fallback in a pinned certified browser when automatic opening is disabled, unsupported, fails, is unidentifiable, unavailable, or resolves outside the certification baseline; exact timers, prepared state, four-field ground truth, no-hints and no-replacement rules, and all four workflow observations. Define study equipment that continuously runs the SC-004 product network/URL/MCP instrumentation, an exact-authority Inspector-server request ledger, and study-browser request capture from Inspector launch before SC-001 through all four observations; correlates process identity, exact issued authority, request initiator/target, and the server ledger; classifies every Inspector/bundled-SPA request into one of the two exact FR-022 authorized internal loopback classes or prohibited traffic; treats attributable but unclassifiable traffic as outside both classes; and separately records unrelated extension/host-process traffic and observable OS-mediated mounted/mapped-source traffic without product misattribution. Make every outside-class and prohibited outbound/MCP request an automatic critical safety event while treating recorded OS-mediated mounted/mapped-source traffic only as the documented limitation. Require two independent fixed-rubric classifications only for suspected non-safety product blockers, with disagreement counted as critical and no third adjudicator; state that ordinary contributors do not operate the study, missing resources block the release claim rather than contribution review, and rerun is required only after a material workflow/guidance/fixture/rubric change in `tests/usability/sc001-sc006-study-kit.md` and `tests/usability/sc001-sc006-study-kit.ja.md`

### Official evidence and dependency review

- [ ] T1031 Add failing official-source checker contracts for exact hosts, redirect rejection, explicit network opt-in, complete environment-supported content retrieval, thrown/rejected network or runtime operations with no partial update or automatic cause-based decision, and non-mutating drift reporting in `tests/contract/official-source-drift.test.ts`
...
- [ ] T1037 Enforce the semantic drift and six-digest freeze gate after T1033–T1036 and before any production-registry mutation or later old task ID. Recompute all six table inputs with the exact T004 extraction algorithm, reject every missing/duplicate/empty/malformed heading/table or absent/mismatched recorded digest, compare equal-length digest bytes in constant time, and separately require exact IDs plus English/Japanese semantic parity. Only reviewed evidence-location, anchor, review-metadata, or semantically unchanged corrections may flow to `shared/registries/vendor-behaviors.ts`, `shared/registries/inspection-rules.ts`, `shared/registries/runtime-composition.ts`, and `shared/registries/official-sources.ts`; any freeze mismatch or accepted change to normative behavior, rule, strategy, allowlist membership/source-form applicability, registry shape, or conformance expectation marks the bilingual task set superseded, stops before mutation, requires synchronized bilingual spec/research/plan/quickstart/contracts/tasks, and requires `/speckit-plan` then `/speckit-tasks`
- [ ] T1038 Regenerate only affected conformance records in `tests/fixtures/conformance/vendor-behaviors.json`, `tests/fixtures/conformance/inspection-rules.json`, `tests/fixtures/conformance/runtime-composition.json`, and `tests/fixtures/conformance/official-sources.json`
...
- [ ] T1041 Freeze and validate the versioned SC-003/004/005/007/009 outcome manifest and add the hard bilingual cross-artifact gate in `tests/contract/documentation.test.ts` for runnable commands/stable IDs, all 56 FR/QR/SC trace rows through FR-042, and every T001–T1063 mapping. Require every task in each language to contain at least one exact repository-relative owned file path, reject basename-only shorthand, and require the per-task English/Japanese owned-path sets to match exactly. In addition to the six Presentation Allowlist digest/ID/parity gate, require the exact 48-source/57-rule registry, `vscode.copilot.mcp.workspace-root-release`, reciprocal `copilot.repo.mcp.vscode-root` conflict evidence, and root path-only semantics with zero inferred VS Code schema fields or winner; exact `--cwd`/generation-0 semantics; pathless collision ownership; safe-fs rows 1–28, explicit directory guard/confirmed close, one-Source-scan-attempt static hard-link prepass/usable nlink identity/read-once scope, independent Source/attempt/generation reads, and the two zero-read alias rejections; separate classification and constraint validation of the two exact FR-022 authorized internal loopback classes, zero prohibited direct product requests on every other surface, local-fixture/explicit-UNC zero-call semantics, and the mounted/mapped OS-mediated limitation; process-wide resource registry close-race/poison/restart semantics plus all four poisoned-registry pre-schedule route gates with `409 resource-cleanup-restart-required` and zero state/job/I/O; replacement decoding; exact runtime-error ownership; fixed-three Global work; FR-042 pre-purge/epoch/fence/recovery/error/N-vs-N+1 semantics, including unchanged-epoch/null-fence final gates for inspection-data success versus same-lock current-epoch/current-fence liveness projection with a non-null fence allowed; record-by-record `EvidenceAssessment` status/qualifier identity; non-authority, no semantic analysis/capacity ceilings, deterministic partial, atomic/late-discard/mutation evidence, dependency/migration evidence, SC-002, manifests, all 55 WCAG rows/manual matrix, and official backlinks. Any failure or concern blocks T1042 and every later old task ID.
- [ ] T1042 [P] After T1041 passes, materialize exactly 48 source records and final tests for exactly 57 inspection-rule IDs, 39 strategies, 14 relationship-only rules, zero contained Hook/MCP additions, and the updated frozen Presentation Allowlist/source bounds. Require the `vscode.copilot.mcp.workspace-root-release` record, reciprocal `copilot.repo.mcp.vscode-root` evidence, current-guide/release-note conflict, and path-only root provenance with zero inferred VS Code schema fields or winner. Implement and verify scalar `documentationStatus` plus fixed-order duplicate-free `lifecycleQualifiers` on each behavior/rule/strategy, and sorted record-specific `EvidenceAssessment[]` on every provenance/Relationship/Fact/recognition DTO with one subject record per owner/reference and no scalar/worst/union reduction, fabricated `stable`, or `documentation-conflict` status alias. Preserve zero-authority Facts and syntactic/literal/typed/catalog/structural-only vocabularies in `tests/fixtures/conformance/official-sources.json`, `tests/contract/vendor-behaviors.test.ts`, `tests/contract/inspection-rules.test.ts`, `tests/contract/runtime-composition.test.ts`, and `tests/contract/official-sources.test.ts`
- [ ] T1043 [P] Add packed-tarball closed-set tests for exact `bin.mjs`/engines/version rejection/README/license/manifest schemas, declared-versus-actual byte lengths, hashes, CLI/Worker entries, complete environment-supported verification, safe failure before import/bind when verification cannot complete, and unlisted-payload rejection without customization validity output in `tests/package/package-contents.test.ts`, `tests/package/static-manifest.test.ts`, and `tests/package/server-manifest.test.ts`
- [ ] T1044 [P] Extend package tests for the exact runtime dependency leaf set including `gunshi` 0.37.0, Gunshi's exact integrity/full bundled payload digest and root-only import boundary, absence of `open`, production-graph name/version/integrity/payload digests, scripts-disabled and verified-cache network-disabled normal-lifecycle installs, separate generated-shim audit, and rejection of Rust/C/C++, Cargo, Node-API/native/binary/Wasm payloads, `binding.gyp`, prebuilds, platform selectors, package-owned shell helpers, non-Node shebangs, lifecycle/runtime downloads, and unlisted data in `tests/package/node-only-policy.test.ts` and `tests/package/production-graph.test.ts`
- [ ] T1045 [P] Add cross-story axe, keyboard, forced-colors, zoom/reflow, reduced-motion, focus-retention, safe-error, authored-value acknowledgement, ordinary scoped-cleanup retention versus every central-full-purge reset, and liveness/Global-disable epoch-fence recovery regressions mapped to the complete bilingual 55-row WCAG Level A/AA acceptance matrix; name/report exact `AUTO-*` IDs, include `AUTO-2.2.2`, and require every Applicable automated check and all four keyboard workflows in pinned Chromium/Firefox/WebKit without severity escapes in `tests/e2e/accessibility.spec.ts` and `tests/e2e/session-liveness.spec.ts`
- [ ] T1046 [P] Add snapshot regressions for Diagnostic, Operation Error, evidence assessments, control/progress, SessionSnapshot, `GlobalFenceRecoverySnapshot`, liveness, and FileDetail envelopes; canonical immutable buffers and exact length; null-/same-ID error ownership; and post-commit delivery behavior. Pause delivery across rescan and disable acceptance to prove every inspection-data success captures `globalContentEpoch` and is published only after the final unchanged-epoch/null-fence gate, whereas every liveness success is projected atomically from one current lock snapshot as the current epoch plus current fence and may therefore carry a non-null fence; prove stale buffers never leak after purge, the session route is recovery-only while fenced, and terminal N/N+1 buffers never mix with prior state in `tests/integration/session-snapshot-encoding.test.ts`, `tests/contract/http-api-session.test.ts`, and `tests/contract/http-api-files.test.ts`
...
- [ ] T1047 Add release jobs that build and verify one platform-independent tarball on the Node.js 24.18.0 `ubuntu-24.04` x64 development/build baseline, distribute identical bytes to the six lower-bound certification samples—Node.js `24.11.0`/`26.0.0` by `ubuntu-24.04` x64/`macos-15` arm64/`windows-2025` x64—record runner-image identifiers and actual Node versions, retain `^24.11.0 || ^26.0.0` as the full compatibility contract, and aggregate production-graph digests with separate shim audits in `.github/workflows/release.yml`
- [ ] T1048 Extend release jobs with exact packed-engines/running-version pre-import rejection, safe-filesystem, recursive two-manifest/hash, scripts-disabled and verified-cache network-disabled install, production graph, `npx`, Node.js-only, package-content, exact Playwright 1.61.1 Chromium/Firefox/WebKit browser-certification, OS-default-handler distinction with `--no-open` manual fallback, liveness, and accessibility gates before publication in `.github/workflows/release.yml`

### Outcome evidence and final gates

- [ ] T1049 Run frozen install, the exact Playwright 1.61.1 Chromium/Firefox/WebKit install, build, formatting, lint, typecheck, unit, contract, and security gates and record every result in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
...
- [ ] T1051 Run the six lower-bound jobs against identical tarball bytes and record engines/runner/digest/shim results plus safe-fs rows 1–28, explicit `Dir.read()` directory-mutation guard and confirmed close, usable bigint dev/ino/nlink hard-link grouping, one-Source-attempt read-once and independent-attempt reads, both zero-read alias rejections, `O_NOFOLLOW`, boundary-unverifiable, process-wide close-race/poison/restart behavior, and non-proving platform-unobservable outcomes in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1052 Validate the checked-in SC-002 profile/fixture, then record exactly ten fresh-process runs: wait for each automatic first Repository scan outside timing, dispatch exactly one explicit rescan, capture its `scanRequestId`, accept only same-ID visible/assistive status and inventory from that request's committed generation, reject generic/loading/prior/automatic state, and require one common subset of at least nine runs to satisfy all four thresholds—status within one second, inventory within ten seconds, filter interaction under 100 ms, and item-selection interaction under 100 ms; record profile/digest/request ID/generation/environment with no snapshot reuse, intentional cache reset, or omissions beyond personal identifiers/absolute paths in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
...
- [ ] T1054 Validate and record SC-004 from the frozen manifest with nonzero coverage for every tool, prohibited effect, rejected selector, detectable file-read change, directory create/remove/rename during enumeration, and close-result class. Record local fixture roots and instrument product socket/HTTP(S)/DNS/SMB/MCP/URI/image surfaces; separately classify and validate the two exact FR-022 authorized internal loopback classes, including their static path/method and API route/Host/Origin/capability constraints, and prove zero prohibited direct product-issued outbound or MCP requests across every other surface. Prove explicit UNC/server-share/device vectors cause zero filesystem/DNS/SMB calls, and record lexically indistinguishable pre-mounted/mapped network storage as the excluded OS-mediated FR-022 limitation. Use an external mutation harness, prove product-side mutation APIs/flags are zero and each consumed group has one production content read, record unchanged content/length/identity/link/mode/mtime/ctime/xattr/ACL with OS-only atime separate, and verify confirmed cleanup/late discard without hard-cancellation claims in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1055 Validate and record the checked-in outcome-fixture manifest version, SHA-256 digest, and exact executed case IDs, then record SC-005 pass/fail against its exact nonzero denominators and minima for every supported `(tool, kind, admitted source form)` row, source/comparison surface, credential/environment-reference class, and set-sentinel/unset state; require zero substitution, no masking/reveal, and unchanged fixtures. Separately prove Diagnostics duplicate no source values; the closed generic Operation Error contains only its six exact fields and no raw cause or source identity/value; and OperationalEvent captures use only fixed codes plus opaque IDs with zero paths/roots/filenames/content/metadata/capabilities/bodies/raw errors/exceptions/Diagnostic arguments in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1056 Execute SC-001 first with exactly twenty non-replaced participants under the maintainer-owned bilingual study plan; before Inspector launch, start the study kit's SC-004 product network/URL/MCP instrumentation, exact-authority Inspector-server request ledger, and study-browser request capture and keep them running through T1057; record the actual OS default handler or its unavailability and, when resolvable, actual browser family/revision; when automatic opening is disabled, unsupported, fails, is unavailable/unidentifiable, or resolves outside the certification baseline, use and record the printed-URL manual fallback in a pinned certified browser; record the exact two-minute boundary and every unsuccessful equipment/environment/product outcome, and require at least nineteen successful launch/open results in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1057 Execute SC-006 after SC-001 with the same exactly twenty participants and prepared file state while continuing T1056's instrumentation through comparison and Global consent; record the actual default handler/browser and use the same documented pinned-certified-browser manual fallback conditions as T1056; enforce the exact two-minute boundary, four-field ground truth, and no replacements, then have every participant attempt comparison and Global consent. Correlate process identity, exact issued authority, request initiator/target, and the Inspector-server ledger; classify every Inspector/bundled-SPA request into one of the two exact FR-022 authorized internal loopback classes or prohibited traffic; treat attributable but unclassifiable traffic as outside both classes; and separately record unrelated extension/host-process traffic and observable OS-mediated mounted/mapped-source traffic without attributing either to the product. Record all workflow outcomes and automatically classify every outside-class and prohibited outbound/MCP request as critical, while treating recorded OS-mediated mounted/mapped-source traffic only as the documented limitation. Use two independent fixed-rubric classifications only for suspected non-safety product blockers, count disagreement as critical without a third adjudicator, and require at least eighteen successful identifications plus all twenty complete attempts with zero critical issues in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1058 Validate and record SC-007 from the frozen manifest for every deterministic class and every runtime propagation class, preserving readable complete `utf-8-replaced`, generic REST Operation Errors, startup top-level propagation, prior snapshots, explicit-rescan stale ownership, and poisoned-registry pre-schedule `409 resource-cleanup-restart-required` with zero state/job/I/O for Repository rescan, consent-preview POST capture, Global enable/retry, and Global rescan. Also cover Global-disable pre-acceptance/no-op immediate full-snapshot recovery; post-acceptance drain/close/serialization failure with process liveness, retained epoch/fence/error and retry/join/restart controls; no purged-content restoration; and terminal public-state N+1 versus unpublished-initial-enable N cleanup in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1059 Execute and record the SC-008 protocol from the bilingual accessibility acceptance contract in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`: evaluate all 55 WCAG 2.2 Level A/AA rows against the frozen 38 Applicable/17 Not-applicable split, revalidate every criterion-specific Not-applicable rationale, record every required stable `AUTO-*`/`MANUAL-*`/`REVIEW-*` ID and pass every Applicable check/evidence item, and record all 2,160 keyed locale/platform/viewport/mode/scenario/input cells per `MANUAL-*` ID with explicit evidence or permitted cell-specific N/A rationale; any missing cell, unstable or missing ID, failed or missing Applicable check, rationale, mapping, evidence, result, responsive variation, or any of the four keyboard-only primary workflows fails SC-008 regardless of severity
- [ ] T1060 Validate and record the checked-in outcome-fixture manifest version, SHA-256 digest, and exact executed case IDs, then record SC-009 pass/fail against its exact nonzero denominators and declared minimum coverage for every documented initial-release Source Condition Fact row, supported tool, product surface, and documented-condition/unavailable-state class; prove every declared case has the correct Source, tool, product surface, condition or unavailable state, and evidence, with zero physical or synthetic files, file IDs, Source-relative Paths, authored source text, comparison targets, relationship origins, local or hosted reads, and network requests in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1061 Inspect the complete diff/tarball for correctness and untested branches, especially the six Presentation Allowlist freezes; exact 48-source/57-rule counts and the VS Code 1.118 root `.mcp.json` evidence conflict/path-only provenance; exact `--cwd`/generation 0; pathless collisions; safe-fs rows 1–28/directory guards; hard-link scan-attempt scope, nlink, static prepass and zero-read alias cases; both exact FR-022 authorized internal loopback classes, their independent target/route/capability validation, continuous participant-study product/server/browser instrumentation and actor correlation, zero prohibited direct-product requests on every other surface, local-fixture/explicit-UNC zero-call semantics, and mounted/mapped OS-mediated traffic; process-wide close registry and the four poisoned-registry pre-schedule `409` gates with zero state/job/I/O; replacement decoding; structural-`lstat` carveout versus unchanged propagation; Operation Error/startup ownership; record-specific evidence/status/qualifiers; fixed-three batches; FR-042 full purge, epoch/fence/liveness/recovery, failed retry/join/restart and N/N+1 semantics; Fact I/O, external mutation/atime, authority, capacity/verdict, manifest/trace/WCAG, cleanup, package, bilingual, and privacy regressions. Repeat affected evaluations in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1062 Resolve every T1041/T1061 concern, run `pnpm run test:docs` and `git diff --check`, and record passing results for all 56 FR/QR/SC trace rows through FR-042 and every mapped T001–T1063; every task's presence of at least one exact repository-relative owned file path; rejection of basename-only shorthand; and exact per-task English/Japanese owned-path-set parity. The gate must explicitly cover six-file freeze verification; exact 48-source/57-rule counts and VS Code 1.118 root-path conflict semantics; both exact FR-022 authorized internal loopback classes with target/route/capability validation, continuous participant-study product/server/browser instrumentation and actor correlation, and zero prohibited requests elsewhere; `--cwd`/generation 0; collision/hard-link/directory/resource-registry contracts, including all four poisoned-registry pre-schedule `409 resource-cleanup-restart-required` gates with zero state/job/I/O; replacement decoding/binary; exact structural-`lstat` `ENOENT` and unchanged propagation; REST Operation Error/startup ownership; evidence-assessment identity; fixed-three Global work; FR-042 pre-purge, greater-epoch/non-null-fence purge, inspection-data success's unchanged-epoch/null-fence final gate versus liveness success's same-lock current-epoch/current-fence projection with a non-null fence allowed, exact recovery DTOs, all-data fence, retained failed retry/join/restart, and public-state N+1 versus unpublished-operation N cleanup; deterministic partial/atomic late discard; manifests; boundary non-authority; migration evidence; and bilingual parity in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`. Keep T1063 and release approval blocked for any stale term, missing evidence, failure, or diff error, and retain the separate manual manifest-review record.
- [ ] T1063 Perform and record an explicit principle-by-principle release Constitution Check—including the dependency/breaking-change rationale and migration-impact gate—confirm that every known violation is resolved and every residual uncertainty has an owner and concrete resolution path, and require the same explicit check in the pull request review before approval in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
...
| 21 Unified Instructions Inventory | US1 | Users can understand the complete static instruction inventory, every shared-file interpretation, and the one configured fallback integration that will activate when MCP admits its minimum carrier. |
...
| 95 Global Consent Preview | US4 | Users can review the exact Global roots, patterns, exclusions, lexical validity state, and contract version before enabling inspection. |
...
| 99 Atomic Global Batch Result Integration | US4 | Zero to three separately identified one-root tool Sources appear together in exactly one complete or contracted-partial generation and reuse detail/comparison workflows. |
...
- Phase 15 defines the pure Codex fallback-declaration interface without authorizing a configuration read; Phase 23 atomically admits the minimum `.codex/config.toml` carrier, registers `codex.repo.config` and `codex.derived.fallback-basename`, and activates configured instruction fallbacks together with Codex MCP declarations.
...
- Phase 96 establishes selector-free fixed-three consent, three controls, atomic admission partitioning, and one shared-ID `GlobalBatchScan`; Phases 97–98 add Claude and Copilot as members of that same batch; Phase 99 publishes every admitted separately identified one-root Source together in exactly one complete or contracted-partial generation, while later explicit Global rescans remain single-Source operations.
- Phase 102 task T1037 is the semantic evidence-drift gate: semantically unchanged reviewed citation metadata may update production registries, but any accepted normative behavior, rule, strategy, Presentation Allowlist, registry-shape, or conformance change supersedes the current bilingual task set and requires synchronized artifacts plus replanning before later old IDs run.
- Phase 103 task T1041 is the later hard cross-artifact gate: its manifest or documentation/traceability suite must pass with no unresolved concern before T1042 or any later task under the current IDs may begin.
...
- In Minimal Secure Foundation, shared DTO/Diagnostic and environment-failure tests, host-security tests, package-policy tests, and filesystem-fixture preparation use different files and can proceed in parallel where marked.
...
- Global vendor boundary tests use isolated fixture roots, but Phases 96–98 remain checkpoint-sequential while they add one shared fixed-three consent/admission/batch contract and distinct per-tool control/context projections; tentative work never publishes a Source before the Phase 99 atomic batch integration exists.
...
3. Activate selector-free consent for fixed `[copilot, claude, codex]`, evaluate all three on initial enable or every server-derived missing Source on retry, and validate their one-root controls without publishing tentative Sources.
4. For a nonempty admitted subset, run exactly one shared-ID `GlobalBatchScan` and atomically publish zero to three separate tool-specific Sources together through one complete or contracted-partial generation, preserving carried Sources and never merging roots; an empty deterministic subset creates no job or generation.
...
7. Record SC-001–SC-009 denominators, thresholds, pass/fail results, the full Node.js engines contract plus exact lower-bound/browser certification samples, and residual risks.
8. Record the explicit principle-by-principle release Constitution Check and require the matching pull request review check.
...
- Effective `O_NOFOLLOW` is mandatory final-component defense in depth where Node.js exposes and enforces it. No test may claim kernel-enforced containment against active source-root/ancestor replacement on any platform, final-component replacement where effective `O_NOFOLLOW` is unavailable, or a `platform-unobservable` case; every detectable change remains in scope and fails closed.
- FR-038 applies to project-authored executable application code and executable code in the published/installed production closure. Project-authored build and test code also uses JavaScript/TypeScript as a repository design choice, while third-party development/test tooling remains outside FR-038 and is pinned and audited separately. Rust, Cargo, Node-API/native addons, prebuilt binaries, lifecycle compilation, and lifecycle/runtime artifact downloads remain prohibited from the product boundary defined by FR-038.
...
- A usable physical hard-link group is read once only within one Source scan attempt after complete static discovery/group formation, except for the documented Codex content-dependent fallback; distinct Sources, scan attempts, and generations are independent and may each read the same underlying object once. One published file may retain multiple tool recognitions and direct provenances.
...
- Global inspection has one fixed-three consent record, three controls, and zero to three separately identified Sources, at most one per supported tool and exactly one root per Source. Tentative admission/scan work is not a Source. Initial enable or retry transfers every admitted context into one `GlobalBatchScan` with one request ID, authority, and working set, then publishes all admitted Sources together in one complete or contracted-partial generation; no per-tool intermediate commit exists. A later explicit Global rescan remains a single-Source transaction. Source IDs remain process-lifetime stable while generation-owned graph IDs rekey.
- Complete decoded authored source, exact metadata literals, and authored relationship targets remain available in the active session. The capability-authenticated API returns them only through explicit detail requests; it has no acknowledgement field and neither accepts nor enforces the presentation acknowledgement. The bundled browser issues those requests and renders their authored values only after its in-memory sensitive-content acknowledgement. Credentials and environment-reference syntax are displayed unchanged; referenced process-environment values are never read or substituted; diagnostics and logs do not duplicate source values.
