...
- [ ] T004 Confirm that `package.json`, `pnpm-lock.yaml`, configuration, CI, release, and package-policy tasks use the one revalidated baseline, then synchronize its packages, exact versions, and runnable commands in `specs/001-inspect-agent-customizations/research.md`, `specs/001-inspect-agent-customizations/research.ja.md`, `specs/001-inspect-agent-customizations/plan.md`, `specs/001-inspect-agent-customizations/plan.ja.md`, `specs/001-inspect-agent-customizations/quickstart.md`, `specs/001-inspect-agent-customizations/quickstart.ja.md`, `specs/001-inspect-agent-customizations/tasks.md`, and `specs/001-inspect-agent-customizations/tasks.ja.md`
...
- [ ] T017 [P] Add failing tests for public entity shapes, one-root Source invariants, bounded `SourceConditionFact` records with exact tool/surface/rule/evidence/condition fields and no file identity/path/text/comparison/relationship/read authority, exact authored-literal metadata, UTF-16 source ranges, opaque generation-scoped IDs, versioned API envelopes, strict request guards, and rejection of internal authority records in `tests/unit/shared/entities.test.ts` and `tests/unit/shared/api.test.ts`
- [ ] T018 Create deterministic cross-platform fixtures for links, junctions, non-regular entries, deep trees, VCS internals, injected source-root/ancestor/final-component replacement checkpoints, effective and unavailable/ineffective `O_NOFOLLOW`, unverifiable checks, and `platform-unobservable` outcomes in `tests/fixtures/adversarial/build-filesystem-fixtures.ts`
...
- [ ] T021 Add failing boundary tests for same-handle identity, every detectable replacement and byte disposal, final-component defense with effective `O_NOFOLLOW`, explicit no-effective-`O_NOFOLLOW` postcheck receipts, source-attempt rejection for shared-boundary unverifiability, item rejection for candidate unverifiability, and records that never claim proof against active source-root/ancestor replacement or unavailable-final-defense races and never count `platform-unobservable` outcomes as proof in `tests/integration/boundaries/node-safe-fs.test.ts`
...
- [ ] T028 Implement public DTOs, one-root Repository/tool-specific-Global Source invariants, bounded evidence-linked `SourceConditionFact` DTOs with required tool/surface/source refs and an unrepresentable file/relationship/read authority, exact authored-literal metadata and relationships, internal-type exclusion, opaque ID guards, versioned envelopes, deterministic production JSON encoding, and strict manual request guards in `shared/entities.ts` and `shared/api.ts`
...
- [ ] T031 Implement one-time ticket reads with complete ordered verification at pre-open, post-open/pre-read, and post-read, same-handle identity, mandatory effective `O_NOFOLLOW` when enforceable, `effective-o-nofollow` versus `no-effective-o-nofollow-postchecks` receipts, byte disposal, and no publication in `src/inspection/safe-fs.ts`
...
- [ ] T043 [P] [US1] Add failing Gunshi CLI and packaged launch tests for the root `define`/`cli` API, positive default-true `open` with generated `--no-open`, non-binding built-in help/version, `strict: true` unknown-option rejection, explicit positional/rest rejection before bind, awaited completion, explicit validation `AggregateError` handling with fixed bounded output and nonzero failure exit, exact shebang/mode/package fields, isolated installation, one printed closed-grammar loopback URL with a 43-character base64url capability before launch, the fixed `/usr/bin/open` or `/usr/bin/xdg-open` startup helper as the sole permitted product child process with `shell: false`/ignored stdio/unref and exact environment allowlists, proof that it receives no inspected content/path, authored value, user command, or environment-selected handler, delegates only to the OS default browser and cannot certify its version, zero Windows/unsupported-platform spawn, usable `--no-open`/printed-URL certified-browser fallback on disabled/unsupported/helper failure or uncertified handlers, direct shell boot, clean shutdown, no `gunshi/agent`/lazy/custom-plugin import, and zero inspected-source reads in `tests/unit/cli.test.ts` and `tests/package/npx-launch.test.ts`
...
- [ ] T047 [US1] Implement the Gunshi root `define`/`cli` entry with positive default-true `open` and generated `--no-open`, `strict: true`, explicit pre-bind positional/rest rejection, awaited completion, non-binding built-in help/version, explicit validation `AggregateError` handling with fixed bounded rendering and nonzero failure exits, no `gunshi/agent`/lazy/custom-plugin/experimental-combinator import, one-time closed-grammar loopback URL output with a 43-character base64url capability before launch, the fixed `/usr/bin/open` or `/usr/bin/xdg-open` startup helper as the sole permitted product child process with `shell: false`/ignored stdio/unref, exact child-environment allowlists excluding `BROWSER`/`NODE_OPTIONS`/`NODE_PATH`, inspected content/path, authored values, user commands, and environment-selected handlers, explicit default-browser delegation without version selection/verification, zero Windows/unsupported-platform spawn, usable `--no-open`/printed-URL manual certified-browser fallback for disabled/unsupported/helper failure or uncertified handlers, launch-`cwd` capture, and graceful shutdown in `src/cli.ts` and `src/launch-browser.ts`
...
- [ ] T056 [P] [US1] Add zero-activation tests under `--no-open` or instrumentation begun after the isolated permitted startup helper, proving Codex SKILL discovery causes no child process, dynamic evaluation/import, MCP connection, outbound request, URI load, or inspected-source mutation in `tests/integration/security/zero-activation.test.ts`
...
- [ ] T183 [P] [US1] Add the reusable SC-002 harness and versioned published profile validator for exact OS image/version, CPU architecture/model/logical count, memory, storage/filesystem, runtime, benchmark command/configuration, and deterministic fixture manifest/digest; construct the 100,000-entry/500-match fixture, control fresh processes, assert snapshot/cache policy, capture browser-request timing only when a current-request queued/active-phase/complete/partial/failed status is visible and assistive-technology-exposed (reject generic/loading/unchanged/old state), time operable inventory and the two standardized interactions, and run one non-gating smoke pass in `tests/performance/sc002-reference-profile.json`, `tests/performance/repository-scan.test.ts`, and `tests/performance/inventory-interactions.test.ts`; defer the exact 10-run 9/10 release protocol to T918
...
- [ ] T917 [P] [US1] Add complete packaged Gunshi CLI tests for isolated install, fixed assets/Worker, same tarball, launch `cwd`, default-browser delegation without version-certification claims, `--no-open` plus printed-URL certified-browser fallback, non-binding help/version, strict unknown-option and positional/rest rejection with fixed bounded nonzero failures, awaited shutdown, root-only import boundary, and zero extra modes in `tests/package/npx-launch.test.ts`
- [ ] T918 [P] [US1] Extend T183 to the final registry, validate that every run matches the same checked-in profile and unchanged fixture digest, and execute exactly ten fresh processes with browser-request start, no snapshot reuse or intentional OS-cache reset, and a 9/10 pass rule requiring a qualifying current-request status within one second, operable inventory within ten seconds, and both standardized interactions below 100 ms; emit the profile ID, fixture digest, and actual non-personal environment fields, omit only personal identifiers and absolute user paths, and reject cross-profile comparison in `tests/performance/repository-scan.test.ts` and `tests/performance/inventory-interactions.test.ts`
...
- [ ] T925 [P] [US2] Extend zero-activation regression across every Repository family under `--no-open` or instrumentation begun after the isolated permitted startup helper, proving discovery, read, parse, display, comparison, and relationship processing cause zero child processes, evaluation/import, MCP, network, URI/image loads, writes, and referenced reads in `tests/integration/security/zero-activation.test.ts`
- [ ] T926 [P] [US2] Add complete file-detail and removed-reveal-route API contracts for every kind, exact authored source/metadata/relationship literals, rejection of every metadata or relationship tuple absent from the maintained closed presentation allowlist while retaining unknown authored keys/references only in complete source text, strict envelopes, stale IDs, no-store, 4-MiB whole-response failure without truncation, and source-value-free failures in `tests/contract/http-api-files.test.ts`
...
- [ ] T930 [US4] Create isolated Global-home fixtures for exact candidates, exclusions, fallback, invalid overrides, links, aliases, unreadable roots, distinct literal credentials and environment references, sentinel process values, inert executable-looking payloads, before/after content hashes, and mutation sentinels with bilingual usage guidance in `tests/fixtures/global-homes/build-fixtures.ts`, `tests/fixtures/global-homes/README.md`, and `tests/fixtures/global-homes/README.ja.md`
...
- [ ] T995 [P] [US4] Add failing exact-display API and integration tests for Global fixtures containing distinct literal credential text, environment-reference syntax, sentinel process values, inert executable-looking payloads, and mutation/hash sentinels; prove each source/detail/comparison surface and API returns exact authored text, never the process sentinel, and never mutates a fixture in `tests/contract/http-api-files.test.ts` and `tests/integration/global-literal-display.test.ts`
- [ ] T996 [P] [US4] Add failing zero-activation security tests proving those Global fixtures cause zero dynamic evaluation, command or hook execution, browser-helper launch, MCP connection, outbound request, environment lookup or substitution, and source write; also prove origin-file-less Source Condition Facts cause zero local or hosted I/O and create no file, relationship-origin, or comparison identity in `tests/security/global-zero-activation.test.ts` and `tests/integration/source-condition-facts.test.ts`
- [ ] T997 [US4] Add browser acceptance for exact-preview enablement, separately identified Codex/Claude/Copilot Sources after independent commits, source/tool filters, one-root presentation, sibling diagnostics, exact literal credential/environment-reference display with no process substitution or activation, origin-file-less Source Condition Fact separation with no synthetic file action, Global detail/comparison reuse, retryable missing tools, fatal retention, and Repository preservation in `tests/e2e/global-enable.spec.ts`
...
- [ ] T1000 [US4] Implement integrated but isolated per-tool Global scanning, sibling continuation, Codex fallback, and independent bounded ready/partial results for separate one-root Sources while preserving Repository and other tool results in `src/inspection/scan.ts`
- [ ] T1001 [US4] Atomically publish or replace one tool-specific Global Source only when its own scan completes/partially completes, carry Repository and sibling Sources into the new session generation, preserve all Source IDs, rekey generation-owned graph IDs, clear that tool's transient work, and on fatal publish no missing Source while retaining consent/control retry state and prior graphs in `src/session/session.ts` and `src/session/scan-generation.ts`
...
- [ ] T1004 [US4] Complete per-tool Global confirmation/progress/retry controls, focus recovery, active-no-job handling, and separate Source outcome presentation in `app/pages/global-consent.vue` and `app/components/consent/GlobalSourceControls.vue`
...
- [ ] T1006 [US4] Add failing coordinator tests for cross-source FIFO, dequeue-time generations, duplicate scan conflicts, progress transitions, fatal retention, and per-job counters in `tests/unit/session/coordinator.test.ts`
- [ ] T1007 [US4] Extend coordinator tests for carried-Source graph/base-budget accounting, lifecycle/control overlay capacity, per-command reservation transfer/release, and exact-limit/one-over visited-entry/deadline resets per active job in `tests/unit/session/coordinator.test.ts`
...
- [ ] T1012 [US4] Implement FIFO rescan for one identified tool-specific Global Source, successful/partial session-wide commits that preserve all Source IDs, regenerate carried and replaced generation-owned graph IDs, clear only the rescanned Source's stale failure, preserve sibling failures, and invalidate old FileDetail/comparison state in `src/session/session.ts`, `src/session/stale-failures.ts`, and `src/session/scan-generation.ts`
- [ ] T1013 [US4] Implement carried-source budget reservation and per-active-job visited-entry/deadline reset in `src/session/session.ts` and `src/session/scan-generation.ts`
- [ ] T1014 [US4] Implement strict `POST /api/v1/global/rescan` handling for exactly one opaque `sourceId`, one tool-specific Source, disable/duplicate conflicts, bounded-capacity errors, progress, fatal retry, and stale-resource responses in `src/host/api-router.ts`
...
- [ ] T1016 [US4] Add semantically equivalent English/Japanese Global rescan, queue, partial-result, failure-retention, and retry messages in `app/locales/en.ts` and `app/locales/ja.ts`
...
- [ ] T1017 [US4] Add failing coordinator tests for priority cancellation, zero cancellation diagnostics, Repository requeue-once, N+1 retained-Repository graph rekeying with stable Repository `Source.sourceId`, stale generation-owned IDs, joined barriers, no-op disable, and timestamps in `tests/unit/session/coordinator.test.ts`
...
- [ ] T1021 [P] [US4] Add lifecycle tests for removal of every Global Source/file/graph ID, source and control diagnostics, authored-source/detail state, comparison selections, Monaco models/workers, consent, tool controls, frozen preview, root contexts, and handles in `tests/integration/session-lifecycle.test.ts`
...
- [ ] T1029 Draft semantically equivalent operational guidance with verified launch commands; the full Node.js 24/26 engines contract, Node.js 24.18.0 build baseline, and lower-bound certification samples; OS-default-browser delegation versus pinned Playwright certification and `--no-open` manual fallback; Repository and separate one-root Global scope; Source Condition Fact separation; consent/retry/disable; literal-value/no-environment-resolution behavior; the published SC-002 profile/status protocol; exact limits, diagnostics, filesystem residual risks, privacy, accessibility, exclusions, and maintenance commands in `README.md` and `README.ja.md`
- [ ] T1030 Create semantically equivalent SC-001/SC-006 study kits with the maintainer-owned bilingual plan naming study, recruitment/compensation-funding, moderation/review, support, privacy/retention, equipment/session, and accessibility owners; the fixed twenty-participant order, certified-browser record/fallback, exact timers, prepared state, four-field ground truth, no-hints and no-replacement rules, all four workflow observations, automatic critical safety events, and two independent fixed-rubric classifications only for suspected non-safety product blockers, with disagreement counted as critical and no third adjudicator; state that ordinary contributors do not operate the study, missing resources block the release claim rather than contribution review, and rerun is required only after a material workflow/guidance/fixture/rubric change in `tests/usability/sc001-sc006-study-kit.md` and `tests/usability/sc001-sc006-study-kit.ja.md`

### Official evidence and dependency review

- [ ] T1031 Add failing official-source checker contracts for exact hosts, redirect rejection, bounded content, size/time limits, explicit network opt-in, and non-mutating drift reporting in `tests/contract/official-source-drift.test.ts`
...
- [ ] T1034 [P] Enumerate the exact closed metadata-field and relationship-kind presentation allowlist for every supported Copilot customization type and resolve accepted evidence drift without automatic scope expansion in `specs/001-inspect-agent-customizations/contracts/vendors/github-copilot.md` and `specs/001-inspect-agent-customizations/contracts/vendors/github-copilot.ja.md`
- [ ] T1035 [P] Enumerate the exact closed metadata-field and relationship-kind presentation allowlist for every supported Claude customization type and resolve accepted evidence drift without automatic scope expansion in `specs/001-inspect-agent-customizations/contracts/vendors/claude-code.md` and `specs/001-inspect-agent-customizations/contracts/vendors/claude-code.ja.md`
- [ ] T1036 [P] Enumerate the exact closed metadata-field and relationship-kind presentation allowlist for every supported Codex customization type and resolve accepted evidence drift without automatic scope expansion in `specs/001-inspect-agent-customizations/contracts/vendors/openai-codex.md` and `specs/001-inspect-agent-customizations/contracts/vendors/openai-codex.ja.md`
- [ ] T1037 Apply only explicitly reviewed evidence changes to `shared/registries/vendor-behaviors.ts`, `shared/registries/inspection-rules.ts`, `shared/registries/runtime-composition.ts`, and `shared/registries/official-sources.ts`
...
- [ ] T1041 Add documentation and traceability tests for reciprocal English/Japanese links and semantic parity; runnable commands and stable IDs; all 53 FR/QR/SC rows with existing task references and every task mapped to a specification requirement or explicit Constitution obligation; closed presentation allowlists with registry/API parity; literal-display/no-environment-resolution and Source Condition Fact zero-synthetic-file/I/O rules; one-root Sources; the exact engines/build/certification distinction and default-browser fallback; the published SC-002 profile fields, result fields, and personal-identifier/absolute-user-path-only omission rule; `O_NOFOLLOW`, `safe-fs-boundary-unverifiable`, `platform-unobservable`, residual-risk wording, official backlinks, and absence of stale native claims in `tests/contract/documentation.test.ts`
- [ ] T1042 [P] Materialize exactly 47 source records and add final full-registry tests for exactly 56 inspection-rule IDs—35 Repository static, five bounded-derived, ten vendor-excluded, two shared-excluded, and four Global static—plus 39 strategies, 14 relationship-only rules, zero contained Hook/MCP candidate additions, exact presentation-allowlist/registry parity, complete reciprocity, official-source identity bounds, and origin-file-less Source Condition Facts with exact Source/tool/surface/status/evidence plus zero read authorization, synthetic files, or local/hosted I/O in `tests/fixtures/conformance/official-sources.json`, `tests/contract/inspection-rules.test.ts`, `tests/contract/runtime-composition.test.ts`, and `tests/contract/official-sources.test.ts`
- [ ] T1043 [P] Add exact packed-tarball closed-set and recursive-manifest tests for `package.json`, the exact `bin.mjs`, exact packed `engines.node`, pre-import running-version rejection, `dist`, both README files, `LICENSE`, both manifests, every listed payload hash, fixed CLI/Worker entries, and rejection of unlisted payloads in `tests/package/package-contents.test.ts`, `tests/package/static-manifest.test.ts`, and `tests/package/server-manifest.test.ts`
...
- [ ] T1052 Validate the checked-in `tests/performance/sc002-reference-profile.json` fields and fixture digest, then record SC-002 pass/fail for exactly ten fresh-process runs matching that unchanged profile/100,000-entry/500-match fixture, with no prior snapshot reuse or intentional OS-cache reset; require at least 9/10 runs to visibly render and expose to assistive technology a current-request queued/active-phase/complete/partial/failed status within one second, rejecting generic/loading/old state, complete the operable inventory within ten seconds, and keep both standardized interactions below 100 ms; publish the profile ID, digest, and actual environment values while omitting only personal identifiers and absolute user paths, and mark changed profiles non-comparable in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
...
- [ ] T1054 Record SC-004 pass/fail from the final gates and safety-suite denominators, including T996/T997 Global inert-payload fixtures, with the fixed startup browser helper isolated as the sole permitted product child process and zero customization-derived activation, child process, MCP, network, mutation, rejected-selector reads, environment lookup/substitution, or changed-file byte publication in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1055 Record SC-005 pass/fail and denominators, including T930/T995/T997 Global fixtures, proving 100% of maintained literal credentials and environment-reference text remain unmasked and unchanged across API/source/comparison views, zero process-environment sentinel values are introduced, zero masking/reveal controls exist, fixture hashes remain unchanged, and diagnostics/logs duplicate zero source values in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1056 Execute SC-001 first with exactly twenty non-replaced participants under the maintainer-owned bilingual study plan, record the certified browser/default-handler or manual fallback, exact two-minute boundary, and every unsuccessful equipment/environment/product outcome, and require at least nineteen successful launch/open results in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1057 Execute SC-006 after SC-001 with the same exactly twenty participants, prepared file state, certified-browser record/fallback, exact two-minute boundary, four-field ground truth, no replacements, then have every participant attempt comparison and Global consent; record all workflow outcomes and automatic safety events, use two independent fixed-rubric classifications only for suspected non-safety product blockers, count disagreement as critical without a third adjudicator, and require at least eighteen successful identifications plus all twenty complete attempts with zero critical issues in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
...
- [ ] T1061 Inspect the complete diff and packed tarball for correctness, untested branches, literal-display/environment-resolution failures, Source Condition Fact synthesis/I/O, boundary failures, stale claims, source-root merges, engines/certification ambiguity, package-graph divergence, bilingual mismatches, SC-002 personal-identifier or absolute-user-path leakage, and unrelated changes; repeat every affected evaluation in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1062 Finalize residual issues and concrete resolution paths, rerun `pnpm run test:docs`, and run `git diff --check` with results recorded in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
