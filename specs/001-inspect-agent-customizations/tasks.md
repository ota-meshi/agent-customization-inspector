...
| FR-024 | T018–T022, T029–T032, T924, T1051, T1058 |
...
| FR-028 | T016–T017, T027–T028, T032, T923, T927, T1041, T1058 |
...
| FR-039 | T091, T153, T159, T246, T260, T374, T377, T380–T386, T388, T391, T393, T400, T547, T550, T720, T723, T911, T923, T979–T980, T985–T986, T995–T997, T1029, T1041–T1042, T1060 |
...
| SC-006 | T1030, T1057 |
| SC-007 | T018–T023, T055, T067, T915, T922–T924, T927, T930, T995, T1058 |
...
- [ ] T001 Revalidate the plan-approved dependency baseline before changing package or configuration files; if any package or version changes, stop, synchronize the dependency-baseline-bearing `research.md`/`research.ja.md`, `plan.md`/`plan.ja.md`, `quickstart.md`/`quickstart.ja.md`, and `tasks.md`/`tasks.ja.md` pairs under `specs/001-inspect-agent-customizations/`, rerun `/speckit-plan` and `/speckit-tasks`, and otherwise pin Node.js `^24.11.0 || ^26.0.0`, `pnpm@11.13.0`, the exact runtime leaf set `gunshi` 0.37.0, `yaml`, `jsonc-parser`, and `smol-toml`, the approved exact development versions, and the frozen graph in `package.json` and `pnpm-lock.yaml`
...
- [ ] T004 Freeze and validate the complete closed Presentation Allowlist for every supported `(tool, kind)` before dependent parser, recognizer, API, UI, or acceptance work, including exact metadata `fieldId` and relationship-kind sets, admitted source forms with exact extractor applicability, contained-owner rules, both tuple-membership and actual-source-form eligibility gates, and the product-wide boundary that permits only syntax/literal/typed/catalog/structural projection and admits no natural-language interpretation/ranking, content verdict, validation/lint, policy/remediation, synchronization, conversion, formatting, or fixing output; keep semantically equivalent English/Japanese rows in `specs/001-inspect-agent-customizations/contracts/vendors/github-copilot.md`, `specs/001-inspect-agent-customizations/contracts/vendors/github-copilot.ja.md`, `specs/001-inspect-agent-customizations/contracts/vendors/claude-code.md`, `specs/001-inspect-agent-customizations/contracts/vendors/claude-code.ja.md`, `specs/001-inspect-agent-customizations/contracts/vendors/openai-codex.md`, and `specs/001-inspect-agent-customizations/contracts/vendors/openai-codex.ja.md`
...
- [ ] T015 [P] Add failing deterministic tests for parser-message, retained-graph, lifecycle-Diagnostic, control, and complete-envelope construction; verify that Node.js, parser, OS, filesystem, browser, or execution-environment resource failures abort the attempt, return only a closed source-value-free lifecycle failure, publish no item, scan-result record or response, or generation, never classify customization content as valid or invalid, and preserve only the last committed snapshot in `tests/unit/shared/runtime-failures.test.ts`
- [ ] T016 [P] Add failing tests for the closed diagnostic registry, deterministic aggregation, successful complete atomic publication, and recoverable environment-resource failures that return only a typed `fatal-resource` signal with source-value-free lifecycle arguments and no generation, item, result record, or response payload; separately test the closed `OperationalEvent` schema and reject every extra/free-form field, Source-relative/absolute/canonical path, root, filename, inspected content/metadata, authored value, capability, body, parser/system error, exception string, Diagnostic argument, or generic correlation field while allowing only fixed event codes and optional opaque session/source/file/scan/operation IDs in `tests/unit/shared/diagnostics.test.ts` and `tests/unit/shared/operational-events.test.ts`
- [ ] T017 [P] Add failing tests for public entity shapes, one-root Source invariants, closed `ScopeDescriptor` and `OrderDescriptor` unions with stable keys where unknown order is represented only by null plus an evidence-linked `SourceConditionFact`, evidence-linked `SourceConditionFact` records with exact tool/surface/rule/evidence/condition fields and no file identity/path/text/comparison/relationship/read authority, and the closed Diagnostic scope union: file scope requires coherent `sourceId`/`fileId`/`sourceRelativePath`, source scope requires `sourceId` and forbids file/path, and session scope forbids all three; reject missing, extra, cross-source, or fabricated location tuples, while also covering one opaque `scanRequestId` shared by Source/progress/admission/attempt/successful generation, exact authored-literal metadata, UTF-16 source ranges, opaque generation-scoped IDs, versioned API envelopes, strict request guards, the absence of any acknowledgement or customization-validation API field, and rejection of internal authority records in `tests/unit/shared/entities.test.ts` and `tests/unit/shared/api.test.ts`
- [ ] T018 Create deterministic cross-platform fixtures for links, junctions, non-regular entries, deep trees, VCS internals, injected source-root/ancestor/final-component replacement checkpoints, effective and unavailable/ineffective `O_NOFOLLOW`, unverifiable checks, `platform-unobservable` outcomes, instrumented rejection of every mutation-capable open/operation, before/after content/length/identity/link/mode/mtime/ctime/xattr/ACL observations, and separately recorded OS-only atime movement in `tests/fixtures/adversarial/build-filesystem-fixtures.ts`
- [ ] T019 Add failing root-context and enumeration tests for lexical rejection before I/O, component `lstat`, canonical containment, bigint identity, plan-driven `opendir`, raw `Dirent.name` I/O segments, NFC display/classification, NFC-sibling collision fail-closed behavior, VCS exclusion, and detectable device changes in `tests/unit/inspection/node-safe-fs.test.ts`
- [ ] T020 Add failing ticket/read tests for private generation/`scanRequestId` publication binding, one-time use, client-path rejection, read-only/non-create/non-truncate opens, zero mutation-capable calls, cleanup after publication-authority revocation, and root plus every available ancestor followed by candidate `lstat` → `realpath` containment → repeated unchanged `lstat` at enumeration, pre-open, post-open/pre-read, and post-read in `tests/unit/inspection/node-safe-fs.test.ts`
- [ ] T021 Add failing boundary tests for same-handle identity, every detectable replacement and complete byte disposal, final-component defense with effective `O_NOFOLLOW`, explicit no-effective-`O_NOFOLLOW` postcheck receipts, source-attempt rejection for shared-boundary unverifiability, item rejection for candidate unverifiability, instrumented zero mutation-capable operations with unchanged content/length/identity/link/mode/mtime/ctime/xattr/ACL state and separately recorded OS-only atime, plus late-result discard after disable, shutdown, or supersession without claiming hard kernel cancellation; never count `platform-unobservable` outcomes as proof in `tests/integration/boundaries/node-safe-fs.test.ts`
...
- [ ] T026 [P] Add failing generation and session tests for bootstrap generation 0; one opaque `scanRequestId` shared by every automatic/explicit scan admission, Source/progress/attempt/final status, and successful generation; deterministic graph IDs; coordinator-locked generation/payload capture; serialized state transitions; atomic N+1 replacement; fatal retention; per-Source stale failures; ID rekeying; complete canonical single-buffer encoding; rejection before mutation when the active Node.js runtime cannot represent the next `GenerationNumber`; recoverable environment-resource failure that makes the current attempt fatal, commits no N+1 generation, item, record, response, or contracted-partial result, and retains only the prior committed snapshot; and publication-authority revocation with late-result discard after disable, shutdown, or supersession in `tests/unit/session/scan-generation.test.ts` and `tests/unit/session/session.test.ts`

### Implementation

- [ ] T027 Implement one closed failure-observability contract: normalize environment-resource failures, construct complete lifecycle records and production JSON outside scan results, provide a closed Diagnostic registry with source-value-free arguments, and keep a separate closed `OperationalEvent` logger that accepts only fixed codes/opaque IDs and rejects paths/content/metadata/capabilities/bodies/raw errors/Diagnostic fields; do not define Inspector-specific numeric size, count, parser, request, response, queue, time, or concurrency limits, and ensure recoverable Node.js/parser/OS/filesystem/browser failures abort the attempt, commit no item, scan-result record or response, or generation, retain only the prior committed snapshot, and produce no customization validity/lint/verdict in `shared/runtime-failures.ts`, `shared/diagnostics.ts`, `shared/operational-events.ts`, and `src/host/runtime-failures.ts`
- [ ] T028 Implement public DTOs, one-root Repository/tool-specific-Global Source invariants, closed `ScopeDescriptor` and `OrderDescriptor` unions with stable keys where unknown order is represented only by null plus an evidence-linked `SourceConditionFact`, one opaque `scanRequestId` on Source/progress/scan admission/successful generation, evidence-linked `SourceConditionFact` DTOs with unrepresentable file/relationship/read authority, and a discriminated Diagnostic union whose file scope requires coherent `sourceId`/`fileId`/`sourceRelativePath`, source scope requires only `sourceId`, and session scope permits none of those location fields; source-level failures and session sentinels must not fabricate paths, acknowledgement is not an API field, customization validity/lint/remediation output is unrepresentable, and exact authored-literal metadata/relationships, internal-type exclusion, opaque ID guards, versioned envelopes, deterministic JSON, and strict request guards remain enforced in `shared/entities.ts` and `shared/api.ts`
...
- [ ] T030 Define the minimum closed, immutable, versioned `TraversalPlan` and segment-program types with stable schema/version discriminants in `src/inspection/rules/types.ts`, then implement deterministic `opendir` traversal that consumes only those types, raw `Dirent.name` segment retention, NFC display normalization with collision rejection, VCS exclusion, root and every-ancestor checks, ordered candidate validation, and generation-bound `ScanEntryTicket` snapshots in `src/inspection/safe-fs.ts`
- [ ] T031 Implement one-time `scanRequestId`-bound ticket reads with complete ordered verification at pre-open, post-open/pre-read, and post-read; read-only/non-create/non-truncate opens; no mutation-capable API; same-handle identity; effective `O_NOFOLLOW`; complete byte disposal; environment-resource failure handling that returns no bytes, ticket, item, or derived result and propagates a typed `fatal-resource` signal to the coordinator; and publication-authority revocation that treats a pending Node.js operation as cleanup-only, discards every late graph/Diagnostic/DTO/log result, starts no later source I/O, and releases owned resources once without claiming hard kernel cancellation in `src/inspection/safe-fs.ts`
- [ ] T032 Implement `safe-fs-boundary-unverifiable` handling in `src/inspection/safe-fs.ts` that rejects a root/shared ancestor at source scope with no fabricated path and rejects an admitted candidate at file scope only with its coherent `sourceId`/`fileId`/`sourceRelativePath`, without guessing
...
- [ ] T035 Implement root-confined static/server normalization with exact manifest schemas, normalized paths, declared-versus-actual byte-length equality, deterministic hashes, complete environment-supported reads, and closed file sets; enforce accepted HTML/URL cases, remove only required fallbacks, record deterministic CSP hashes, and fail startup safely when complete verification cannot be performed in `scripts/clean-build-output.mjs`, `scripts/build-static-manifest.mjs`, and `scripts/assemble-server-manifest.mjs`
- [ ] T036 Replace the Setup placeholders with deterministic server-manifest generation, production-graph digesting, recursive exact-set verification, and Node.js-only policy enforcement in `scripts/assemble-server-manifest.mjs`, `scripts/build-production-graph.mjs`, and `scripts/verify-package-files.mjs` while retaining the already scaffolded fixed Node ESM entries in `src/cli.ts` and `src/inspection/parsers/worker.ts`
- [ ] T037 Implement deterministic generation construction with `scanRequestId` correlation, coordinator-locked generation/payload selection, serialized state transitions, atomic replacement, generation-0 rejection and rejection before mutation when the active Node.js runtime cannot represent the next `GenerationNumber`, per-Source fatal retention, recoverable environment-resource failure that makes the current attempt fatal, commits no generation, item, record, response, or contracted-partial result, and retains only the prior committed snapshot, and publication-authority revocation with cleanup-only handling and late-result discard after disable, shutdown, or supersession in `src/session/scan-generation.ts`, `src/session/stale-failures.ts`, and `src/session/session.ts`
- [ ] T038 Implement the strict router skeleton with method, media, request-key, no-store, safe-error handling, coordinator-locked complete payload capture, canonical production JSON buffer creation before lock release, exact `Content-Length`, and unchanged-buffer HTTP delivery; a pre-commit environment-resource encoding failure must abort the attempt, commit no item, scan-result record or response, or generation, and retain only the prior committed snapshot, while a post-commit delivery failure must leave the already committed snapshot unchanged, publish zero successful response payloads, and never be represented as partial in `src/host/api-router.ts`
...
- [ ] T043 [P] [US1] Add failing Gunshi CLI and packaged launch tests for the root `define`/`cli` API, positive default-true `open` with generated `--no-open`, non-binding built-in help/version, `strict: true` unknown-option rejection, explicit positional/rest rejection before bind, awaited completion, explicit validation `AggregateError` handling with fixed source-value-free output and nonzero failure exit, exact shebang/mode/package fields, isolated installation, one printed closed-grammar loopback URL with a 43-character base64url capability before launch, and the fixed `/usr/bin/open` or `/usr/bin/xdg-open` startup helper as the sole permitted product child process with `shell: false`/ignored stdio/unref and exact environment allowlists; prove that it receives no inspection-derived content/path, authored value, user command, or environment-selected handler, copies only allowlisted ambient platform keys directly from launch environment with lexical equality changing no provenance or authority, delegates only to the OS default browser and cannot certify its version, spawns nothing on Windows/unsupported platforms, keeps the `--no-open`/printed-URL certified-browser fallback usable on disabled/unsupported/helper failure or uncertified handlers, boots directly from a shell, shuts down cleanly, imports no `gunshi/agent`/lazy/custom-plugin path, and reads zero inspected sources in `tests/unit/cli.test.ts` and `tests/package/npx-launch.test.ts`
...
- [ ] T046 [US1] Implement the verified loopback host-bootstrap boundary: use built-ins only for supported-engine checks and complete exact-set, schema, declared-length, and hash verification before CLI evaluation/bind, then perform exactly one dynamic `import('./dist/cli.mjs')`; fail safely without binding when the execution environment cannot complete verification, and implement loopback-only binding, capability-protected data-free liveness, the path/content-free fixed-code/opaque-ID `OperationalEvent` sink kept separate from fixed CLI presentation output and authenticated Diagnostics, and zero startup documentation/network access in `bin.mjs`, `src/host/api-router.ts`, `src/host/server.ts`, and `src/host/operational-events.ts`
- [ ] T047 [US1] Implement the Gunshi root `define`/`cli` entry with positive default-true `open` and generated `--no-open`, `strict: true`, explicit pre-bind positional/rest rejection, awaited completion, non-binding built-in help/version, explicit validation `AggregateError` handling with fixed source-value-free rendering and nonzero failure exits, no `gunshi/agent`/lazy/custom-plugin/experimental-combinator import, one-time closed-grammar loopback URL output with a 43-character base64url capability before launch, and the fixed `/usr/bin/open` or `/usr/bin/xdg-open` startup helper as the sole permitted product child process with `shell: false`/ignored stdio/unref; copy only the exact allowlisted ambient platform keys directly from launch environment, with lexical equality changing no provenance or authority, and exclude `BROWSER`/`NODE_OPTIONS`/`NODE_PATH`, every inspection-derived content/path or authored value, user commands, and environment-selected handlers; explicitly delegate to the default browser without version selection/verification, spawn nothing on Windows/unsupported platforms, retain a usable `--no-open`/printed-URL manual certified-browser fallback for disabled/unsupported/helper failure or uncertified handlers, capture launch `cwd`, and shut down gracefully in `src/cli.ts` and `src/launch-browser.ts`
...
- [ ] T050 [US1] Create Codex SKILL fixtures for positive, nested, near-miss, hard-link, malformed-name, linked, empty, secret-bearing, performance, and injected execution-environment resource-failure cases in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T055 [P] [US1] Add failing scan tests for launch-`cwd` admission, `scanRequestId`-bound ticket-only reads, deterministic order, hard-link aliases, exactly one `ToolRecognition` per `(fileId, tool, kind)`, compatible provenance merge, atomic recognition failure for incompatible extraction, recoverable Node.js/parser/OS/filesystem resource failures that abort the attempt, publish no item, recognition, derived result, scan-result record or response, or generation, and retain only the prior committed snapshot, late-result discard after publication-authority revocation, deterministic entry-local non-capacity failures as the only contracted-partial path, no customization validity/lint verdict, and no relationship-target reads in `tests/integration/repository-scan.test.ts`
- [ ] T056 [P] [US1] Add zero-activation/mutation tests under `--no-open` or instrumentation begun after the isolated startup helper, proving Codex SKILL discovery causes no child process, dynamic evaluation/import, MCP connection, outbound request, URI load, mutation-capable open or filesystem operation; compare content/length/identity/link/mode/mtime/ctime/xattr/ACL state and record OS-only atime separately in `tests/integration/security/zero-activation.test.ts`
- [ ] T057 [P] [US1] Add failing contracts for exactly one launch-`cwd` Repository Source, Source-relative paths, `GET /api/v1/session`, `POST /api/v1/repository/rescan` returning `ScanAdmission { scanRequestId, source }`, and data-free liveness; require the same request ID across admission/Source/progress/final status/successful generation, reject prior state as completion, and cover duplicate conflicts, automatic-initial-failure generation 0, explicit fatal rollback, stale IDs, same-buffer whole-generation publication, and paused delivery with coherent generation/payload in `tests/contract/http-api-session.test.ts`
...
- [ ] T067 [US1] Implement plan-driven Repository enumeration, `scanRequestId`-bound ticket verification, deterministic candidate order, hard-link aggregation, exactly one `ToolRecognition` per `(fileId, tool, kind)`, compatible provenance merge, atomic recognition failure for incompatible extraction, environment-owned capacity, recoverable Node.js/parser/OS/filesystem resource-failure handling that aborts the attempt, publishes no item, recognition, derived result, scan-result record or response, or generation, and retains only the prior committed snapshot, deterministic entry-local non-capacity failures as the only contracted-partial path, no validity/lint/verdict, publication-authority revocation, and late-result discard in `src/inspection/scan.ts`
- [ ] T068 [US1] Implement automatic first scan and FIFO explicit rescan with one opaque `scanRequestId` preserved through Source/progress/attempt/successful generation; serialized coordinator admission; dequeue-time generation selection; duplicate rejection; atomic complete publication or contracted-partial publication only after complete traversal and a deterministic entry-local non-capacity failure; initial-failure generation-0 retention; explicit fatal last-commit retention; target-Source stale clearing; generation-ID invalidation; and late-result discard after disable, shutdown, or supersession in `src/session/session.ts`, `src/session/stale-failures.ts`, and `src/session/scan-generation.ts`
- [ ] T069 [US1] Implement deterministic Codex skill summaries and Repository `ScanAdmission { scanRequestId, source }` responses with Source-relative paths only in authenticated DTOs, request-correlated progress/status/generation, conflicts, stale failures, retained-snapshot status, stale-resource handling, source-value-free Diagnostics, and no path projection into `OperationalEvent` in `src/host/api-router.ts`
...
- [ ] T075 [P] [US2] Add failing post-read verified-byte tests for NUL→binary diagnostic-only, strict UTF-8, one leading recorded/removed BOM, unsupported invalid UTF-8 without replacement/alternate decoding, no source/comparison for binary or unsupported input, inert Markdown/frontmatter, YAML core-schema, no-alias, no-custom-tag, exact-source-range, JSON-safe canonical integer/float/date-time strings, atomic recognition failure, mandatory Worker dispatch with no synchronous fallback, and injected parser/execution-environment resource failures that return no decoded value, parser output, extraction, recognition, item, or derived result, propagate a typed `fatal-resource` signal for the whole scan attempt, and produce no validity verdict in `tests/unit/inspection/parsers.test.ts`
- [ ] T076 [P] [US2] Add failing Worker-pool tests for a fixed package URL; environment-owned worker/message/memory/time capacity with no Inspector-defined numeric cap; crash/resource-exit handling that returns no parser, extraction, recognition, item, record, response, or contracted-partial result and propagates `fatal-resource` for the current scan attempt; Worker replacement only for a later admitted attempt; publication-authority revocation; late-result discard; and source-value-free failure reporting in `tests/unit/inspection/seed-parsers.test.ts`
...
- [ ] T081 [P] [US2] Add failing file-detail contracts for complete inert authored source, exact ordered `authoredLiteral` values, strict/stale IDs, no-store behavior, source-value-free diagnostics, and minimum metadata; prove that a pre-commit environment-resource encoding failure is fatal to the current request, creates no new result or generation, and publishes zero FileDetail success payloads, while a post-commit delivery failure leaves the committed snapshot unchanged, publishes zero successful response payloads, and is never represented as partial in `tests/contract/http-api-files.test.ts`
...
- [ ] T089 [US2] Implement the parser Worker pool with a fixed package URL; environment-owned worker/message/memory/time capacity with no Inspector-defined numeric cap; crash/resource-exit handling that returns no parser, extraction, recognition, item, record, response, or contracted-partial result and propagates `fatal-resource` for the current scan attempt; Worker replacement only for a later admitted attempt; publication-authority revocation with late-result discard; and fixed source-value-free failures in `src/inspection/parsers/pool.ts` and `src/inspection/parsers/worker.ts`
...
- [ ] T097 [US2] Implement `GET /api/v1/files/{fileId}` with strict opaque IDs, complete authored-source DTOs, exact ordered literal metadata, production encoding, no-store behavior, diagnostics, and stale responses; a pre-commit environment-resource encoding failure is fatal to the current request, creates no new result or generation, and publishes zero FileDetail success payloads, while a post-commit delivery failure leaves the committed snapshot unchanged, publishes zero successful response payloads, and is never represented as partial in `src/host/api-router.ts`
...
- [ ] T116 [P] [US2] Add failing Codex skill-metadata tests for allowlisted field IDs, ordered duplicate occurrences, exact YAML source slices/UTF-16 ranges, typed-semantic separation, seed provenance, unknown fields, malformed/overlapping ranges, deterministic entry-local non-capacity parser-failure isolation, a typed environment-resource failure that returns no parser, extraction, metadata, recognition, derived, item, scan-result record or response, or contracted-partial output, propagates `fatal-resource` for coordinator-wide attempt abort, and retains only the prior committed snapshot, environment-reference non-resolution, and exact evidence in `tests/unit/inspection/codex-metadata.test.ts`
- [ ] T117 [P] [US2] Add failing file-detail and removed-reveal-route contracts for exact ordered skill-metadata literals, stale IDs, and zero client retention; prove that a pre-commit environment-resource encoding failure is fatal to the current request, creates no new result or generation, and publishes zero FileDetail success payloads, while a post-commit delivery failure leaves the committed snapshot unchanged, publishes zero successful response payloads, and is never represented as partial in `tests/contract/http-api-files.test.ts`
...
- [ ] T141 [P] [US2] Add failing relationship tests for `targetOrigin`, exact authored target slices/range reuse, null-authored documented defaults, internal semantic normalization, provenance-relative targets, boundary status, direct non-recursive relationships from the originating recognition, complete deterministic relationship retention subject only to execution-environment capacity, recoverable resource failure that returns no relationship, provenance, recognition, or other derived output and propagates a typed `fatal-resource` signal for the whole scan attempt, rejection of nested/transitive projection before target access, and zero relationship read authority in `tests/unit/inspection/relationships.test.ts`
...
- [ ] T149 [US2] Integrate atomic Claude extraction and emit only direct one-hop provenance-scoped relationships in `src/inspection/scan.ts`; never recurse, expand, or read a relationship target, confer zero read authority, retain complete deterministic relationships subject only to execution-environment capacity, and when capacity is unavailable abort the attempt, publish no item, recognition, relationship or other derived result, scan-result record or response, or generation, retain only the prior committed snapshot, and report only a source-value-free lifecycle failure
...
**Independent Test**: Use an all-tool fixture with unique skills, duplicate names, shared physical files, hard-link aliases, item failures, secrets, and injected recoverable environment-resource failures; verify deterministic rows, multi-recognition, filters, contracted-partial continuity only for deterministic entry-local non-capacity failures, whole-attempt abort with no item, recognition, derivation, scan-result record or response, or generation for resource failures while only the prior committed snapshot remains, rescan replacement, and responsive interaction performance.

**Visible Checkpoint**: Users can filter and understand the complete skill-first inventory.

### Fixtures and tests first

- [ ] T178 [US1] Create the all-tool SKILL fixture with every supported selector, shared files, hard-link aliases, duplicate names, near misses, failures, secrets, and injected execution-environment resource failures in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T180 [P] [US1] Add failing integration tests for deterministic physical-file and recognition order, read-once merging, alias aggregation, atomic continuity, progress, contracted-partial publication only after complete traversal and deterministic entry-local non-capacity failure, and recoverable environment-resource failure that makes the whole attempt fatal, performs no extra reads, publishes no new complete or contracted-partial generation, item, record, response, or derived result, and retains only the prior committed snapshot in `tests/integration/repository-scan.test.ts`
...
- [ ] T208 [US1] Add failing scan tests for static Codex instructions and separate pure-function tests that feed validated in-memory fallback declarations into the derivation interface, proving complete retention of all configured declarations on success subject only to vendor/runtime and execution-environment capacity with no Inspector-defined numeric declaration cap; capacity failure returns no partial declaration, plan, candidate, or other derived result and propagates `fatal-resource` so the scan attempt commits no generation, item, record, or response and retains only the prior committed snapshot; also prove ancestry comparability, orphan/config escape rejection, and zero carrier or target access before rule registration in `tests/integration/repository-scan.test.ts`
...
- [ ] T217 [P] [US2] Add failing import/reference tests for exact authored target slices, `targetOrigin`, null-authored documented defaults, internal semantic normalization, lexical status, cycles, boundary status, direct non-recursive relationships, complete deterministic relationship retention subject only to execution-environment capacity, nested/transitive projection rejection before target access, environment-reference non-resolution, zero target read authority, and recoverable resource failure that returns no relationship, provenance, recognition, or other derived output and propagates a typed `fatal-resource` signal for the whole scan attempt in `tests/unit/inspection/relationships.test.ts` and `tests/integration/inspection-safety.test.ts`
...
- [ ] T223 [US2] Integrate Codex instruction exact authored-literal preservation, atomic parsing, complete deterministic direct relationship-only references subject only to execution-environment capacity, parser scratch/transient-semantic disposal while retaining complete authored source for non-capacity outcomes, resource failure that aborts the attempt, publishes no item, recognition, relationship or other derived result, scan-result record or response, or generation, and retains only the prior committed snapshot, and projection of fallback provenance only when a later admitted carrier has already produced an admitted candidate in `src/inspection/scan.ts`; never recurse, expand, or read a relationship target
...
- [ ] T238 [P] [US2] Add failing Claude import tests for exact authored target slices/ranges, internal semantic normalization, cycles, boundary status, direct non-recursive relationships, complete deterministic relationship retention subject only to execution-environment capacity, nested/transitive projection rejection before target access, environment-reference non-resolution, zero target read authority, and recoverable resource failure that returns no relationship, provenance, recognition, or other derived output and propagates a typed `fatal-resource` signal for the whole scan attempt in `tests/unit/inspection/relationships.test.ts`
...
- [ ] T243 [US2] Integrate Claude instruction parsing, exact authored-literal extraction, complete deterministic direct relationship-only imports subject only to execution-environment capacity, parser scratch/transient-semantic disposal while retaining complete authored source for non-capacity outcomes, and resource failure that aborts the attempt, publishes no item, recognition, relationship or other derived result, scan-result record or response, or generation, and retains only the prior committed snapshot in `src/inspection/scan.ts`; never recurse, expand, or read a relationship target
...
- [ ] T268 [US1] Finalize the pre-carrier all-vendor instruction fixture with `AGENTS.md` Codex+Copilot, root `CLAUDE.md` Claude+Copilot, nested `CLAUDE.md` Claude-only plus a dormant configured-fallback variant, Claude-only `CLAUDE.local.md`, every other selector, failures, secrets, exclusions, aliases, and injected execution-environment resource failures in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T270 [P] [US1] Add failing integration tests for one-read shared-file assembly, the exact pre-carrier recognition matrix, zero Codex recognition on a dormant nested fallback, deterministic provenance order, alias aggregation, atomic continuity, contracted-partial publication only after complete traversal and deterministic entry-local non-capacity failure, recoverable environment-resource failure that makes the whole attempt fatal, creates no new generation, item, record, response, or derived result, and retains only the prior committed snapshot, and no config or rejected-target access in `tests/integration/repository-scan.test.ts`
...
- [ ] T282 [P] [US1] Add failing matcher tests for atomic `codex.repo.config` plus `codex.derived.fallback-basename` registration, exact config-carrier admission, and complete configured derived instructions on success subject to environment-owned capacity with no Inspector-defined numeric declaration cap; any capacity failure must return no partial registry, plan, candidate, instruction, or other derived result and propagate `fatal-resource`; also prove no standalone Codex MCP candidate and no promotion of plugin, agent-reference, User, managed, or arbitrary config paths in `tests/unit/inspection/rules.test.ts`
...
- [ ] T321 [US2] Implement inert strict-JSON extraction with closed Claude MCP field IDs, exact tree-token source spans/UTF-16 ranges, source-order duplicates, authored-literal round trips, internal typed semantics, schema distinctions, and environment-owned parser capacity with no Inspector-defined numeric cap; deterministic entry-local non-capacity parse/extraction failure is recognition-atomic, while capacity/resource failure returns no parser, extraction, recognition, or derived result and propagates `fatal-resource` for the whole scan attempt; keep diagnostics source-value-free in `src/inspection/parsers/json.ts`
...
- [ ] T371 [US2] Add inert JSONC mode with tree-backed exact source spans/UTF-16 ranges, closed VS Code MCP field IDs, source-order duplicates, authored literals plus internal typed semantics, comments, schema distinctions, and environment-owned parser capacity with no Inspector-defined numeric cap; deterministic entry-local non-capacity parse/extraction failure is recognition-atomic, while capacity/resource failure returns no parser, extraction, recognition, or derived result and propagates `fatal-resource` for the whole scan attempt; keep diagnostics source-value-free in `src/inspection/parsers/json.ts`
...
- [ ] T392 [P] [US1] Add failing integration tests for read-once shared MCP, deterministic recognition/provenance order, currently admitted owner attachment, dormant-owner nonpublication, aliases, contracted-partial continuity only for deterministic entry-local non-capacity failures, injected recoverable environment-resource failures that abort the attempt, publish no item, recognition, derived result, scan-result record or response, or generation, and retain only the prior committed snapshot, and zero connection/target reads in `tests/integration/repository-scan.test.ts`
...
**Independent Test**: Verify one physical item/read and two recognitions for root direct-child `.claude/commands/*.md`, Claude-only recognition for nested commands, deterministic namespaces/provenance, filters, exclusions, aliases, injected recoverable environment-resource failures, and rescan cleanup.

**Visible Checkpoint**: Users can distinguish shared root commands from nested Claude-only commands.

### Tests first

- [ ] T475 [US1] Finalize command fixtures for recursive Claude namespaces, root Copilot-compatible commands, nested Claude-only files, duplicate names, secrets, references, aliases, injected recoverable environment-resource failures, and near misses in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T478 [P] [US1] Add failing integration tests for read-once root commands, deterministic recognition/provenance order, aliases, contracted-partial continuity only for deterministic entry-local non-capacity failures, injected recoverable environment-resource failures that abort the attempt, publish no item, recognition, derived result, scan-result record or response, or generation, and retain only the prior committed snapshot, and no referenced-target reads in `tests/integration/repository-scan.test.ts`
...
- [ ] T517 [P] [US2] Add failing inert TOML parsing tests for Codex agent fields; malformed input with deterministic entry-local non-capacity recognition-atomic extraction failure; environment-owned parser capacity with no Inspector-defined numeric cap; and recoverable resource failure that returns no parser, extraction, recognition, item, record, response, or contracted-partial result and propagates `fatal-resource` for the whole scan attempt in `tests/unit/inspection/parsers.test.ts`
...
- [ ] T565 [US1] Finalize all-vendor custom-agent fixtures for every supported path, layer, duplicate name, shared Claude/Copilot file, Claude/Copilot owner-attached MCP declaration, Codex carrier-inheritance relationship, malformed metadata, secret field, reference, exclusion, alias, and injected recoverable environment-resource failure in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T568 [P] [US1] Add failing integration tests for read-once shared agents, deterministic Claude/Copilot agent/MCP recognition and provenance order, Codex relationship-only carrier inheritance, aliases, isolated deterministic entry-local non-capacity failures, injected recoverable environment-resource failures that abort the attempt, publish no item, recognition, derived result, scan-result record or response, or generation, and retain only the prior committed snapshot, zero synthetic files/connections, and zero relationship-target reads in `tests/integration/repository-scan.test.ts`
...
- [ ] T589 [US2] Add failing inert TOML tests for arrays/tables, strict UTF-8, relative-path bases, malformed values with deterministic entry-local non-capacity recognition-atomic extraction failure, environment-owned parser capacity with no Inspector-defined numeric cap, and recoverable resource failure that returns no parser, extraction, recognition, item, record, response, or contracted-partial result and propagates `fatal-resource` for the whole scan attempt in `tests/unit/inspection/parsers.test.ts`
- [ ] T590 [P] [US2] Add failing Codex config tests for root-to-`cwd` layers, closest-value behavior, trust, all configured literal fallback basenames subject only to vendor/runtime and execution-environment capacity, declarations, excluded higher scopes, and a typed resource failure that returns no parser, extraction, config declaration, metadata, recognition, derived, item, scan-result record or response, or contracted-partial output, propagates `fatal-resource` for whole-attempt abort, and retains only the prior committed snapshot in `tests/unit/inspection/codex-metadata.test.ts`
...
- [ ] T612 [US2] Add failing inert JSONC tests for comments, known fields, strict UTF-8, malformed structures with deterministic entry-local non-capacity recognition-atomic extraction failure, environment-owned parser capacity with no Inspector-defined numeric cap, and recoverable resource failure that returns no parser, extraction, recognition, item, record, response, or contracted-partial result and propagates `fatal-resource` for the whole scan attempt in `tests/unit/inspection/parsers.test.ts`
...
- [ ] T646 [P] [US1] Add failing integration tests for read-once shared settings, deterministic settings/MCP recognition and provenance order, hard-link aliases, isolated deterministic entry-local non-capacity failures, injected recoverable environment-resource failures that abort the attempt, publish no item, recognition, derived result, scan-result record or response, or generation, and retain only the prior committed snapshot, no synthetic MCP file or connection, and no configured-target access in `tests/integration/repository-scan.test.ts`
...
**Independent Test**: Open malformed, secret-bearing, and environment-resource-failed catalogs and verify atomic JSON parsing, local source forms, remote/absolute/home/traversal rejection, complete deterministic validated targets per independently admitted static seed, relationship-only components, exact authored-literal preservation, and diagnostics. A deterministic entry-local non-capacity malformed failure may produce only a contracted-partial result after complete traversal; an environment-resource failure aborts the whole attempt, publishes no item, recognition, derived result, generation, record, or response, retains only the prior committed snapshot, and performs zero plugin-target reads.
...
- [ ] T690 [P] [US2] Add failing local-source validation tests for leading-`./` catalog-relative containment, one target per declaration, all distinct validated targets per independently admitted static seed in stable typed-extractor/source-occurrence order, one-edge preparation, Git/HTTP/npm/absolute/home/traversal rejection from derived authority while retaining authored relationships, and environment-resource failure that aborts the attempt, publishes no item, recognition, source plan or other derived result, scan-result record or response, or generation, and retains only the prior committed snapshot in `tests/integration/repository-scan.test.ts`
...
- [ ] T697 [US2] Integrate atomic catalog parsing, complete authored-source retention, all distinct validated local targets per static seed in deterministic order, relationship-only rejected/remote components, environment-resource failure that aborts the attempt, publishes no item, recognition, source plan or other derived result, scan-result record or response, or generation, and retains only the prior committed snapshot, and no derived reads yet in `src/inspection/scan.ts`
...
**Independent Test**: Open malformed, secret-bearing, and environment-resource-failed catalogs and verify optional/local source forms, catalog-relative containment, remote relationship retention, complete deterministic validated targets per independently admitted static seed, owner-attached MCP declarations, registration/activation uncertainty, exact authored-literal preservation, and diagnostics. A deterministic entry-local non-capacity malformed failure may produce only a contracted-partial result after complete traversal; an environment-resource failure aborts the whole attempt, publishes no item, recognition, derived result, generation, record, or response, retains only the prior committed snapshot, and performs zero connection or plugin-target read.
...
- [ ] T710 [P] [US2] Add failing Claude validation tests for leading-`./` catalog-relative containment, one target per declaration, all distinct validated targets per independently admitted static seed in stable typed-extractor/source-occurrence order, exclusion of Git/HTTP/npm/absolute/home/traversal paths from derived authority while retaining authored relationships, and environment-resource failure that aborts the attempt, publishes no item, recognition, source plan or other derived result, scan-result record or response, or generation, and retains only the prior committed snapshot in `tests/integration/repository-scan.test.ts`
...
- [ ] T717 [US2] Integrate Claude catalog parsing with complete authored-source retention, all distinct validated local targets per static seed in deterministic order, environment-resource failure that aborts the attempt, publishes no item, recognition, source plan or other derived result, scan-result record or response, or generation, and retains only the prior committed snapshot, owner-attached MCP with no synthetic file/connection, and relationship-only rejected/remote components without derived reads in `src/inspection/scan.ts`
...
- [ ] T730 [P] [US2] Add failing source-validation tests for `plugins/foo` and `./plugins/foo`, catalog containment, documented four-target order per declaration, one edge, all distinct validated targets per independently admitted static seed in stable typed-extractor/source-occurrence order, exclusion of forbidden source types from derived authority while retaining authored relationships, and environment-resource failure that aborts the attempt, publishes no item, recognition, source plan or other derived result, scan-result record or response, or generation, and retains only the prior committed snapshot in `tests/integration/repository-scan.test.ts`
...
- [ ] T737 [US2] Integrate Copilot catalog parsing with complete authored-source retention, documented four-target order, all distinct validated targets per static seed in deterministic order, environment-resource failure that aborts the attempt, publishes no item, recognition, source plan or other derived result, scan-result record or response, or generation, and retains only the prior committed snapshot, and relationship-only rejected/remote components without derived reads in `src/inspection/scan.ts`
...
**Independent Test**: Verify one physical item/read with three marketplace recognitions plus Claude owner-attached MCP for the shared catalog, deterministic provenance and root-form ordering, no synthetic MCP file or connection, local-source plans, filters, exclusions, injected recoverable environment-resource failures, diagnostics, and rescan cleanup.
...
- [ ] T739 [US1] Finalize marketplace fixtures for every root form, local/remote source, shared triple-recognition file with Claude owner-attached MCP, malformed/secret catalog, alias, exclusion, and injected execution-environment resource failure in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T742 [P] [US1] Add failing integration tests for read-once shared catalogs, deterministic marketplace/MCP recognition and provenance order, aliases, complete per-seed distinct-target handling subject only to execution-environment capacity, resource failure that aborts the attempt, publishes no item, recognition, source plan or other derived result, scan-result record or response, or generation, and retains only the prior committed snapshot, zero synthetic MCP file/connection, and zero plugin-target reads before derivation phases in `tests/integration/repository-scan.test.ts`
...
- [ ] T744 [US1] Complete read-once marketplace physical-file assembly, deterministic multi-tool plus owner-attached MCP provenance, exact authored occurrences, and complete deterministic semantic source-plan retention on success subject to environment-owned capacity with no Inspector-defined numeric source-plan cap; a capacity/resource failure makes the scan attempt fatal, creates no semantic source plan, provenance, recognition, item, generation, record, response, or contracted-partial result, and retains only the prior committed snapshot; preserve no synthetic file and all exclusions in `src/inspection/scan.ts`
...
**Independent Test**: Inventory the authored root manifest and one `.codex-plugin/plugin.json` below each validated `./` local Codex marketplace source; verify one-edge containment, complete deterministic retention subject only to execution-environment capacity, missing target as no candidate, no orphan/remote/escaping/linked candidate, no recursive derivation, and one verified read per physical file. A resource failure aborts the whole attempt, publishes no item, recognition, derived result, generation, record, or response, and retains only the prior committed snapshot.

**Visible Checkpoint**: Users can filter authored Codex plugin manifests with static or marketplace-derived provenance.

### Fixtures and tests first

- [ ] T751 [US1] Create Codex plugin-manifest fixtures for exact root, valid `./` local catalog sources, exact `.codex-plugin/plugin.json` targets, missing and numerous sources, remote/absolute/home/traversal sources, links, aliases, component declarations, near misses, and injected execution-environment resource failure in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T754 [US1] Add failing scan tests for complete deterministic static/derived Codex manifest retention subject only to execution-environment capacity, missing-derived-target/no-candidate handling, containment, links, aliases, read-once assembly, no component reads, and resource failure that aborts the attempt, publishes no manifest item, recognition, provenance or other derived result, scan-result record or response, or generation, and retains only the prior committed snapshot in `tests/integration/repository-scan.test.ts`
...
- [ ] T761 [US1] Integrate deterministic one-edge Codex manifest admission, one verified read, and alias aggregation; a recoverable environment-resource failure makes the scan attempt fatal, publishes no manifest item, recognition, provenance, derived result, generation, record, response, or contracted-partial result, and retains only the prior committed snapshot; expose only a source-value-free lifecycle diagnostic in `src/inspection/scan.ts`
...
- [ ] T774 [US1] Create Claude plugin-manifest fixtures for exact root, valid local catalog sources, optional absence, numerous sources, ancestor near misses, links, aliases, components, forbidden sources, and injected execution-environment resource failure in `tests/fixtures/repositories/build-fixtures.ts`
...
- [ ] T777 [US1] Add failing scan tests for complete deterministic static/derived Claude manifest retention subject only to execution-environment capacity, containment, links, aliases, read-once assembly, no component reads, and resource failure that aborts the attempt, publishes no manifest item, recognition, provenance or other derived result, scan-result record or response, or generation, and retains only the prior committed snapshot in `tests/integration/repository-scan.test.ts`
...
- [ ] T784 [US1] Integrate deterministic Claude manifest admission, one verified read, aliases, and optional absence; a recoverable environment-resource failure makes the scan attempt fatal, publishes no manifest item, recognition, provenance, derived result, generation, record, response, or contracted-partial result, and retains only the prior committed snapshot; expose only a source-value-free lifecycle diagnostic in `src/inspection/scan.ts`
...
- [ ] T799 [US1] Add failing plugin matcher/derivation and registry tests for four root forms, `plugins/foo`/`./plugins/foo`, documented four-target order, direct one-edge derivation over all validated targets subject only to execution-environment capacity, forbidden source forms, shared recognition, `copilot.excluded.cli-extensions` → `copilot.behavior.cli.extensions`, no extension-as-plugin candidate, and resource failure that returns no derivation program, plan, candidate, manifest, or other derived output, propagates typed `fatal-resource`, aborts the attempt with no item, recognition, scan-result record or response, or generation, and retains only the prior committed snapshot in `tests/unit/inspection/rules.test.ts`, `tests/integration/repository-scan.test.ts`, and `tests/contract/inspection-rules.test.ts`
...
- [ ] T805 [US1] Implement `copilot.derived.local-plugin-manifest` for documented local forms, four-target order, direct one-edge derivation over all validated targets subject only to execution-environment capacity, containment, and forbidden-source rejection; a resource failure must return no derivation program, plan, candidate, manifest, or other derived output and propagate a typed `fatal-resource` signal for the whole scan attempt in `src/inspection/rules/copilot.ts`
...
- [ ] T807 [US1] Integrate deterministic Copilot manifest admission, one verified read, aliases, and complete environment-supported handling on success; a recoverable resource failure makes the scan attempt fatal, publishes no manifest item, recognition, provenance, derived result, generation, record, response, or contracted-partial result, and retains only the prior committed snapshot; expose only a source-value-free lifecycle diagnostic in `src/inspection/scan.ts`
...
- [ ] T821 [P] [US1] Add local-manifest integration regressions for complete literal derived metadata availability, catalog-relative provenance, complete deterministic target retention subject only to execution-environment capacity, resource failure that aborts the attempt, publishes no manifest item, recognition, provenance or other derived result, scan-result record or response, or generation, and retains only the prior committed snapshot, read-once shared files, Claude owner-attached MCP, zero synthetic file/connection, and no component expansion in `tests/integration/repository-scan.test.ts`
- [ ] T822 [US1] Add browser acceptance for unified plugin-manifest inventory, filters, plan-driven derivation, shared recognition, Claude owner-attached MCP versus component paths, exclusions, environment-resource diagnostics, and keyboard use in `tests/e2e/plugin-manifests-inventory.spec.ts`
...
- [ ] T903 [P] [US1] Add failing integration tests for read-once shared owners, deterministic Hook recognition order, aliases, atomic continuity, contracted-partial publication only after complete traversal and deterministic entry-local non-capacity failure, recoverable environment-resource failure that makes the whole attempt fatal, performs zero referenced-hook or later reads, publishes no new Hook, recognition, item, generation, record, response, or contracted-partial result, and retains only the prior committed snapshot in `tests/integration/repository-scan.test.ts`
...
- [ ] T905 [US1] Complete read-once owner/file assembly, deterministic Hook recognition/provenance order, and no synthetic files; a recoverable environment-resource failure makes the whole scan attempt fatal, publishes no new Hook, recognition, provenance, item, generation, record, response, or contracted-partial result, and retains only the prior committed snapshot; expose only a source-value-free lifecycle diagnostic in `src/inspection/scan.ts`
...
- [ ] T915 [US1] Add integration tests that inject recoverable Node.js, parser, OS, filesystem, and execution-environment resource failures during enumeration, reading, alias handling, and publication; verify that each failure aborts the attempt, publishes no item, recognition, derived result, scan-result record or response, or generation, and retains only the prior committed snapshot; also verify no product-defined numeric size/count/depth/time/concurrency ceiling, no valid/invalid, correctness/compliance, or lint verdict from resource availability, responsive API behavior, authority revocation with discarded late results, and no hard-kernel-cancellation claim in `tests/integration/runtime-failures.test.ts`
- [ ] T916 [P] [US1] Add complete session/rescan API contracts for every Repository kind, strict envelopes without source text/metadata in inventory, one `scanRequestId` across `ScanAdmission`/Source/progress/final state/successful generation, per-Source stale failures, conflicts, stale IDs, atomic publication, capability-only host authorization with no acknowledgement field, source-value-free failures, path-free OperationalEvent capture, and negative DTO assertions against validation/lint/semantic-ranking/verdict/remediation fields in `tests/contract/http-api-session.test.ts`
- [ ] T917 [P] [US1] Add complete packaged Gunshi CLI tests for isolated install, fixed assets/Worker, same tarball, launch `cwd`, default-browser delegation without version-certification claims, `--no-open` plus printed-URL certified-browser fallback, non-binding help/version, strict unknown-option and positional/rest rejection with fixed source-value-free nonzero failures, awaited shutdown, root-only import boundary, and zero extra modes in `tests/package/npx-launch.test.ts`
...
- [ ] T921 [P] [US2] Add four-parser matrix tests for JSONC, YAML, TOML, Markdown/frontmatter, strict encodings, atomic extraction, Worker replacement, environment-owned capacity, and typed recoverable resource failures that return no parser, extraction, recognition, derived, item, scan-result record or response, or contracted-partial output, propagate `fatal-resource` for whole-attempt abort, and retain only the prior committed snapshot in `tests/unit/inspection/parsers.test.ts` and `tests/unit/inspection/seed-parsers.test.ts`
- [ ] T922 [US2] Add environment-resource failure tests for relationships, provenances, derivations, fallbacks, exact source occurrences, complete authored text, parser messages, retained graphs, and file details; verify the attempt aborts atomically, publishes no item, recognition, derived result, scan-result record or response, or generation, retains only the prior committed snapshot, and produces no validity classification in `tests/integration/runtime-failures.test.ts`
- [ ] T923 [US2] Add environment-resource failure tests for `SourceConditionFact`, `ApplicabilityAssessment`, and Diagnostic records, verifying that the failure aborts the attempt, publishes no item, recognition, `SourceConditionFact`, `ApplicabilityAssessment`, result Diagnostic or other derived result, scan-result record or response, or generation, retains only the prior committed snapshot with unaffected previously committed Source continuity, reports only a source-value-free lifecycle failure, and introduces no product-defined numeric caps in `tests/integration/runtime-failures.test.ts`
...
- [ ] T925 [P] [US2] Extend zero-activation regression across every Repository family under `--no-open` or post-helper instrumentation, proving discovery/read/parse/display/comparison/relationship processing causes zero child/evaluation/MCP/network/URI/image/mutation/reference reads; capture operational output and require only fixed codes/opaque IDs with zero paths, roots, filenames, inspected content/metadata, authored values, capabilities, bodies, raw errors, exception strings, or Diagnostic arguments in `tests/integration/security/zero-activation.test.ts`
- [ ] T926 [P] [US2] Add complete file-detail and removed-reveal-route API contracts for every kind, exact authored literals, source-form allowlist enforcement, unknown-key retention only in complete source text, strict envelopes, stale IDs, no-store, and capability-only authorization with no acknowledgement field/route; prove that a pre-commit environment-resource encoding failure is fatal to the current request, creates no new result or generation, and publishes zero FileDetail success payloads, while a post-commit delivery failure leaves the committed snapshot unchanged, publishes zero successful response payloads, is never represented as partial, and produces no validity classification; also enforce cross-surface rejection of natural-language interpretation/ranking, customization validity/correctness/compliance/effectiveness/quality verdicts, validation/lint/remediation/fix output in `tests/contract/http-api-files.test.ts`
- [ ] T927 [US2] Add Repository-complete browser acceptance plus the documented inspection-safety target for memory-only presentation acknowledgement before detail requests, capability-only host authorization, complete literal detail, exact metadata/relationships, literal credentials/environment references, and no masking/reveal; verify malformed or boundary-local deterministic non-capacity failure may produce contracted-partial diagnostics only after complete traversal, while recoverable environment-resource failure makes the attempt fatal, publishes no new complete or contracted-partial generation, item, record, response, or derived result, and leaves only the prior committed snapshot visible; verify separately that an explicit fatal rescan marks only its Source stale, whereas an automatic first Repository failure creates no stale overlay and preserves any pre-existing snapshot state; also verify no validation/lint/verdict, product-wide non-analysis/remediation controls, stale routes, and zero executable rendering in `tests/e2e/repository-complete-detail.spec.ts` and `tests/e2e/inspection-safety.spec.ts`
...
- [ ] T929 [US3] Add Repository-complete browser acceptance plus the documented comparison target for literal comparison and typed structural differences only; assert no semantic ranking, merge, validation, lint, content verdict, policy/remediation, synchronization, conversion, formatting, or fix suggestion; also cover late-owner MCP selection through real owner IDs, runtime-only/dormant rejection, fallback behavior, accessibility, and lifecycle cleanup in `tests/e2e/repository-complete-comparison.spec.ts` and `tests/e2e/comparison.spec.ts`
...
**Independent Test**: Use isolated environment inputs and fake homes to verify zero proposed-path I/O, exact three-tool preview entries, complete environment-supported escaping with no partial preview on resource failure, invalid overrides, versioned digest binding, stale/replayed rejection, accessible bilingual review, and one-time non-authorizing ownership of `codex.behavior.user.memories`, `codex.behavior.user.prompts`, and `claude.behavior.user.workflows`.

**Visible Checkpoint**: Users can review the exact Global roots, patterns, exclusions, environment-resource status, and contract version before enabling inspection.

### Fixtures and tests first

- [ ] T930 [US4] Create isolated Global-home fixtures for exact candidates, exclusions, fallback, invalid overrides, links, aliases, unreadable roots, injected Node.js/OS/filesystem resource failures, distinct literal credentials/environment references, sentinel process values, inert executable-looking payloads, and before/after content/length/identity/link/mode/mtime/ctime/xattr/ACL observations with OS-only atime recorded separately; document that Inspector defines no file-size/count validation and resource availability never yields validation/lint/verdict in bilingual guidance at `tests/fixtures/global-homes/build-fixtures.ts`, `tests/fixtures/global-homes/README.md`, and `tests/fixtures/global-homes/README.ja.md`
...
- [ ] T932 [P] [US4] Add failing preview tests for zero filesystem I/O, absent versus invalid overrides, exact lexical roots, complete environment-supported escaping, recoverable environment-resource failure that returns only a closed typed failure with zero complete or partial preview DTO, leaves stored preview/digest/consent/control state unchanged, and grants no path authority, and the exact minimal preview shape in `tests/unit/host/global-consent.test.ts`
...
- [ ] T934 [P] [US4] Add failing `GET /api/v1/global/consent-preview` contracts for the exact preview shape and statuses; require environment-resource failure to return only a closed failure status with zero complete or partial preview success-payload bytes, leave stored preview/digest/consent/control state unchanged, grant no client path authority, preserve no-store behavior, and perform zero proposed-root I/O in `tests/contract/http-api-global.test.ts`
- [ ] T935 [US4] Add failing browser acceptance for bilingual roots, patterns, states, exclusions, environment-resource errors, keyboard review, and zero pre-consent source results or enable requests in `tests/e2e/global-consent-preview.spec.ts`
...
- [ ] T938 [US4] Implement complete environment/default-home preview construction and streaming escaping with capacity determined by the execution environment; a recoverable failure must return only a closed typed failure, create no complete or partial preview DTO or digest, leave stored preview/consent/control state unchanged, and grant no path authority, without filesystem access, normalization, or root creation in `src/host/global-consent.ts`
...
- [ ] T940 [US4] Implement only the strict `GET /api/v1/global/consent-preview` handler with exact response and statuses; an environment-resource failure must return only a closed failure status with zero complete or partial preview success-payload bytes, leave stored preview/digest/consent/control state unchanged, preserve no-store behavior, and grant no client path authority in `src/host/api-router.ts`
- [ ] T941 [US4] Implement accessible preview presentation for exact roots, patterns, states, exclusions, version, and environment-resource errors without Inspector-defined capacity fields or values in `app/components/consent/GlobalConsentPreview.vue`
...
- [ ] T943 [US4] Add semantically equivalent English/Japanese Global preview, environment-resource failure, override, digest, and consent messages in `app/locales/en.ts` and `app/locales/ja.ts`

---

## Phase 96: Codex Global Boundary Admission and Enable Foundation

**Purpose**: Validate the exact stored preview through the selector-free enable endpoint, establish the serialized enable and tool-control transitionss, and admit a Codex root for a provisional tool-specific scan without publishing a Source before commit.

**Independent Test**: Submit the exact preview-bound body with no tool selector, reject false/stale/mismatched or extra-key requests, serialize the complete derived work set before mutation, and verify Codex-only fixtures create active consent, one Codex `GlobalToolControl`, and one provisional scan job but no public Source before a complete or contracted-partial commit; verify exact accepted/rejected partitioning, disable-race disposition, retryable failure state, Repository preservation, and ownership of exactly `codex.excluded.user-runtime`.
...
- [ ] T944 [P] [US4] Add failing Codex post-consent tests for component-identical canonical roots, links, junctions, case/Unicode/short-name aliases, invalid overrides, and exhaustive first-non-empty traces: non-empty override short-circuits every regular-file operation; only exact not-found or safely proven empty/BOM-only/whitespace-only advances; unsafe, unreadable, undecodable, non-regular, or disappearing overrides fail without fallback; an environment-resource-failed override must additionally return no fallback, root, candidate, traversal plan, read authority, provisional job, or derived result and propagate a typed `fatal-resource` signal for the whole attempt in `tests/unit/host/global-consent.test.ts`
...
- [ ] T946 [P] [US4] Add failing initial-enable coordinator tests for serialized all-or-none state mutation across all confirmed tools; operation ID/epoch checks; FIFO generations; duplicate conflicts; zero provisional Source; environment-resource failure that aborts the whole attempt before publication, commits no item, Source, scan-result record or response, or generation, retains only the prior committed snapshot without creating a stale overlay, and leaves consent/control/root-context state intact; publication-authority revocation with late-result discard after disable/shutdown/supersession; and one `202` versus `409 global-disable-pending` linearization in `tests/unit/session/coordinator.test.ts`
...
- [ ] T957 [US4] Implement an isolated Codex provisional scan job with exact fallback behavior, source-value-free diagnostics, one admitted root, and zero public Global Source or graph until that job's complete/contracted-partial atomic commit in `src/inspection/scan.ts`
- [ ] T958 [US4] Implement selector-free initial enable/exact-consent retry with all-eligible/missing-tool derivation, serialized all-or-none state mutation, active consent/per-tool controls, `scanRequestId`-bearing provisional jobs, FIFO dequeue snapshots, accepted/rejected partitioning, `active-no-job`, zero pre-commit Source, environment-resource failure that aborts the whole attempt, commits no item, Source, scan-result record or response, or generation, retains only the prior committed snapshot without creating a stale overlay, and leaves consent/control/root-context state intact, plus publication-authority revocation with cleanup-only late-result discard after disable/shutdown/supersession in `src/session/session.ts` and `src/session/scan-generation.ts`
...
- [ ] T975 [US4] Implement independent Claude control/context/job progress, rejection and retry state, serialized state transitions, environment-resource failure that aborts the attempt with no item, scan-result record or response, or generation and retains only the prior committed snapshot, late-result discard, and preservation of consent, sibling controls/Sources, and prior committed graphs in `src/session/session.ts`
...
- [ ] T988 [US4] Implement isolated Copilot provisional scanning, instruction-subtree-only traversal, sibling-safe diagnostics, exactly one root, and zero Copilot Source publication until that scan's own complete/contracted-partial commit in `src/inspection/scan.ts`
- [ ] T989 [US4] Implement independent Copilot control/context/job progress, rejection and retry state, serialized state transitions, environment-resource failure that aborts the attempt with no item, scan-result record or response, or generation and retains only the prior committed snapshot, late-result discard, and preservation of consent, sibling controls/Sources, and prior committed graphs in `src/session/session.ts`
...
- [ ] T993 [P] [US4] Add coordinator tests for independent per-tool atomic Source publication, stable `Source.sourceId`, one-root/tool invariants, session-wide carried-Source generation rekeying, sibling failure, contract-defined atomic contracted-partial commits, missing-tool no-Source retry state, progress, and duplicate conflicts in `tests/unit/session/coordinator.test.ts`
- [ ] T994 [P] [US4] Add lifecycle tests proving Repository and all tool-specific Global `Source.sourceId` values remain process-lifetime stable and each carried Repository semantic inventory and authored source content remains unchanged while file, recognition, provenance, relationship, diagnostic, and related generation-owned IDs rekey, with stale FileDetail/comparison/Monaco cleanup and no provisional context or pending-admission leakage in `tests/integration/session-lifecycle.test.ts`
- [ ] T995 [P] [US4] Add failing exact-display API/integration tests for Global fixtures containing literal credentials, environment references, process sentinels, inert executable-looking payloads, injected execution-environment resource failures, and mutation observations; prove source/detail/comparison/API preserve exact authored text without process substitution, recoverable resource failures abort the attempt, publish no item, recognition, derived result, scan-result record or response, or generation, retain only the prior committed snapshot, publish no validity/lint verdict, and leave content/length/identity/link/mode/mtime/ctime/xattr/ACL unchanged while OS-only atime is recorded separately in `tests/contract/http-api-files.test.ts` and `tests/integration/global-literal-display.test.ts`
- [ ] T996 [P] [US4] Add failing zero-activation security tests proving Global fixtures cause zero dynamic evaluation, command/hook execution, browser-helper launch, MCP/network, environment substitution, or mutation-capable filesystem call; require path/content-free fixed-code/opaque-ID operational output, and prove Source Condition Facts cause zero local/hosted I/O and no file/relationship/comparison identity in `tests/security/global-zero-activation.test.ts` and `tests/integration/source-condition-facts.test.ts`
- [ ] T997 [US4] Add browser acceptance for exact-preview enablement, separate one-root Sources, filters, diagnostics, exact literal credential/environment-reference display, no activation/substitution, recoverable environment-resource failures that abort the attempt with no item, scan-result record or response, or generation, retain only the prior committed snapshot without creating a `StaleSourceFailure` or changing any pre-existing snapshot state, and publish no validation/lint/verdict, Source Condition Fact separation, Global detail/comparison reuse, product-wide absence of semantic interpretation/ranking or remediation controls, retry/fatal retention, and Repository semantic preservation with stable `sourceId` despite generation-wide rekey/invalidation in `tests/e2e/global-enable.spec.ts`

### Implementation

- [ ] T998 [US4] Finalize post-consent per-tool admission as independent one-root `GlobalToolControl` contexts and provisional jobs, with all-eligible initial confirmation, missing-tool-only retry, and sibling retention in `src/host/global-consent.ts`
...
- [ ] T1000 [US4] Implement integrated but isolated per-tool Global scanning, sibling continuation, Codex fallback, and independent contract-defined atomic complete or contracted-partial results for separate one-root Sources while preserving Repository and other tool results in `src/inspection/scan.ts`
- [ ] T1001 [US4] Atomically publish or replace one tool-specific Global Source only when its own scan commits a complete result or a contracted-partial result after complete traversal and a deterministic entry-local non-capacity failure; carry Repository and sibling Sources' semantic inventory and authored source content unchanged into the mandatory new session generation, preserve all Source IDs, rekey every generation-owned graph ID, invalidate old detail/comparison/editor state, clear that tool's transient work, and on fatal or all-rejected outcomes commit no missing Source/generation while retaining the exact contracted consent/control retry state and prior graphs in `src/session/session.ts` and `src/session/scan-generation.ts`
- [ ] T1002 [US4] Complete `POST /api/v1/global/enable` responses for exact accepted/rejected tool partitions, queued/active-no-job outcomes, sibling diagnostics, conflicts, and retry state while leaving Source publication to later session polls after per-tool commits in `src/host/api-router.ts`
- [ ] T1003 [US4] Implement Repository plus separately identified Codex/Claude/Copilot Global Source and tool filters, one-root summaries, and shared detail/comparison navigation in `app/composables/filters.ts`, `app/composables/session.ts`, and `app/pages/index.vue`
- [ ] T1004 [US4] Complete per-tool Global confirmation/progress/retry controls, focus recovery, active-no-job handling, and separate Source outcome presentation in `app/pages/global-consent.vue` and `app/components/consent/GlobalSourceControls.vue`; reuse the T071 pause/resume and on-demand-refresh contract for every automatically updating Global status presented beside other content
- [ ] T1005 [US4] Add semantically equivalent English/Japanese tool-specific Source integration, one-root, accepted/rejected, active-no-job, sibling-failure, retry, source/tool-filter, detail/comparison, and progress messages in `app/locales/en.ts` and `app/locales/ja.ts`
...
- [ ] T1006 [US4] Add failing coordinator tests for serialized cross-source FIFO processing, dequeue-time generations, one `scanRequestId` across admission/progress/final status/commit, duplicate conflicts, fatal retention, and per-job counters; require every execution-environment capacity failure to abort the attempt, publish no item, recognition, derived result, scan-result record or response, or generation, and retain only the prior committed snapshot in `tests/unit/session/coordinator.test.ts`
- [ ] T1007 [US4] Extend coordinator tests for carried-Source graph construction, lifecycle/control state, serialized state transitions, exactly-once resource cleanup, publication-authority revocation after disable/shutdown/supersession with pending filesystem work treated as cleanup-only, late discard, no later source I/O, responsive API, and no hard-cancellation assertion in `tests/unit/session/coordinator.test.ts`
- [ ] T1008 [P] [US4] Add failing `POST /api/v1/global/rescan` contracts for strict `sourceId`, `ScanAdmission { scanRequestId, source }`, same-ID waiting/active/final status and successful generation, one identified Global Source, unknown/removed Source, disable-pending/duplicate conflicts, environment-resource failure that returns only path-free lifecycle status, aborts the attempt with no item, scan-result response or record, or generation, and retains only the stale prior committed snapshot, fatal retry, stale IDs, and rejection of older request state as completion in `tests/contract/http-api-global.test.ts`
...
- [ ] T1010 [P] [US4] Add lifecycle tests proving every complete or contracted-partial tool-specific Global commit preserves all Repository/Global `Source.sourceId` values and all carried semantic inventory/authored source content, advances exactly one generation, rekeys carried and replaced file/recognition/provenance/relationship/diagnostic graph IDs, clears only that Source's stale failure, preserves sibling failures, and invalidates old FileDetail/comparison/Monaco state; also prove an all-rejected enable/retry commits no generation and changes no carried ID in `tests/integration/session-lifecycle.test.ts`
...
- [ ] T1013 [US4] Implement serialized carried-source generation construction and per-job counters; capacity is determined by the execution environment, and a recoverable resource failure aborts the attempt, publishes no item, recognition, derived result, scan-result record or response, or generation, and retains only the prior committed snapshot; disable/shutdown/supersession revokes publication authority, retains pending work only for cleanup, discards late results, and releases owned resources once in `src/session/session.ts` and `src/session/scan-generation.ts`
- [ ] T1014 [US4] Implement strict `POST /api/v1/global/rescan` handling for one opaque `sourceId`, returning `ScanAdmission { scanRequestId, source }` and preserving that ID through progress/final status/commit; enforce disable/duplicate conflicts, environment-resource failure that returns only path-free lifecycle status, aborts the attempt with no item, scan-result response or record, or generation, and retains only the stale prior committed snapshot, fatal retry, stale-resource responses, and path-free operational output in `src/host/api-router.ts`
...
- [ ] T1017 [US4] Add failing coordinator tests for a serialized priority disable barrier, publication-authority revocation, zero cancellation diagnostics, Repository requeue-once, N+1 retained-Repository rekeying with stable `sourceId`, stale IDs, joined/no-op barriers, timestamps, and pending uncancellable Global filesystem work that drains cleanup-only before physical-completion claims or exactly-once resource release in `tests/unit/session/coordinator.test.ts`
- [ ] T1018 [P] [US4] Add failing `POST /api/v1/global/disable` contracts for empty bodies, cancelling progress, joined completion, idempotent no-op, one removal commit, and `200` responses in `tests/contract/http-api-global.test.ts`
- [ ] T1019 [P] [US4] Add concurrency tests for interrupted Repository work, interrupted Global work, queued Global cancellation, joined disable, requeue-once, and empty no-op behavior in `tests/integration/global-concurrency.test.ts`
- [ ] T1020 [P] [US4] Add boundary instrumentation proving Global disable performs zero filesystem enumeration or reads and emits zero barrier-cancellation diagnostics in `tests/integration/global-boundaries.test.ts`
- [ ] T1021 [P] [US4] Add lifecycle tests for authority revocation and cleanup-only drain of pending Global filesystem work, then removal of every Global Source/file/graph ID, diagnostics, authored/detail state, comparisons, Monaco state, consent, controls, preview, root contexts, and handles; prove no late byte/DTO/Diagnostic/log result publishes and each owned operation/handle resource releases once in `tests/integration/session-lifecycle.test.ts`
- [ ] T1022 [US4] Add browser acceptance plus the documented Global-consent command target for the complete preview/enable/rescan/disable workflow, disable progress across all tool-specific Sources, enable/disable race outcomes, joined/no-op requests, focus restoration, Global route/editor/detail teardown, control/diagnostic removal, and retained Repository results in `tests/e2e/global-disable.spec.ts` and `tests/e2e/global-consent.spec.ts`

### Implementation

- [ ] T1023 [US4] Implement the serialized priority zero-I/O barrier, immediate publication-authority revocation, cleanup-only drain of uncancellable work without diagnostics or late mutation, queued-Global discard, Repository requeue-once with its original `scanRequestId`, joined barriers, and no-op detection in `src/session/session.ts`
- [ ] T1024 [US4] Implement the N+1 zero-I/O commit after required physical-handle drain, removing all Global Sources/graphs/authored content/consent/controls/preview/root contexts/diagnostics/details/comparisons and releasing every retained resource once; preserve Repository `sourceId`, rekey its generation-owned graph IDs, and stale all prior IDs without claiming publication-authority revocation physically cancelled kernel I/O in `src/session/session.ts`, `src/session/stale-failures.ts`, and `src/session/scan-generation.ts`
- [ ] T1025 [US4] Implement strict `POST /api/v1/global/disable` handling with empty-body validation, cancelling progress, joined completion, no-op behavior, and one removal commit in `src/host/api-router.ts`
- [ ] T1026 [US4] Implement disable loading, joined/no-op handling, focus restoration, and Global route/editor/model cleanup in `app/pages/global-consent.vue`, `app/components/consent/GlobalSourceControls.vue`, and `app/composables/session.ts`
- [ ] T1027 [US4] Remove every Global tool/Source filter, selection, diagnostic, recovery, FileDetail, comparison, Monaco model/worker/subscription, and cached DTO state after the committed barrier while retaining the authorized-session acknowledgement until a liveness purge in `app/composables/filters.ts`, `app/composables/liveness.ts`, `app/composables/comparison.ts`, and `app/composables/monaco.ts`
- [ ] T1028 [US4] Add semantically equivalent English/Japanese Global barrier, cancellation, disable, no-op, removal, and Repository-preservation messages in `app/locales/en.ts` and `app/locales/ja.ts`
...
- [ ] T1029 Draft semantically equivalent operational guidance in `README.md`/`README.ja.md` covering verified launch; Node engines/build baseline/finite lower-bound certification; browser fallback; exact Repository/one-root Global allowlists; relationship/read boundaries; Source Condition Facts; Diagnostic scopes versus path-free fixed-code/opaque-ID operational events; literal display and client-only acknowledgement with capability-only API authorization; the product-wide syntactic/literal/typed/catalog/structural-only boundary and absence of semantic ranking, validation/lint/verdict/policy/remediation/sync/convert/format/fix features; the absence of Inspector-defined numeric size/count/parser/request/response/queue/time/concurrency caps; environment-owned capacity and recoverable resource failure that aborts its attempt with no item, scan-result record or response, or generation, retains only the prior committed snapshot, and produces no validity classification; `scanRequestId`-bound SC-002 explicit-rescan protocol; mutation/OS-atime evidence; residual uncancellable I/O risk; privacy/exclusions/maintenance; and the bilingual 55-row WCAG contract
- [ ] T1030 Create semantically equivalent SC-001/SC-006 study kits with the maintainer-owned bilingual plan naming study, recruitment/compensation-funding, moderation/review, support, privacy/retention, equipment/session, and accessibility owners; the fixed twenty-participant order; recording of the actual OS default handler or its unavailability and, when resolvable, the actual browser family/revision; mandatory use and recording of the printed-URL manual fallback in a pinned certified browser when automatic opening is disabled, unsupported, fails, is unidentifiable, unavailable, or resolves outside the certification baseline; exact timers, prepared state, four-field ground truth, no-hints and no-replacement rules, all four workflow observations, automatic critical safety events, and two independent fixed-rubric classifications only for suspected non-safety product blockers, with disagreement counted as critical and no third adjudicator; state that ordinary contributors do not operate the study, missing resources block the release claim rather than contribution review, and rerun is required only after a material workflow/guidance/fixture/rubric change in `tests/usability/sc001-sc006-study-kit.md` and `tests/usability/sc001-sc006-study-kit.ja.md`

### Official evidence and dependency review

- [ ] T1031 Add failing official-source checker contracts for exact hosts, redirect rejection, explicit network opt-in, complete environment-supported content retrieval, recoverable network/runtime resource failure with no partial update, and non-mutating drift reporting in `tests/contract/official-source-drift.test.ts`
...
- [ ] T1033 Resolve accepted source or section drift in `specs/001-inspect-agent-customizations/contracts/official-sources.md`, `specs/001-inspect-agent-customizations/contracts/official-sources.ja.md`, `specs/001-inspect-agent-customizations/contracts/runtime-composition.md`, and `specs/001-inspect-agent-customizations/contracts/runtime-composition.ja.md`
- [ ] T1034 [P] Review the Setup-frozen exact closed metadata-field, relationship-kind, admitted-source-form, and exact-extractor-applicability Presentation Allowlist for every supported Copilot customization type against accepted evidence drift, and update it only through explicit review without automatic scope expansion in `specs/001-inspect-agent-customizations/contracts/vendors/github-copilot.md` and `specs/001-inspect-agent-customizations/contracts/vendors/github-copilot.ja.md`
- [ ] T1035 [P] Review the Setup-frozen exact closed metadata-field, relationship-kind, admitted-source-form, and exact-extractor-applicability Presentation Allowlist for every supported Claude customization type against accepted evidence drift, and update it only through explicit review without automatic scope expansion in `specs/001-inspect-agent-customizations/contracts/vendors/claude-code.md` and `specs/001-inspect-agent-customizations/contracts/vendors/claude-code.ja.md`
- [ ] T1036 [P] Review the Setup-frozen exact closed metadata-field, relationship-kind, admitted-source-form, and exact-extractor-applicability Presentation Allowlist for every supported Codex customization type against accepted evidence drift, and update it only through explicit review without automatic scope expansion in `specs/001-inspect-agent-customizations/contracts/vendors/openai-codex.md` and `specs/001-inspect-agent-customizations/contracts/vendors/openai-codex.ja.md`
- [ ] T1037 Enforce the semantic drift gate on accepted T1033–T1036 changes before touching production registries or continuing to any later task under the current IDs. If changes are limited to evidence location, section anchor, review metadata, or a semantically unchanged correction, apply only those explicitly reviewed updates to `shared/registries/vendor-behaviors.ts`, `shared/registries/inspection-rules.ts`, `shared/registries/runtime-composition.ts`, and `shared/registries/official-sources.ts` and continue. If any accepted change alters normative behavior, inspection rules, runtime strategies, Presentation Allowlist membership/source-form applicability, registry shape, or conformance expectations, mark the remaining English/Japanese task set superseded, stop before production-registry mutation, synchronize English/Japanese spec, research, plan, quickstart, and contracts, rerun planning and task generation, and continue only from the regenerated task set
...
- [ ] T1040 Review `pnpm outdated`, licenses, notices, compatible-version rationale, and public-contract effects and record every accept/reject decision in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`; if no change is accepted, record the baseline unchanged and continue, but if any change is accepted, mark the current `tasks.md`/`tasks.ja.md` superseded, stop before editing package/configuration or executing any later old task ID, synchronize the affected English/Japanese research, plan, and quickstart artifacts, rerun `/speckit-plan` then `/speckit-tasks`, and apply/verify the change only from the regenerated task set
...
- [ ] T1041 Add and run one cross-artifact documentation/traceability contract suite in `tests/contract/documentation.test.ts` for reciprocal bilingual links/parity; runnable commands/stable IDs; all 54 FR/QR/SC rows with task references and every task mapped; exact README path/relationship boundaries; presentation allowlist parity; Diagnostic scopes separated from a path/content-free fixed-code/opaque-ID OperationalEvent schema; literal display, client-only acknowledgement/capability-only API, Source Condition Fact isolation, and one-root Sources; negative cross-surface coverage proving Inventory/Detail/Comparison/Global/Diagnostics/Facts/API/CLI/docs expose no natural-language ranking, validity/correctness/compliance/effectiveness/quality verdict, validation/lint/policy/remediation/sync/convert/format/fix behavior; no Inspector-defined numeric resource caps or size-derived validity; environment-owned capacity where a resource failure aborts its attempt, publishes no item, Source, recognition, derived result, scan-result record or response, or generation, and retains only the prior committed snapshot; contracted-partial publication only for deterministic entry-local non-capacity failures after complete traversal; atomic failure and residual cleanup wording; mutation identity/atime evidence; finite engine certification; request-ID-bound SC-002 profile/results; all 55 WCAG rows and closed manual matrix; official backlinks; and absence of stale native claims. Any suite failure or unresolved concern leaves T1041 incomplete and stops T1042 or any later task under the current task IDs until it is resolved
- [ ] T1042 [P] Materialize exactly 47 source records and add final full-registry tests for exactly 56 inspection-rule IDs—35 Repository static, five bounded-derived, ten vendor-excluded, two shared-excluded, and four Global static—plus 39 strategies, 14 relationship-only rules, zero contained Hook/MCP additions, exact presentation parity/reciprocity/source bounds, and Source Condition Facts with zero authority/I/O; additionally prove registry/runtime DTO vocabularies permit only documented syntactic/literal/typed/catalog/structural projection and cannot encode customization validity, semantic ranking, policy/remediation, validation/lint, synchronization, conversion, formatting, or fixing in `tests/fixtures/conformance/official-sources.json`, `tests/contract/inspection-rules.test.ts`, `tests/contract/runtime-composition.test.ts`, and `tests/contract/official-sources.test.ts`
...
- [ ] T1045 [P] Add cross-story axe, keyboard, forced-colors, zoom/reflow, reduced-motion, focus-retention, safe-error, sensitive-content acknowledgement, and liveness purge/resume regressions mapped to the complete bilingual 55-row WCAG Level A/AA acceptance matrix; name and report each test with its exact `AUTO-*` ID, including `AUTO-2.2.2` coverage of pause/resume and on-demand refresh for automatically updating status, and require every named automated check for every Applicable row plus all four keyboard workflows to pass in the pinned Chromium, Firefox, and WebKit projects, without treating an axe severity label as an acceptance escape or those revisions as the exhaustive user-browser list, in `tests/e2e/accessibility.spec.ts` and `tests/e2e/session-liveness.spec.ts`
- [ ] T1046 [P] Add snapshot regressions for complete lifecycle Diagnostic, control/progress, SessionSnapshot, and FileDetail envelopes; canonical production encoding; the same unchanged HTTP buffer and exact `Content-Length`; pre-commit environment-resource encoding failure that aborts the attempt, creates no new item, scan-result record or response, or generation, retains only the prior committed snapshot, and publishes zero success payloads; post-commit delivery failure that leaves the committed snapshot unchanged, publishes zero successful response payloads, and is never represented as partial; and paused delivery across rescan/Global-disable commits without mixing each coordinator-captured generation and payload in `tests/integration/session-snapshot-encoding.test.ts`, `tests/contract/http-api-session.test.ts`, and `tests/contract/http-api-files.test.ts`
...
- [ ] T1051 Run the exact six lower-bound certification jobs—Node.js `24.11.0`/`26.0.0` by `ubuntu-24.04` x64/`macos-15` arm64/`windows-2025` x64—against identical tarball bytes, verify the packed full Node.js 24/26 engines contract and out-of-range rejection, and record runner identifiers, actual versions, production-graph digest agreement, shim audit, detectable unsafe-state rejection, `safe-fs-boundary-unverifiable`, effective `O_NOFOLLOW`, and non-proving `platform-unobservable` outcomes in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1052 Validate the checked-in SC-002 profile/fixture, then record exactly ten fresh-process runs: wait for each automatic first Repository scan outside timing, dispatch exactly one explicit rescan, capture its `scanRequestId`, accept only same-ID visible/assistive status within one second and inventory from that request's committed generation within ten seconds, reject generic/loading/prior/automatic state, and require the same at least nine runs to pass both sub-100-ms interactions; record profile/digest/request ID/generation/environment with no snapshot reuse, intentional cache reset, or omissions beyond personal identifiers/absolute paths in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
...
- [ ] T1054 Record SC-004 pass/fail and denominators, including Global inert fixtures, with the fixed startup helper isolated and zero customization-derived activation/child/MCP/network/rejected reads/environment substitution/mutation-capable calls/changed-byte publication; record unchanged content/length/identity/link/mode/mtime/ctime/xattr/ACL observations, OS-only atime separately as neither failure nor proof, and publication-authority revocation with late-result/resource cleanup without hard-cancellation claims in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1055 Record SC-005 pass/fail and denominators for exact credentials/environment references, zero process substitution, no masking/reveal, and unchanged fixtures; separately prove authenticated Diagnostics duplicate no source values and OperationalEvent captures use only fixed codes plus session/source/file/scan/operation IDs with zero paths/roots/filenames/content/metadata/capabilities/bodies/raw errors/exceptions/Diagnostic arguments in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1056 Execute SC-001 first with exactly twenty non-replaced participants under the maintainer-owned bilingual study plan; record the actual OS default handler or its unavailability and, when resolvable, actual browser family/revision; when automatic opening is disabled, unsupported, fails, is unavailable/unidentifiable, or resolves outside the certification baseline, use and record the printed-URL manual fallback in a pinned certified browser; record the exact two-minute boundary and every unsuccessful equipment/environment/product outcome, and require at least nineteen successful launch/open results in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1057 Execute SC-006 after SC-001 with the same exactly twenty participants and prepared file state; record the actual default handler/browser and use the same documented pinned-certified-browser manual fallback conditions as T1056; enforce the exact two-minute boundary, four-field ground truth, and no replacements, then have every participant attempt comparison and Global consent; record all workflow outcomes and automatic safety events, use two independent fixed-rubric classifications only for suspected non-safety product blockers, count disagreement as critical without a third adjudicator, and require at least eighteen successful identifications plus all twenty complete attempts with zero critical issues in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1058 Record SC-007 pass/fail and denominators for unreadable, malformed, environment-resource-failed, cyclic, stale, and boundary fixtures; distinguish complete/contracted-partial continuity from environment-resource and other fatal attempts that publish zero new or partial results and retain only the last snapshot; require only an explicit fatal rescan to mark its affected Source stale, while an automatic first Repository failure or initial Global enable failure preserves any pre-existing snapshot state without creating a stale overlay; require 100% unaffected usability and actionable diagnostics; explicitly record that Inspector defines no file-size/count validation and resource availability yields no valid/invalid, correctness/compliance, or lint verdict in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
...
- [ ] T1060 Record SC-009 pass/fail across 100% of maintained origin-file-less Source Condition Fact fixtures, proving the correct Source, tool, product surface, documented condition or unavailable state, and evidence, with zero physical or synthetic files, file IDs, Source-relative Paths, authored source text, comparison targets, relationship origins, local or hosted reads, and network requests in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1061 Inspect the complete diff/tarball for correctness, untested branches, literal/environment failures, Fact synthesis/I/O, mutation/atime evidence, path-bearing operational output, file-size/count validation leakage, semantic analysis/ranking/verdict/remediation features, acknowledgement/API confusion, publication-authority or hard-cancellation claims, coordinator leaks, stale/source-root/engine/package/bilingual claims, and SC-002 request-ID or privacy leakage; repeat affected evaluations in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
- [ ] T1062 Resolve every failure or unresolved concern produced by T1041 and T1061, then run `pnpm run test:docs` and `git diff --check` and record passing results for the absence of Inspector-defined numeric resource caps; environment-resource failure that aborts its attempt, publishes no item, Source, recognition, derived result, scan-result record or response, or generation, and retains only the prior committed snapshot; contracted-partial publication only for deterministic entry-local non-capacity failures after complete traversal; atomic publication; late-result discard; request correlation; and bilingual traceability in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`; this gate remains incomplete while any T1041/T1061 failure, unresolved concern, command failure, or diff error remains, and T1063, pull-request approval, and release work MUST NOT begin until it passes
- [ ] T1063 Perform and record an explicit principle-by-principle release Constitution Check, confirm that every known violation is resolved and every residual uncertainty has an owner and concrete resolution path, and require the same explicit check in the pull request review before approval in `specs/001-inspect-agent-customizations/validation.md` and `specs/001-inspect-agent-customizations/validation.ja.md`
...
| 95 Global Consent Preview | US4 | Users can review the exact Global roots, patterns, exclusions, environment-resource status, and contract version before enabling inspection. |
...
| 99 Tool-Specific Global Result Integration | US4 | Zero to three separately identified one-root tool Sources appear only after their own complete or contracted-partial commits and reuse detail/comparison workflows. |
...
- Phase 96 establishes selector-free all-eligible consent, serialized per-tool controls, and Codex provisional admission; Phases 97–98 add independent Claude and Copilot admission; Phase 99 publishes zero to three separate one-root Sources only through each tool job's atomic complete or contracted-partial commit.
- Phase 102 task T1037 is a hard semantic-drift gate: later implementation and validation phases may proceed only after the allowlist, plan, contracts, data model, and task vocabulary agree that product-defined capacity ceilings are forbidden and environment-resource failures abort their attempt, publish no item, Source, recognition, derived result, scan-result record or response, generation, or contracted-partial result, and retain only the prior committed snapshot.
...
4. Atomically publish zero to three separate tool-specific Sources through their own complete or contracted-partial jobs, preserving Repository and sibling Sources and never merging roots.
...
- One physical file is read once per generation and may retain multiple tool recognitions and multiple direct provenances.
...
- Global inspection has one consent/control record and zero to three separately identified Sources, at most one per supported tool and exactly one root per Source. Provisional validation/scan work is not a Source; successful complete or contracted-partial commits publish or replace only the applicable tool Source while carrying Repository and sibling Sources into the session-wide generation. Source IDs remain process-lifetime stable while generation-owned graph IDs rekey.
