...
**Purpose**: The gaps `/speckit-converge` found between the specification, plan, and tasks and the code on 2026-09-10, each traced to its source and gap type.

- [X] T068 Add the `gemini` member to the Global boundary integration gate — the five-member transaction tuple and the per-member scan case asserting `homes.expectedCandidatePaths.gemini` admitted and `homes.nearMissPaths.gemini` never enumerated, opened, or read — in `tests/integration/global-boundaries.test.ts`, per T041 / FR-010 / SC-002 (partial).
- [X] T069 Add the `.gemini/skills/<a>/<b>/SKILL.md` near miss to the Gemini skill fixture's `nearMissPaths` in `tests/fixtures/repositories/build-fixtures.ts` and assert its rejection in `tests/integration/repository-scan.test.ts`, per SC-002 / QR-002 (partial).
- [X] T070 Add session and history state to the Gemini CLI clause of the parent FR-018 in `specs/001-inspect-agent-customizations/spec.md` and `spec.ja.md`, per FR-010 (partial).
- [X] T071 Amend the parent FR-045 in `specs/001-inspect-agent-customizations/spec.md` and `spec.ja.md` — the member is one of five FR-013 preview entries, and `skills/<skill-name>/SKILL.md` is the location Codex, Copilot, and Gemini CLI document — per FR-012 (partial).
- [X] T072 Extend the `confirmedTools` cases in `tests/unit/app/session-view-state.test.ts` to the five-member tuple, per T039 / QR-002 (partial).
- [X] T073 Add the `gemini` label and mark expectations to `tests/unit/shared/display-text.test.ts` and add `gemini` to its product-identifier guard regex, per T003 / QR-002 (partial).
- [X] T074 Add built repository fixtures for a string `context.fileName: "AGENTS.md"` (a root `AGENTS.md`, a nested `AGENTS.md`, a root `GEMINI.md`) and for an invalid value in `tests/fixtures/repositories/build-fixtures.ts`, asserted in `tests/integration/repository-scan.test.ts`, per T052 / FR-004 (partial).
- [X] T075 Update the consent workflow ground truth — `expectedDirectories` with `~/.gemini`, and a `matchRule` naming `GEMINI_CLI_HOME` and its `.gemini` join — in `tests/usability/sc001-sc006-study-inputs/ground-truth.json` and `ground-truth.ja.json`, per T060 / US2/AC1 (partial).
- [X] T076 Reword the `GlobalMemberId`, `GlobalPreviewEntryDto.member`, and `GlobalConsentPreviewDto.entries` doc comments in `src/shared/api-types.ts` and the `~/.agents` sentence in the `GLOBAL_MEMBER_TEXT` comment of `src/shared/api-text.ts` to four tools and five members, per T015 / FR-001 (partial).
- [X] T077 Update `## Registry completeness` in `specs/001-inspect-agent-customizations/contracts/runtime-composition.md` and `.ja.md` to the 47 strategy IDs the four vendor contracts reference, and add `gemini.behavior.user.extensions` and `google.gemini-cli.extensions-reference` to the `shared.excluded.managed-remote-state` row, which `src/shared/registries/shared/relations.ts` already carries, per FR-015 / QR-004 (partial).
- [X] T078 Add the Gemini CLI rows to `## Normative relationship-only registry` in `specs/001-inspect-agent-customizations/contracts/runtime-composition.md` and `.ja.md` — the context file's `@file.md` import, a command's `!{...}` and `@{...}` references, a skill's resource paths, a hook's `command`, an agent's `mcpServers` and `tools` — and update the relationship-only ID count, per FR-005 / QR-001 (partial).
- [X] T079 Update the `**Denominators.**` paragraph in `specs/001-inspect-agent-customizations/validation.md` and `.ja.md` to the manifest version 4 set — 36 `(tool, kind)` rows, four tools, four Global source forms — per T062 / QR-002 (partial).
- [X] T080 Amend the remaining parent artifacts in both languages — `spec.md` User Story 1 scenario 1 ("all three tools") and User Story 4 scenario 2 ("four-member preview"); `contracts/http-api.ja.md` member lists ("0〜4個", "1〜4個", no `member: gemini`); `quickstart.md` ("three documented tool-home environment properties", the consent-view roots without Gemini CLI, "all three vendor-contract language pairs"); `contracts/official-sources.md` and `.ja.md` ("the three vendor contracts", "all six inputs") — under `specs/001-inspect-agent-customizations/`, per FR-001 / QR-004 (partial).
- [X] T081 Correct `## VendorSurface` in `specs/002-gemini-cli-support/data-model.md` and `data-model.ja.md`: `VENDOR_SURFACE_TEXT['gemini-cli']` is `CLI`, because the table names the surface within its product and `SUPPORTED_TOOL_TEXT` names the product, as `src/shared/registries/behavior-text.ts` does, per data-model.md § VendorSurface (contradicts).
- [X] T082 Reword every member-count comment to five members and four tool homes in `src/server/host/global-consent.ts` (module header, `GlobalRootInputCapture`, "fixed-four", "all four"), `src/server/session/global-control.ts`, `src/server/inspection/global-admission.ts`, `src/server/host/devframe-app.ts` ("the four members", "all three tools"), `src/app/session/api-client.ts`, `src/app/session/view-state.ts`, `src/server/host/file-opener.ts`, `src/app/pages/global-consent.vue`, and `src/server/session/session.ts`, per FR-001 / QR-001 (partial).
- [X] T083 Reword every tool-count comment to four products in `src/app/components/inspection/declaration-order.ts` ("All three products' skills", "all three products' agent fields", "three products spelling one declaration", "the four carrier schemas"), `src/server/inspection/rules/mcp/server-map.ts` ("all four documents"), `src/server/inspection/rules/agents/declared-name.ts` ("two of the three products"), `src/app/components/inventory/rows/SkillRow.vue`, `src/app/components/inventory/rows/skill-row-files.ts`, the same-name statement comment in `src/shared/api-types.ts`, and `src/app/composables/custom-agent-comparison.ts`, per QR-001 (partial).
