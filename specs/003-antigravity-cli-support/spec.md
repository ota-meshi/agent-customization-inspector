...
- Q: The parent specification repeats its first-use evaluation only when the designated file's ground truth moves. That file is the repository root `AGENTS.md`, and this tool reads it. Is a run owed? → A: Yes. The file's ground truth gains this tool — and Claude Code, which reads the file too — so it names four recognizing tools, and the condition the parent set is met: the study inputs are updated and the twenty agent-driven sessions are run before release, with the result recorded in the parent's `validation.md`. A reader of the designated file now has a harder answer to give, and a criterion measured against a page that no longer matches would be measuring nothing. (Amended 2026-09-24: Claude Code reads the file too, so the run was owed again.)
