...
- Q: A static analysis of the published `agy` 1.2.0 binary shows the terminal discovering only the folder shape, and shows an unnamed skill resolving to `SKILL` rather than to its folder name. Does the release follow the binary or the pages? → A: Both, each where it is the better evidence. The flat shape stays admitted, because a reader who followed the terminal's own page has that file and declining it would show them nothing about it, while admitting it costs a row the vendor's own documentation supports; the observation is recorded on the vendor contract and the rule goes when a page or a later build settles it. The naming follows the binary, because a row's name is the one the recognizing product resolves and that is a fact about resolution rather than about which page is right — so an unnamed skill of this vendor is named `SKILL`, and the same file is named by its folder for the products that resolve it that way. The global allowlist follows the binary too, admitting both documented global roots rather than ranking them, because the terminal walks both.
...
- **FR-006**: A custom agent MUST be published under the custom-agent kind in both admitted
  shapes, named by the `name` its frontmatter declares and by the file's or the directory's own
...
- **FR-009**: The Global inspection path allowlist for that member MUST admit exactly:
  `GEMINI.md`; `config/mcp_config.json`; `config/hooks.json`; a custom agent below
  `config/agents/`; a skill at `antigravity-cli/skills/<name>/SKILL.md`,
  `config/skills/<name>/SKILL.md`, or `antigravity-cli/skills/<name>.md`; and
  `antigravity-cli/settings.json`. Both documented global skill roots are admitted rather than
  ranked, because the terminal walks both. The editor extensions' own global skill directory
