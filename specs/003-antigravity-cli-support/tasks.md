...
- [X] T020 [P] [US1] Add the repository fixture builders — a skills tree holding skill folders under `.agents/` and `.agent/`, a rules tree holding one file per documented activation mode under both spellings and one in a subdirectory, a standalone `.agents/hooks.json`, an agents tree holding both shapes, an MCP tree with a local and a remote server plus a legacy key, and a context tree with the root pair, a nested pair, and a directory's `.agents/` pair — with their near-miss paths, a flat `.agents/skills/<name>.md`, `.agent/skills/<name>.md`, and `.agents/plugins/` among them, in `tests/fixtures/repositories/build-fixtures.ts`. (Amended 2026-09-24 by T085: the Rules page documents context files and rules at every level, and no page documents a flat skill.)
...
- [X] T022 [P] [US1] Add unit tests for the folder-shaped skill unit and the vendor's own invocation-name answer — an absent `name` on a skill folder's `SKILL.md` resolves to the folder for this tool exactly as it does for the products beside it, so one file stays one row — in `tests/unit/inspection/antigravity-metadata.test.ts` (contracts/vendors/antigravity-cli.md § Known uncertainties item 7). (Amended 2026-09-24 by T084: no page documents a flat skill, so the tests of its unit are removed.)
- [X] T023 [US1] Add an integration scan over the repository fixtures asserting the listed set, the root `GEMINI.md` with two recognitions, the root `AGENTS.md` with four, a nested context file carrying this tool's recognition beside those of the other products that read it, and no read request for any near miss, in `tests/integration/repository-scan.test.ts`. (Amended 2026-09-24 by T083 and T085: the Rules page documents the context files at every level.)
...
- [X] T025 [US1] Wire the context pair at every depth to the instruction units in `src/server/inspection/rules/instructions/antigravity.ts` — a workspace file governing the directory holding it, or holding its `.agents/`, and a global file the whole boundary — and name `antigravity.repo.context` in the comment on `copilot.repo.instructions.gemini-root` in `src/shared/registries/copilot/rules.ts` as the static rule that gives the root row its second reader. (Amended 2026-09-24 by T083: the Rules page documents the context files at every level.)
- [X] T026 [US1] Compile this vendor's skill rules to the folder-shaped skill unit every vendor shares — the name is the frontmatter `name`, or the skill folder when it declares none — in `src/server/inspection/rules/skills/antigravity.ts` (research.md § 2). (Amended 2026-09-24 by T084: no page documents a flat skill, so the file-shaped unit is removed.)
- [X] T027 [US1] Recognize every skill admission through that one unit in `src/server/inspection/recognizers/candidate.ts`, with no discriminant between skill shapes. (Amended 2026-09-24 by T084: with one skill shape there is nothing to discriminate.)
...
- [X] T030 [US1] Publish every skill definition with its folder's companion census, so the row draws a supporting-file count wherever the folder ships companions, in `src/app/components/inventory/rows/skill-row-files.ts` and `SkillRow.vue`. (Amended 2026-09-24 by T084: every skill is its folder.)
- [X] T031 [US1] Render every skill's detail with its file panel and tab strip, headed by the skill's own folder, in `src/app/pages/skills/detail/[source]/[...path].vue`. (Amended 2026-09-24 by T084: the panel-only detail is removed with the flat skill.)
...
- [X] T033 [P] [US1] Add `tests/e2e/antigravity-skills-detail.spec.ts` covering a skill folder in `.agents/skills/` with its file panel, and a flat Markdown file beside it on no row. (Amended 2026-09-24 by T085: no page documents a flat skill.)
...
- [X] T039 [P] [US2] Replace the fourth home's rows in `tests/fixtures/global-homes/build-fixtures.ts` — dropping its environment-variable entry, which no property backs any more, and with it the member-id and root expectations in `tests/contract/http-api-global.test.ts` that read the fixture — — `GEMINI.md`, `AGENTS.md`, `config/GEMINI.md`, `config/AGENTS.md`, a rule under each of `config/rules/` and `antigravity-cli/rules/`, `config/mcp_config.json`, `config/hooks.json`, `config/agents/`, a skill folder under each of `antigravity-cli/skills/` and `config/skills/`, `antigravity-cli/settings.json`, and the near misses and excluded paths beside them, a flat file under `antigravity-cli/skills/`, the plugin copies, import manifest, credentials, and session state among them — and update `tests/fixtures/global-homes/README.md` and `README.ja.md` to the three environment properties. (Amended 2026-09-24 by T085: the Rules page documents the home's other context files and its rules, and no page documents a flat skill.)
...
- [X] T043 [US2] Wire the Global context, rule, MCP, agent, and skill rules to their compiled units in `src/server/inspection/rules/**/antigravity.ts`, the context rule reaching the global instruction unit whose range is the whole boundary and the skill rule the folder-shaped unit. (Amended 2026-09-24 by T083 and T084: the Rules page documents global context files and rules, and no page documents a flat skill.)
...
- [X] T052 [P] [US3] Extend the skills fixture with a skill folder the three products read and a flat Markdown file beside it that none reads, in `tests/fixtures/repositories/build-fixtures.ts`. (Amended 2026-09-24 by T085: no page documents a flat skill.)
- [X] T053 [P] [US3] Assert that a skill folder in `.agents/skills/` is one row carrying one definition per recognizing product and a flat file beside it is on no row, in `tests/integration/repository-scan.test.ts`, and that two folders of one name under `.agents` and `.agent` clash for this vendor, in `tests/unit/shared/skill-collision.test.ts`. (Amended 2026-09-24 by T085: no page documents a flat skill.)
- [X] T054 [P] [US3] Needs no test of its own: every skill is its folder, and the count a row draws from the folder's census is `tests/unit/app/skill-row-files.test.ts`'s existing case. (Amended 2026-09-24 by T084: no skill is file-shaped.)

### Implementation for User Story 3

- [X] T055 [US3] Group every skill definition under one invocation name with one definition per file per recognizing product, in `src/server/session/session.ts` and the skill grouping it calls. (Amended 2026-09-24 by T084: every definition is a folder's.)
...
- [X] T057 [P] [US3] Compare every copy in `tests/e2e/skills-comparison.spec.ts` and `tests/e2e/skill-metadata-comparison.spec.ts` as a folder, file by corresponding file, as every vendor's copies are. (Amended 2026-09-24 by T085: no page documents a flat skill.)
- [X] T058 [P] [US3] Add no copy naming a shape to the row's definition list in `src/app/components/inventory/rows/skill-row-files.ts`: every skill is its folder, and which products read a folder is what the marks on its line state. (Amended 2026-09-24 by T084: no page documents a flat skill.)
...
- [X] T061 [P] Update the first-use study inputs to name the tools this release supports, drop the environment property FR-008 removes, and set the designated file's recognizing tools to the four that read the repository root `AGENTS.md`, in `tests/usability/sc001-sc006-study-inputs/`. (Amended 2026-09-24 by the parent's T1218: Claude Code reads the file too.)
...
- [X] T095 Rewrite T061 and the Clarifications answer on the first-use evaluation in `specs/003-antigravity-cli-support/tasks.md`, `tasks.ja.md`, `spec.md`, and `spec.ja.md` to the four recognizing tools the designated `AGENTS.md` now has, as the parent's T1218 set its ground truth, each with a dated note saying briefly why, so the next run is not scored against a superseded count, per QR-003.
