...
- Q: `.agents/skills/` は `deploy/SKILL.md` の隣に `deploy.md` を抱えうる。inventory は1行か2行か？ → A: 1行であり、その行はフォルダだけを持つ。skill の行の単位は「1つの名前を各製品がどう解決するか」であり、それが `.agents/skills/x/SKILL.md` と `.claude/skills/x/SKILL.md` を1行に置いている。フラットな `deploy.md` はどの製品の skill でもない (FR-004) ので、どの行にも載らず、2つの間の優先順位も述べない。（2026-09-24 修正: フラットファイルが allowlist から外れたので、行は1つの形だけを持つ。）
- Q: このリリースはこのツールについてどの skill の形を admit するのか？ → A: `SKILL.md` を持つフォルダを、workspace と home で admit する。端末の skill の location を示すページはどれもその形を示しており — 端末の skills ページも vendor の Agent Skills ページも同様である — 出荷された端末は `skills/` の直下にあるフラットな Markdown ファイルを、名前を読む前に除外するので、フラットファイルはどこでも admit しない。`.agents/skills/` は vendor の3製品が読む1つのディレクトリなので、1つの `.agents/skills/deploy/SKILL.md` は3つの recognition を持つ。（2026-09-24 修正: フラットな形は allowlist から外れた。端末自身のページがもはやそれを文書化せず、端末もそれを検出しないからである。）
...
- Q: skills と rules のページはどちらも、現行の `.agents/` の隣に旧綴りの `.agent/` を記録している。これは admit するのか？ → A: admit する。ただし、そのページがその location について文書化している形に限る。すなわち `.agent/skills/<name>/SKILL.md` と `.agent/rules/<name>.md` であり、rules ディレクトリは現行の綴りが admit されるすべての深さで admit する。それ以外はない。後方互換は location を述べるページが述べているので、旧綴りが届くのはそのページがそこで示す形までである。（2026-09-24 修正: rules ディレクトリは両方の綴りですべての深さで admit し、フラットな skill はどちらの綴りでも admit しない。）
- Q: 公開されている `agy` 1.2.0 バイナリの静的解析は、端末がフォルダ形しか検出しないこと、および `name` 無しの skill がフォルダ名ではなく `SKILL` に解決されることを示している。このリリースはバイナリとページのどちらに従うのか？ → A: ページに従う。検出については両者が今は一致している。ページはフォルダ形だけを文書化し、バイナリはフォルダ形だけを検出する。名付けはバイナリに従わない。行の名前は認識する製品が解決する名前であり、同じバイナリの `GetSkillsCreatePath` は `{workspace}/.agents/skills/{skill_name}/SKILL.md` を組み立てる。つまり端末自身がフォルダを名前の持ち手として扱っている。よって名前無しのフォルダはそのフォルダ名で名付ける。これは同じファイルを読む他の2製品が与える答えでもある。global の allowlist はバイナリに従い、2つの global skill root を順位づけずに両方 admit する。端末が両方を歩くからである。（2026-09-11 修正: この答えの名付けの側を、FR-004 と出荷される unit が述べる fallback に合わせた。バイナリ内の2つの読み取りを互いに突き合わせた結果である。2026-09-24 修正: 検出の側はページに従う。ページはもはやフラットな skill を文書化しない。）
...
- Q: 親仕様は、指定ファイルの ground truth が動いたときにだけ初回利用評価をやり直す。そのファイルはリポジトリルートの `AGENTS.md` であり、このツールはそれを読む。実施は必要か？ → A: 必要である。ファイルの ground truth にはこのツールと、同じファイルを読む Claude Code が加わって読み手は4つになり、親が定めた条件を満たす。そのため study input を更新し、リリース前に20セッションのエージェント駆動実行を行い、結果を親の `validation.md` に記録する。指定ファイルの読み手が増えたことで読み手が述べるべき答えは難しくなっており、一致しなくなったページに対して測る criterion は何も測っていないことになる。(2026-09-24 修正: Claude Code もこのファイルを読むので、実行は再び必要になった。)
...
- Q: vendor の Rules ページは今、リポジトリルートより下の context file と rule、global の `GEMINI.md` の隣の global な `AGENTS.md`、home の `config/` 配下の同じ2つ、そしてモジュール化された global rule を文書化している。allowlist はそれに従うのか？ → A: 従う。ページが述べるものに正確に限る。リポジトリでは、任意のディレクトリとその `.agents/` にある `GEMINI.md` と `AGENTS.md`、およびすべての深さで rules ディレクトリの直下の `.md` の子。home では、`GEMINI.md` の隣の `AGENTS.md`、`config/GEMINI.md`、`config/AGENTS.md`、および `config/rules/<name>.md` または `antigravity-cli/rules/<name>.md` の rule。recognition は vendor が階層を文書化するのを待っており、今それが文書化された。
- Q: 端末の skills ページはもはやフラットな `.md` の skill を示さず、出荷された端末はそれを除外する。フラットの rule は残るのか？ → A: 残らない。2つのフラット rule — `.agents/skills/<name>.md` と `antigravity-cli/skills/<name>.md` — をどちらも取り除き、それとともにファイル形の skill とフォルダ形の skill を見分ける仕組みも取り除く。ファイル形を admit する rule が無くなれば、どの skill もそのフォルダであり、1つの値しか取れない discriminant は同じ事実についての2つ目の state である。

## User Scenarios & Testing _(mandatory)_

### User Story 1 - Antigravity CLI をリポジトリのファイルの読み手として見る (Priority: P1)

開発者が `.agents/` ディレクトリを持つリポジトリを開く。そこには skill フォルダ、rules
ディレクトリ、hooks ファイル、MCP 設定、custom agent があり、ルートとサブディレクトリには
`GEMINI.md` と `AGENTS.md` がある。現状の
...
1. **前提** `.agents/skills/format-tests/SKILL.md` があるリポジトリ、**操作** skill inventory を
...
7. **前提** `packages/api/AGENTS.md` があるリポジトリ、**操作** instructions inventory を開く、
   **結果** そのファイルが Antigravity CLI を読み手に含めて挙がり、`packages/api/` を govern する。
...
1. **前提** `GEMINI.md`、`config/mcp_config.json`、`config/agents/reviewer.md`、
   ...
   `config/rules/style.md`、`antigravity-cli/settings.json` を持つ consent 済みの
...
### User Story 3 - 共有された skill フォルダをどの製品が読むかを見る (Priority: P3)

`.agents/skills/` は3つの製品が読む1つのディレクトリである。OpenAI Codex、GitHub Copilot、
Antigravity CLI はそろって、そこにある skill フォルダの `SKILL.md` を取る。そのディレクトリを見る
読み手は、1つのフォルダが3つの読み手を持つ1つの skill であること、そしてそのディレクトリの直下に
置かれた Markdown ファイルはどれの skill でもないことを見る必要がある。

**優先度の理由**: 最初の2つの story が既に届ける範囲の中の理解の問題であり、価値はあるが、この
機能を出荷する理由そのものではない。

**独立テスト**: `.agents/skills/` に skill フォルダとフラットな Markdown ファイルを置いた
リポジトリを調べ、フォルダの行が3製品すべてを述べ、フラットファイルがどの行にも載らないことを
確認する。

**受け入れシナリオ**:

1. **前提** `.agents/skills/release/SKILL.md`、**操作** skill inventory を開く、**結果** 行は
   そのフォルダを読む3製品すべてを名指す。
2. **前提** `.agents/skills/deploy.md`、**操作** skill inventory を開く、**結果** それは挙がらず、
   決して読まれない。
...
- **FR-002**: Antigravity CLI の Repository inspection path allowlist は次を正確に admit し
  なければならない (MUST): 任意のディレクトリ（`.agents/` を含む）の context file `GEMINI.md` と
  `AGENTS.md`、`.agents/skills/<name>/SKILL.md` の skill、ルートとその配下のすべてのディレクトリに
  ついての `<dir>/.agents/rules/<name>.md` の rule、hooks carrier の `.agents/hooks.json`、
...
- **FR-004**: Antigravity CLI の skill はフォルダ形で publish されなければならず (MUST)、
  frontmatter が宣言する `name`、宣言がなければ skill フォルダで名付けられなければならない
  (MUST)。その fallback は同じファイルを解決する他のすべての製品が使うものなので、1つの
  `SKILL.md` は2つの名前で2行に分かれることなく、3つの読み手を持つ1行であり続ける。skills
  ディレクトリの直下にある Markdown ファイルは、リポジトリでも home でも admit してはならない
  (MUST NOT)。引用したどのページもその形を文書化せず、出荷された端末は名前を読む前にそれを除外する
  (§ Clarifications)。
...
- **FR-009**: その member の Global inspection path allowlist は次を正確に admit しなければ
  ならない (MUST): `GEMINI.md`、`AGENTS.md`、`config/GEMINI.md`、`config/AGENTS.md`、
  `config/rules/<name>.md` または `antigravity-cli/rules/<name>.md` の rule、
  `config/mcp_config.json`、`config/hooks.json`、
  `config/agents/<name>.md` または `config/agents/<name>/agent.md` の custom agent、
  `antigravity-cli/skills/<name>/SKILL.md` または `config/skills/<name>/SKILL.md` の skill、
  `antigravity-cli/settings.json`。4つの context file はすべてのプロジェクトを等しく govern する。
  ページが、それらはすべてのプロジェクトに適用され常に有効であると述べているからである。
...
- **Antigravity CLI skill**: 1つの skill 名。リポジトリでも home でも `SKILL.md` を持つフォルダ
  として綴られる。
- **Antigravity CLI rule**: リポジトリまたは home の rules ディレクトリの直下にある1つの
  Markdown ファイル。frontmatter が宣言する activation mode を持つ。
...
- **QR-002**: skill kind は1つの行の形を保たなければならない (MUST)。skill はどの製品にとっても
  そのフォルダなので、どの rule も admit しないファイル形の skill を区別するフィールド、unit、
  分岐があってはならない (MUST NOT)。
...
- **QR-003**: 自動検証は Antigravity CLI について次を覆わなければならない (MUST): admit される
  ...
  near-miss。2つの recognition を持つ1ファイルとしてのルート `GEMINI.md`。入れ子の context file
  の組と、あるディレクトリの `.agents/` の組（それぞれが govern する範囲とともに）。1つの
  `.agents/skills/` ディレクトリ内と旧綴りの `.agent/` 配下の skill フォルダ（両方でフラットな
  ファイルが reject されること）。文書化された
...
- 上記の Antigravity CLI の surface は 2026-09-10 に、Rules と skills のページは 2026-09-24 に
  もう一度、`https://antigravity.google/docs/` の公式
