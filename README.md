...
The Spec Kit skills are committed here — `.agents/skills/speckit-*` for Codex and Antigravity
CLI, `.claude/skills/speckit-*` for Claude Code, `.github/skills/speckit-*` for Copilot — so
cloning is the whole setup and there is no `specify init` to run. A finished task is ticked in
`tasks.md` and `tasks.ja.md` together, in the change that finishes it.
